...
Surface: `GET /api/diagnostics/health` returns the last `HealthCheckResult` (or `{neverRun:true}`); `POST` runs one on demand (admin-token gated + rate-limited, mirroring `app/api/diagnostics/database/route.ts`). The activity feed renders `health_check` rows via the existing fallback-tolerant `ACTIVITY_TYPE_LABELS`/`ICONS` maps in `SettingsView.tsx`. Contract pinned by `tests/health-check.test.ts` (notably the "live run is NOT cleared" case).
