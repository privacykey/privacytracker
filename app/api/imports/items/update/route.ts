...
async function updateImportItemRoute(request: Request) {
  ...
  try {
    ...
    if ("appId" in body) {
      patch.appId = typeof body.appId === "string" ? body.appId : null;
    }
    if ("appName" in body) {
      patch.appName = typeof body.appName === "string" ? body.appName : null;
    }
    if ("developer" in body) {
      patch.developer =
        typeof body.developer === "string" ? body.developer : null;
    }
    if ("url" in body) {
      patch.url = typeof body.url === "string" ? body.url : null;
    }
    if ("iconUrl" in body) {
      patch.iconUrl = typeof body.iconUrl === "string" ? body.iconUrl : null;
    }
    if ("country" in body) {
      patch.country = typeof body.country === "string" ? body.country : null;
    }
    if ("scrapeError" in body) {
      patch.scrapeError =
        typeof body.scrapeError === "string" ? body.scrapeError : null;
    }
