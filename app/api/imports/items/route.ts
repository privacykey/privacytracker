  ...
  try {
    ...
    const cleaned = rawItems
      .map((item): Parameters<typeof addImportItemsAsync>[1][number] | null => {
        ...
        if (!IMPORT_ITEM_STATUSES.includes(status as ImportItemStatus)) {
          return null;
        }
        ...
        return {
          query,
          editedQuery:
            typeof item?.editedQuery === "string" &&
            item.editedQuery.trim().length > 0
          ...
          developer:
            typeof item?.developer === "string" ? item.developer : null,
          url: typeof item?.url === "string" ? item.url : null,
          iconUrl: typeof item?.iconUrl === "string" ? item.iconUrl : null,
          country: typeof item?.country === "string" ? item.country : null,
          scrapeError:
            typeof item?.scrapeError === "string" ? item.scrapeError : null,
          nextAttemptAt,
        };
      })
