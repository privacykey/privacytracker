...
export async function POST(request: NextRequest) {
  ...
  if (!body.appIds.every((id): id is string => typeof id === 'string' && id.length > 0)) {
    return NextResponse.json({ error: 'every appId must be a non-empty string' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ count: verdicts.length, verdicts }, { status: 201 });
  } catch (e) {
    console.error('[/api/verdicts/bulk POST] failed:', e);
    return NextResponse.json({ error: 'Failed to bulk-set verdicts' }, { status: 500 });
  }
}
