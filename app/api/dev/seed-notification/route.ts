...
import { recordAudit, readBoundedJson } from '@/lib/security';
...
export async function POST(request: Request) {
  const guard = requireMutationGuard(request, {
    ...
    rateLimit: {
      ...
      limit: 30,
      windowMs: 10 * 60_000,
      message: 'Rate limit exceeded for dev notification seeding. Try again later.',
    },
  });
  ...
  try {
    ...
    console.error('[/api/dev/seed-notification] createNotification failed:', error);
    recordAudit({
