...
export async function POST(request: Request) {
  ...
  try {
    const entry = addShortlistEntry({
      ...
      candidateDeveloper:
        typeof body.candidateDeveloper === "string"
          ? body.candidateDeveloper
          : undefined,
      candidateIconUrl:
        typeof body.candidateIconUrl === "string"
          ? body.candidateIconUrl
          : undefined,
      candidateStoreUrl: String(body.candidateStoreUrl ?? ""),
      candidateBundleId:
        typeof body.candidateBundleId === "string"
          ? body.candidateBundleId
          : undefined,
    ...
    const message =
      error instanceof Error ? error.message : "Shortlist add failed";
...
export async function DELETE(request: Request) {
  ...
  if (id) {
    ...
    return NextResponse.json(
      {
        error:
          "Provide `id`, both `sourceAppId` and `candidateAppleId`, or `all=1`",
      },
      { status: 400 }
    );
  }
