...
export async function POST(request: Request) {
  ...
  if (!rate.allowed) {
    return NextResponse.json({ ok: false, message: 'Rate limit exceeded. Try again shortly.' }, { status: 429 });
  }
...
async function testProvider({
  ...
  baseUrl: string;
  apiKey: string;
}): Promise<{ ok: boolean; message: string; status?: number; modelsCount?: number }> {
...
async function pingOpenAiCompatible({
  ...
  if (!res.ok) {
    ...
    return {
      ok: false,
      status: res.status,
      message: shortBodyMessage(res.status, body) || `Endpoint returned HTTP ${res.status}.`,
    };
  }
  ...
  try {
    ...
    if (Array.isArray(payload?.data)) modelsCount = payload.data.length;
    else if (Array.isArray(payload?.models)) modelsCount = payload.models.length;
  } catch {
...
async function pingAnthropic({
  ...
  if (!res.ok) {
    ...
    return {
      ok: false,
      status: res.status,
      message: shortBodyMessage(res.status, body) || `Anthropic returned HTTP ${res.status}.`,
    };
  }
...
function shortBodyMessage(status: number, body: string): string {
  ...
  if (!cleaned) {
    ...
    if (status === 403) return 'Forbidden — API key does not have access to this endpoint.';
    if (status === 404) return 'Not found — double-check the base URL.';
    if (status === 429) return 'Rate limited — try again shortly.';
    return '';
  }
  const snippet = cleaned.slice(0, 180);
  if (status === 401) return `Unauthorized (${snippet})`;
  if (status === 403) return `Forbidden (${snippet})`;
...
function friendlyNetworkMessage(message: string): string {
  ...
  if (/ECONNREFUSED|refused/i.test(message)) return 'Connection refused — is the server running at this URL?';
  if (/ENOTFOUND|getaddrinfo/i.test(message)) return 'Hostname not found — check the base URL.';
