...
export async function POST(request: Request) {
  ...
  if (provider === "disabled") {
    return NextResponse.json({
      ok: false,
      message: "Pick an AI provider first.",
    });
  }
...
async function discoverModels({
  ...
  const primary = await fetchOpenAiCompatibleModels({
    baseUrl,
    apiKey,
    provider,
  ...
  const fallback = await fetchOllamaTags({ baseUrl }).catch(
    () => [] as DiscoveredModel[]
  );
  return fallback;
}
...
async function fetchOpenAiCompatibleModels({
  ...
  const ids = payload.data
    ...
    .filter(
      (id) =>
        Boolean(id) && (provider !== "openai" || isLikelyOpenAiTextModel(id))
    );
...
function isLikelyOpenAiTextModel(id: string): boolean {
  ...
  if (
    /embedding|embed|whisper|tts|audio|transcribe|image|dall-e|moderation|realtime/.test(
      lowered
    )
  ) {
    return false;
