...
export async function POST(request: Request) {
  ...
  if (provider === 'disabled') {
    return NextResponse.json({ ok: false, error: 'Pick an AI provider before running a sample summary.' }, { status: 400 });
  }

  const model = typeof body.model === 'string' ? body.model.trim() : '';
  if (!model) {
    return NextResponse.json({ ok: false, error: 'Pick a model before running a sample summary.' }, { status: 400 });
  }
  if (model.length > 200) {
    return NextResponse.json({ ok: false, error: 'Model ID is too long.' }, { status: 400 });
  }

  const apiKey = resolveSubmittedApiKey(body.apiKey);
  if (providerRequiresApiKey(provider) && !apiKey) {
    return NextResponse.json({ ok: false, error: 'An API key is required to test this provider.' }, { status: 400 });
  }
  ...
  if (!verdict.ok) {
    return NextResponse.json({
      ok: false,
      error: verdict.error === 'private_host'
  ...
  try {
    ...
    const message = friendlyAiMessage(error instanceof Error ? error.message : String(error));
    recordActivity({
      type: 'policy_summary',
      status: 'error',
      appId: null,
...
function friendlyAiMessage(message: string): string {
  if (/aborted|timeout/i.test(message)) return 'Timed out while generating the sample summary.';
  if (/ECONNREFUSED|refused/i.test(message)) return 'Connection refused — is the model server running at this URL?';
  ...
  if (/fetch failed/i.test(message)) return 'Could not reach the model endpoint.';
  return message;
}
