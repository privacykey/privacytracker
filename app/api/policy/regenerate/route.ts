...
export async function POST(request: Request) {
  ...
  try {
    const body = await readBoundedJson<{ appId?: unknown; phase?: unknown; stream?: unknown }>(
      request,
      8 * 1024,
    );
    ...
    const rawPhase = typeof body?.phase === 'string' ? body.phase.trim() : 'all';
    const phase = (VALID_PHASES.includes(rawPhase as PolicyPhase) ? rawPhase : 'all') as PolicyPhase;
    const wantStream = body?.stream === true;

    const app = db
      .prepare('SELECT id, name, developer, privacyPolicyUrl FROM apps WHERE id = ?')
      .get(appId) as
      | { id: string; name: string; developer: string | null; privacyPolicyUrl: string | null }
      | undefined;
    ...
    if (!app.privacyPolicyUrl) {
      return NextResponse.json(
        { error: 'This app does not expose a developer privacy-policy link on its App Store page.' },
        { status: 409 },
      );
    }
    ...
    const forceResummarise = phase === 'fetch' || phase === 'all' || phase === 'summarise';
      ...
      const stream = new ReadableStream<Uint8Array>({
        async start(controller) {
          ...
          const phaseStream: PolicyPhaseStream = {
            emit: (phaseEvent: PolicyRunPhase) => write({ type: 'phase', phase: phaseEvent }),
          };
