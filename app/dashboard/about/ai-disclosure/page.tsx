...
export default async function AiDisclosurePage() {
  if (resolveFlagFromDb('flag.about.ai_disclosure') !== 'on') notFound();
  const t = await getTranslations('ai_disclosure_page');
  return (
    <>
      <Nav />
      <div className="ai-disclosure-page">
        <div className="ai-disclosure-shell">
          <nav aria-label={t('breadcrumb_aria')} className="ai-disclosure-crumbs">
          ...
          <header className="ai-disclosure-header">
            <h1>{t('title')}</h1>
            <p className="ai-disclosure-lede">
              {t('lede')}
            </p>
          </header>
          ...
          <section className="ai-disclosure-section">
            ...
            <p>
              {t('section_provider_intro')}
            </p>
            <ul className="ai-disclosure-list">
              <li>
                <strong>{t('provider_disabled_label')}</strong>{t('provider_disabled_text')}
              </li>
              <li>
                <strong>{t('provider_openai_label')}</strong>{t('provider_openai_text')}
              </li>
              <li>
                <strong>{t('provider_anthropic_label')}</strong>{t('provider_anthropic_text')}
              </li>
              <li>
                <strong>{t('provider_custom_label')}</strong>{t('provider_custom_text')}
              </li>
            </ul>
            <p>
              {t('section_provider_footer_pre')}<code>{t('section_provider_footer_code')}</code>{t('section_provider_footer_post')}
            </p>
          </section>
          ...
          <section className="ai-disclosure-section">
            ...
            <ul className="ai-disclosure-list">
              <li><strong>{t('model_claude')}</strong></li>
              <li><strong>{t('model_gemini')}</strong></li>
              <li><strong>{t('model_gpt')}</strong></li>
            </ul>
          ...
          <section className="ai-disclosure-section">
            ...
            <p>
              {t('section_off_pre')}
              <Link href="/dashboard/settings" className="ai-disclosure-inline-link">
                {t('section_off_settings_link')}
              </Link>
              {t('section_off_post_pre')}<strong>{t('section_off_disabled')}</strong>{t('section_off_post_suffix')}
            </p>
          </section>
