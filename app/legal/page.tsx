...
function pkgVersion(name: string): string {
  ...
  const raw = deps[name];
  if (!raw) {
    throw new Error(`pkgVersion: "${name}" is not listed in package.json (dependencies or devDependencies).`);
  }
...
const LICENSE_META: Record<SpdxLicense, LicenseMeta> = {
  ...
  'ISC': {
    id: 'ISC',
    name: 'ISC License',
    blurb: 'Functionally equivalent to MIT but shorter. Permissive, attribution required, no warranty.',
...
const DEPENDENCIES: DependencyEntry[] = [
  ...
  },
  {
    ...
    license: 'MIT',
    about: 'Synchronous, zero-config SQLite binding for Node — binary wheel built against N-API.',
    usage:
  ...
  },
  {
    ...
    license: 'Apache-2.0',
    about: 'Apache ECharts — interactive charting library originally from Baidu, now an Apache top-level project.',
    usage:
  ...
  },
  {
    ...
    license: 'MIT',
    about: 'Thin React wrapper around ECharts — handles mount/unmount and option diffing.',
    usage:
  ...
  },
  {
    ...
    license: 'Apache-2.0',
    about: 'WebAssembly port of the Tesseract OCR engine. Runs entirely in the browser or in Node.',
    usage:
  ...
  },
  {
    ...
    license: 'Apache-2.0',
    devOnly: true,
    about: 'Command-line tool for building Tauri desktop apps (Rust-backed webview wrappers).',
    usage:
  ...
  },
  {
    ...
    license: 'MIT',
    devOnly: true,
    about: 'TypeScript type definitions for better-sqlite3, maintained by the DefinitelyTyped community.',
    usage: 'Dev-time only — provides autocomplete and type checking for the DB binding. Not shipped.',
    links: {
  ...
  },
  {
    ...
    usage: 'Dev-time only — provides types for built-in modules (fs, path, crypto, etc.). Not shipped.',
    links: {
      repo: 'https://github.com/DefinitelyTyped/DefinitelyTyped',
  ...
  },
  {
    ...
    usage: 'Dev-time only — provides types for the React component API. Not shipped.',
    links: {
      repo: 'https://github.com/DefinitelyTyped/DefinitelyTyped',
  ...
  },
  {
    ...
    usage: 'Dev-time only — provides types for the DOM renderer API. Not shipped.',
    links: {
      repo: 'https://github.com/DefinitelyTyped/DefinitelyTyped',
  ...
  },
  {
    ...
    license: 'MIT',
    devOnly: true,
    about: 'Tiny shim that sets environment variables the same way across Unix and Windows shells.',
    usage:
...
function groupByLicense(deps: DependencyEntry[]): Record<SpdxLicense, DependencyEntry[]> {
    ...
    <article className="legal-dep-card">
      <header className="legal-dep-head">
        <h3 className="legal-dep-name">
          {dep.name}
          {dep.devOnly && <span className="priv-nec-chip priv-nec-optional" style={{ marginLeft: 8 }}>dev-only</span>}
        </h3>
        <span className="legal-dep-version">v{dep.version}</span>
      </header>
      <p className="legal-dep-use"><strong>How we use it:</strong> {dep.usage}</p>
      <p className="legal-dep-about">{dep.about}</p>
      <div className="legal-dep-links">
        {dep.links.website && (
          <a className="legal-dep-link" href={dep.links.website} target="_blank" rel="noopener noreferrer">
            Website ↗
          </a>
        )}
        {dep.links.repo && (
          <a className="legal-dep-link" href={dep.links.repo} target="_blank" rel="noopener noreferrer">
            Source ↗
          </a>
        )}
        {dep.links.npm && (
          <a className="legal-dep-link" href={dep.links.npm} target="_blank" rel="noopener noreferrer">
            npm ↗
          </a>
        )}
        {dep.links.docs && (
          <a className="legal-dep-link" href={dep.links.docs} target="_blank" rel="noopener noreferrer">
            Docs ↗
          </a>
        )}
        {dep.links.privacy && (
          <a className="legal-dep-link" href={dep.links.privacy} target="_blank" rel="noopener noreferrer">
            Privacy policy ↗
          </a>
        )}
      </div>
...
export default async function LegalPage() {
  ...
  // the export today.
  const showAuditBundleNote = resolveFlagFromDb('flag.legal.audit_bundle_note') === 'on';
        ...
        <aside className="legal-sidebar" aria-label={t('sidebar_aria')}>
          ...
          <ul className="legal-sidebar-list">
            {licenseGroups.map(id => (
              <li key={id}>
                <a href={`#${licenseSlug(id)}`} className="legal-sidebar-link">
                  <span>{LICENSE_META[id].name}</span>
                  <span className="legal-sidebar-count">{grouped[id].length}</span>
                </a>
              </li>
            ))}
          </ul>
            ...
            return (
              ...
              >
                <header className="legal-license-head">
                  <h2 id={`${licenseSlug(id)}-heading`} className="legal-license-name">
                    {meta.name}
                  </h2>
                ...
                <div className="legal-dep-list">
                  {grouped[id].map(dep => <DepCard key={dep.name} dep={dep} />)}
                </div>
              </section>
            );
          })}
