...
export default function DesktopAppSection() {
  ...
  const [shortcutDraft, setShortcutDraft] = useState<string>(EMPTY.global_shortcut);
    ...
    fetch('/api/settings/desktop')
      ...
      .then((body) => {
        ...
        const next: DesktopSettings = {
          ...
          global_shortcut: String(body.global_shortcut ?? EMPTY.global_shortcut),
          ...
          theme_override: ['system', 'light', 'dark'].includes(body.theme_override)
  ...
  const persist = useCallback(
    async (patch: Partial<DesktopSettings>, key: string): Promise<DesktopSettings | null> => {
      setSaving(key);
      try {
  ...
  const onToggleNativeNotifications = async (next: boolean) => {
    ...
    flashStatus(next ? tDesk('status_native_notifs_on') : tDesk('status_native_notifs_off'));
  };
  ...
  const onSaveShortcut = async () => {
    ...
    if (ok) {
      ...
      flashStatus(bound ? tDesk('status_shortcut_updated') : tDesk('status_shortcut_unbound'));
    }
  };
  ...
  const onTestTouchId = async () => {
    ...
    try {
      ...
      if (ok === true) flashStatus(tDesk('status_touchid_succeeded'));
      else if (ok === false) flashStatus(tDesk('status_touchid_cancelled'));
      else flashStatus(tDesk('status_touchid_unavailable'));
    } finally {
  ...
  if (inDesktop === null) return null; // waiting on mount
  if (!inDesktop) return null; // running in a web browser, nothing to show

  const disabled = (key: string) => saving === key;

  return (
    <section className="settings-section" aria-labelledby="desktop-app-heading">
      ...
      <label className="settings-row">
        ...
        <span>
          ...
          <span className="muted block">
            Display the privacytracker icon in the macOS menu bar so you can show / hide the window and trigger Sync now without opening the Dock. Disable to keep the menu bar uncluttered.
          </span>
        </span>
      </label>
      ...
      <div className="settings-row">
        <label htmlFor="desktop-shortcut" className="block">
          <strong>{tDesk('shortcut_label')}</strong>
          <span className="muted block">
            {tDesk('shortcut_help_pre')}<code>CmdOrCtrl+Shift+P</code>{tDesk('shortcut_help_or')}<code>Alt+Space</code>{tDesk('shortcut_help_post')}
          </span>
        </label>
        <input
          id="desktop-shortcut"
        ...
        <button
          ...
          disabled={disabled('shortcut') || shortcutDraft.trim() === settings.global_shortcut}
        >
      ...
      <p className="muted">
        {tDesk('url_scheme_pre')}<code>privacytracker://app/&lt;id&gt;</code>{tDesk('url_scheme_post')}
      </p>
      ...
      <div className="settings-row">
        <label htmlFor="auto-lock">
          ...
          <span className="muted block">
            {tDesk('auto_lock_help_pre')}<code>0</code>{tDesk('auto_lock_help_post')}
          </span>
        </label>
        <input
          id="auto-lock"
      ...
      <div className="settings-row" role="radiogroup" aria-labelledby="theme-heading">
        <span id="theme-heading" className="block"><strong>{tDesk('theme_label')}</strong></span>
        {(['system', 'light', 'dark'] as ThemeMode[]).map((mode) => (
          <label key={mode} style={{ marginRight: 12 }}>
            ...
            />{' '}
            {mode === 'system' ? tDesk('theme_system') : mode === 'light' ? tDesk('theme_light') : tDesk('theme_dark')}
          </label>
        ))}
      </div>
      ...
      <div className="settings-row" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <button type="button" onClick={() => { openDataDir(); }}>{tDesk('show_data_folder')}</button>
        <button type="button" onClick={() => { openLogDir(); }}>{tDesk('show_log_folder')}</button>
        <button type="button" onClick={onCopyDiagnostics}>{tDesk('copy_diagnostics')}</button>
        <button type="button" onClick={() => { toggleDevtools(); }}>{tDesk('toggle_devtools')}</button>
      </div>
      <p className="muted">
        {tDesk('diagnostics_help_pre')}<code>privacy.db</code>{tDesk('diagnostics_help_post')}
      </p>

      {status && (
        <p role="status" aria-live="polite" className="muted" style={{ marginTop: 16 }}>
          {status}
        </p>
      )}
    </section>
