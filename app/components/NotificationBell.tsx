...
export default function NotificationBell({ pollingEnabled = true }: NotificationBellProps = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const resolved = data?.prefs && typeof data.prefs === 'object'
  ...
  const { visibleNotifs, visibleUnread, hiddenUnread } = useMemo(() => {
    ...
    for (const n of notifs) {
      ...
      if (notificationPrefs[typeKey]) {
        visible.push(n);
        if (n.read === 0) vUnread += 1;
      } else if (n.read === 0) {
        hUnread += 1;
      }
    }
    return { visibleNotifs: visible, visibleUnread: vUnread, hiddenUnread: hUnread };
    ...
    const handler = () => {
      ...
      void (async () => {
        try {
          ...
          const refreshed = await fetch('/api/notifications').then(r => r.json()).catch(() => null);
          if (refreshed) {
  ...
  const ariaLabel = visibleUnread > 0
  ...
  return (
    <div className="notif-bell-wrap" ref={dropRef}>
      ...
      >
        ...
        {visibleUnread > 0 && (
          <span className="notif-badge" aria-hidden="true">{visibleUnread > 9 ? '9+' : visibleUnread}</span>
        )}
      </button>
        ...
        >
          <div className="notif-dropdown-header">
            ...
            {visibleNotifs.length > 0 && (
              <Link href="/dashboard/stats" className="btn btn-ghost btn-sm" style={{ fontSize: 12 }}>
                {t('view_all')} →
              </Link>
            )}
          </div>
          ...
          <div className="notif-list">
            {visibleNotifs.length === 0 ? (
              ...
              const isTimeout = n.app_id === AI_TIMEOUT_NOTIFICATION_APP_ID
                || n.change_summary[0]?.type === 'ai_timeout';
              const isManualAppsPrompt = n.app_id === MANUAL_APPS_NOTIFICATION_APP_ID
                || n.change_summary[0]?.type === 'manual_apps_prompt';
              const isImportCompletion = n.app_id === IMPORT_COMPLETION_NOTIFICATION_APP_ID
              ...
              const resumeType = n.change_summary[0]?.type as ResumeEntryType | undefined;
              const isWaybackResume = n.app_id === WAYBACK_RESUME_NOTIFICATION_APP_ID
                || resumeType === 'wayback_resumed'
                || resumeType === 'wayback_stale_cleared';
              const isSyncResume = n.app_id === SYNC_RESUME_NOTIFICATION_APP_ID
                || resumeType === 'sync_resumed'
                || resumeType === 'sync_stale_cleared';
              const isPolicyResume = n.app_id === POLICY_RESUME_NOTIFICATION_APP_ID
                || resumeType === 'policy_resumed'
                || resumeType === 'policy_stale_cleared';
              const isResume = isWaybackResume || isSyncResume || isPolicyResume;
              const isStaleHeal = resumeType === 'wayback_stale_cleared'
                || resumeType === 'sync_stale_cleared'
                || resumeType === 'policy_stale_cleared';
              ...
              const importStatus = isImportCompletion
                ? (n.change_summary[0] as { status?: 'ok' | 'partial' | 'error' } | undefined)?.status
                : undefined;
              ...
              const headline = isTimeout
                ...
                : isManualAppsPrompt
                  ...
                  : isImportCompletion
                    ...
                    : isWaybackResume
                      ? (isStaleHeal ? tHeadlines('wayback_lock_cleared') : tHeadlines('wayback_resumed'))
                      : isSyncResume
                        ? (isStaleHeal ? tHeadlines('sync_lock_cleared') : tHeadlines('sync_resumed'))
                        : isPolicyResume
                          ? (isStaleHeal ? tHeadlines('policy_lock_cleared') : tHeadlines('policy_resumed'))
                          : n.app_name;
              const subline = isTimeout
                ? (() => {
                    ...
                    const evt = n.change_summary[0] as
                      | { phase?: string; timeoutMs?: number; observedMs?: number; modelLabel?: string; description?: string }
                      | undefined;
                    if (evt?.phase && typeof evt.timeoutMs === 'number' && typeof evt.observedMs === 'number') {
                      const budget = Math.max(1, Math.round(evt.timeoutMs / 1000));
                      const observed = Math.max(0, Math.round(evt.observedMs / 1000));
                      if (evt.modelLabel) {
                    ...
                    return evt?.description ?? tSublines('ai_timeout_default');
                  })()
                : isManualAppsPrompt
                  ? (() => {
                      ...
                      const evt = n.change_summary[0] as
                        | { unmatchedCount?: number; sourceLabel?: string | null; description?: string }
                        | undefined;
                      ...
                      return evt?.description ?? tSublines('manual_apps_prompt_default');
                    })()
                  : isImportCompletion
                    ? (() => {
                        ...
                        if (
                          ...
                          if (evt.status === 'partial') {
                            ...
                            const tail: string[] = [];
                            if ((evt.queued ?? 0) > 0) {
                              tail.push(tSublines('import_partial_queued', { count: evt.queued! }));
                            }
                            if ((evt.errored ?? 0) > 0) {
                              tail.push(tSublines('import_partial_failed', { count: evt.errored! }));
                            }
                            if ((evt.unmatched ?? 0) > 0) {
                              tail.push(tSublines('import_partial_unmatched', { count: evt.unmatched! }));
                            }
                            return tail.length > 0 ? `${main}${sep}${tail.join(', ')}` : main;
                          }
                          // status === 'error'
                          if ((evt.total ?? 0) > 0 && (evt.itemCount ?? 0) === 0) {
                            return evt.sourceLabel
                              ? tSublines('import_error_no_apps_with_source', { source: evt.sourceLabel })
                              : tSublines('import_error_no_apps');
                          }
                          return evt.sourceLabel
                            ? tSublines('import_error_with_counts_and_source', {
                                total: evt.total,
                        ...
                        return evt?.description ?? tSublines('import_completed_default');
                      })()
                    : isResume
                      ? (() => {
                          ...
                          const evt = n.change_summary[0] as
                            | { appsRemaining?: number; totalApps?: number; description?: string }
                            | undefined;
                          if (isStaleHeal) {
                            if (isWaybackResume) return tSublines('wayback_stale_cleared');
                            if (isSyncResume)    return tSublines('sync_stale_cleared');
                            if (isPolicyResume)  return tSublines('policy_stale_cleared');
                          } else if (
                            typeof evt?.appsRemaining === 'number' &&
                            typeof evt?.totalApps === 'number'
                          ) {
                            const args = { remaining: evt.appsRemaining, total: evt.totalApps };
                            if (isWaybackResume) return tSublines('wayback_resumed_with_data', args);
                            if (isSyncResume)    return tSublines('sync_resumed_with_data', args);
                            if (isPolicyResume)  return tSublines('policy_resumed_with_data', args);
                          }
                          return evt?.description ?? tSublines('resume_default');
                        })()
                      : n.change_summary[0]
                        ...
                        : tSublines('n_changes', { count: n.change_summary.length });
              ...
              const linkHref = isStale && href.startsWith('/apps/') ? '#' : href;
              return (
                <Link
                  key={n.id}
                  href={linkHref}
                  ...
                  onClick={() => setOpen(false)}
                  aria-disabled={isStale && linkHref === '#' ? true : undefined}
                >
                  {isTimeout ? (
                    <div className="notif-icon-placeholder" aria-hidden="true">⏱</div>
                  ) : isManualAppsPrompt ? (
                    <div className="notif-icon-placeholder" aria-hidden="true">🔖</div>
                  ) : isImportCompletion ? (
                    <div className="notif-icon-placeholder" aria-hidden="true">
                      {importStatus === 'ok' ? '✓' : importStatus === 'partial' ? '⚠' : '⚠'}
                    </div>
                  ) : isResume ? (
                    ...
                    <Image src={n.iconUrl} alt={n.app_name} width={32} height={32} className="notif-icon" unoptimized />
                  ) : (
                    <div className="notif-icon-placeholder">{n.app_name[0]}</div>
                  )}
