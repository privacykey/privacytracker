...
function placeTooltip(rect: DOMRect): ResolvedPlacement {
  ...
  function resolve(side: Placement, r: DOMRect): ResolvedPlacement {
    ...
    // Clamp to viewport.
    top = Math.max(VIEWPORT_PAD, Math.min(vh - TOOLTIP_H_GUESS - VIEWPORT_PAD, top));
    left = Math.max(VIEWPORT_PAD, Math.min(vw - TOOLTIP_W - VIEWPORT_PAD, left));
    return { side, top, left };
  }
}

export default function CoachmarkTour({ enabled, audience, goals, aiConfigured }: Props) {
  ...
  const possessive = useMemo(() => possessiveFor(tPoss, audience), [tPoss, audience]);
  ...
  useEffect(() => {
    if (!enabled || !mounted || !apiCheckDone) return;
    // Skip if already completed/dismissed and no replay since.
    if (typeof window !== 'undefined' && window.localStorage.getItem(COMPLETED_KEY) === 'true') {
      return;
    }
    refreshActiveSteps();
    const handle = window.setInterval(refreshActiveSteps, 800);
    const stop = window.setTimeout(() => window.clearInterval(handle), 4_000);
    return () => {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        e.preventDefault();
        skipTour();
      } else if (e.key === 'ArrowRight' || (e.key === 'Enter' && !isFormField(e.target))) {
        e.preventDefault();
        goNext();
  ...
  useEffect(() => {
    if (!running) return;
    if (!targetRect || !placement) return;
  ...
  return (
    <div className="coachmark-tour" role="presentation">
      ...
      >
        <div className="coachmark-tooltip-progress" aria-live="polite">
          {tTour('progress', { current: stepIndex + 1, total })}
        </div>
        <h2 id="coachmark-tooltip-title" className="coachmark-tooltip-title">
          {title}
        </h2>
