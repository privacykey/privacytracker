...
export default function StatsView({
  ...
  const [radarHasAnyPolicy, setRadarHasAnyPolicy] = useState<boolean | null>(
    null
  );
  ...
  const [recentChangesFilter, setRecentChangesFilter] =
    useState<RecentChangesFilter>("all");

  const visibleChanges = useMemo(() => {
    if (recentChangesFilter === "all") {
    ...
    return stats.recentChanges.filter((n) => {
      ...
      // filter.
      if (first.category === "wayback-attempt") {
        return false;
      }
      if (first.type && NON_PRIVACY_LABEL_TYPES.has(first.type)) {
        return false;
      }
      ...
      if (recentChangesFilter === "privacy") {
        return category === "privacy-label";
      }
      if (recentChangesFilter === "accessibility") {
  ...
  const accessibilityPct = showAccessibilityCard
    ? Math.round(
        (stats.appsWithAccessibilityLabels /
  ...
  const summaryCards = [
    {
      label: tStats("tile_apps_label"),
      value: stats.totalApps,
      sub: tStats("tile_apps_sub"),
      color: "var(--blue)",
    },
    {
      label: tStats("tile_categories_label"),
      value: stats.totalCategories,
      sub: tStats("tile_categories_sub", {
        count: stats.totalUniqueCategories,
      }),
      color: "var(--orange)",
    },
    {
      label: tStats("tile_changes_label"),
      value: stats.appsWithChanges,
      sub: tStats("tile_changes_sub"),
      color: stats.appsWithChanges > 0 ? "var(--red)" : "var(--green)",
    },
    {
      label: tStats("tile_resync_label"),
      value: stats.staleApps,
      sub: tStats("tile_resync_sub"),
      color: stats.staleApps > 0 ? "var(--yellow)" : "var(--green)",
    },
      ...
      ? [
          {
            ...
            sub:
              stats.appsNotMatchingProfile === 0
                ? "all apps match your profile"
                : "exceed your privacy preferences",
            color:
              stats.appsNotMatchingProfile > 0 ? "var(--red)" : "var(--green)",
          },
        ]
      : []),
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {stats.totalSyncs} total sync{stats.totalSyncs === 1 ? "" : "s"}{" "}
            recorded
          </p>
        </div>
                  ...
                  return (
                    <div className="bar-row" key={c.identifier}>
                      <div className="bar-label-wrap">
                        ...
                        {meta?.description && (
                          <InfoTooltip side="right" text={meta.description} />
                        )}
                      </div>
            ...
            <div className="recent-changes-toolbar">
              ...
              <span className="recent-changes-count">
                ...
                {hiddenChangesCount > 0 && (
                  <span className="recent-changes-count-muted">
                    {" "}
          ...
          {stats.recentChanges.length === 0 ? (
            ...
            <div className="empty-state" style={{ padding: "24px 0" }}>
              ...
              {recentChangesFilter === "accessibility" ? (
                ...
                <>
                  ...
                  >
                    Switch back to All to see accessibility, wayback imports and
                    other events.
                  </div>
                </>
              )}
            ...
            <div className="recent-changes-scroll">
              {visibleChanges.map((n) => {
                const { href, headline } = resolveRecentChangeDestination(
                  n,
                  tNotifHeadlines
                );
                ...
                const firstType = n.change_summary[0]?.type as
                  | string
                  | undefined;
                ...
                return (
                  <Link className="recent-change-row" href={href} key={n.id}>
                    ...
                    <div className="recent-change-date">
                      {timeAgo(tStats, n.created_at)}
                    </div>
                  </Link>
                );
              })}
      ...
      {f.vizAccessibilityBars && showAccessibilityPanel && (
        ...
        >
          <h2 className="stats-panel-title">
            {tPanels("accessibility_features")}
          </h2>
          <p className="stats-panel-sub">
            How many of your tracked apps claim each accessibility feature, out
            of {stats.appsEvaluatedForAccessibility} evaluated app
            {stats.appsEvaluatedForAccessibility === 1 ? "" : "s"}. Apple lets
            developers self-declare — test before you rely on it.
          </p>
          ...
          {stats.appsEvaluatedForAccessibility === 0 ? (
            <div className="empty-state" style={{ padding: "32px 0" }}>
              ...
              <div
                style={{ fontSize: 13, marginTop: 4, color: "var(--text-3)" }}
              >
            ...
            <div className="bar-chart a11y-bar-chart">
              {stats.accessibilityFeatureFrequency.map((f) => {
                const pct = Math.round(
                  (f.appCount / accessibilityDenominator) * 100
                );
                return (
                  <div className="bar-row" key={f.identifier}>
                    <div className="bar-label-wrap">
        ...
        <section className="glass-card stats-panel" style={{ marginTop: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {stats.staleAppsList.map((app) => (
              <div className="stale-row" key={app.id}>
                {app.iconUrl ? (
                  <Image
                    alt={app.name}
