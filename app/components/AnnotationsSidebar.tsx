...
annotationRenderer.link = function annotationLinkRenderer({ href, title, tokens }) {
  const inlineText = this?.parser?.parseInline
    ? this.parser.parseInline(tokens ?? [])
    : escapeHtml((tokens ?? []).map((t: { raw?: string }) => t.raw ?? '').join(''));
  const safeHref = typeof href === 'string' && /^(https?:|mailto:|#)/i.test(href.trim())
    ? href.trim()
    : '';
...
function applyMarkdownAction(
  ...
  const toggled = lines.map((line) => {
    ...
    if (action === 'ul') {
      if (ulMatch.test(line)) return line.replace(ulMatch, '');
      return prefix + line;
    }
    if (action === 'ol') {
      if (olMatch.test(line)) return line.replace(olMatch, '');
      return prefix + line;
    }
    if (action === 'quote') {
      if (quoteMatch.test(line)) return line.replace(quoteMatch, '');
      return prefix + line;
    }
    return line;
  });
...
export default function AnnotationsSidebar({
  ...
  useEffect(() => {
    if (initialAnnotations !== undefined) return;
    let cancelled = false;
    void (async () => {
      try {
        const res = await fetch(`/api/annotations?appId=${encodeURIComponent(appId)}`);
  ...
  async function handleUpdate(id: string, patch: Partial<Annotation>): Promise<void> {
    // Optimistic update.
    setAnnotations((prev) =>
  ...
  async function handleUndo(id: string): Promise<void> {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = (await res.json()) as { annotation: Annotation };
      setAnnotations((prev) => [data.annotation, ...prev.filter((a) => a.id !== id)]);
    } catch (e) {
  ...
  useEffect(() => {
    try {
      ...
      if (stored === 'wide') setLayout('wide');
      else if (stored === 'split') setLayout('split');
    } catch {
      /* ignore */
    }
  }, []);
    ...
    >
      ...
      */}
      <header className="annotations-sidebar__header">
        ...
        >
          <span className="annotations-sidebar__chevron" aria-hidden="true">
            <svg
              width="14"
              height="14"
              viewBox="0 0 24 24"
          ...
          <h3 id="annotations-sidebar-title" className="annotations-sidebar__title">
            {t('sidebar_title')}
        ...
        {expanded && (
          <div className="annotations-sidebar__header-actions">
            <button
              ...
              title={layout === 'split' ? t('layout_wide_title') : t('layout_split_title')}
              aria-label={layout === 'split' ? t('layout_wide_aria') : t('layout_split_aria')}
      ...
      */}
      <div
        ...
        className={
          ...
          (layout === 'split' ? ' annotations-sidebar__body--composing-split' : '')
        }
      ...
      >
        ...
        */}
        ...
        <div className="annotations-sidebar__list" aria-label={t('list_aria')}>
          {loading && <p className="annotations-sidebar__loading">{t('loading')}</p>}

          {!loading && annotations.length === 0 && (
            <div className="annotations-sidebar__empty">
              <p>{t('empty_title')}</p>
              <p className="annotations-sidebar__empty-sub">
                {t('empty_sub')}
              </p>
            </div>
          )}

          {!loading && visibleNotes.map((annotation) => (
            <AnnotationCard
              key={annotation.id}
          ...
          {!loading && hiddenCount > 0 && (
            ...
            >
              {showAllNotes
                ...
                : t('show_all', { count: annotations.length, hidden: hiddenCount })}
            </button>
          )}
        ...
        {undoToast && (
          <div role="status" className="annotations-sidebar__undo-toast" aria-live="polite">
            {t('undo_deleted')}{' '}
            <button
              type="button"
...
function AnnotationCard({ annotation, onUpdate, onDelete }: CardProps) {
  ...
  useEffect(() => {
    if (!editing) return;
    if (content === annotation.content) return;
  ...
  const sourceLabel =
    annotation.source === 'imported'
      ? t('source_imported', { name: annotation.sourceName ?? t('source_default_name') })
      : t('source_self');

  const charCount = content.length;
  const charClass = charCount > SOFT_LIMIT
  ...
  function handleVisibilityToggle() {
    const next: AnnotationVisibility = annotation.visibility === 'private' ? 'export' : 'private';
      ...
      */}
      <header className="annotation-card__header">
        <div className="annotation-card__byline">
          ...
          <span className="annotation-card__byline-sep" aria-hidden="true">·</span>
        ...
        <div className="annotation-card__header-right">
          <div className="annotation-card__header-actions">
            <button
              ...
              aria-label={annotation.visibility === 'private' ? t('visibility_private_aria') : t('visibility_public_aria')}
              title={annotation.visibility === 'private' ? t('visibility_private_title') : t('visibility_public_title')}
            >
          ...
          >
            <span className="annotation-card__tag-chip-icon" aria-hidden="true">🏷</span>
          ...
          <div className="annotations-composer-header">
            ...
            >
              ...
              <button
                ...
                title={!content.trim() ? t('preview_disabled_title') : t('preview_enabled_title')}
              >
                {t('preview_tab')}
              </button>
            </div>
            {editMode === 'edit' && (
              <MarkdownToolbar textareaRef={editorRef} setContent={setContent} />
            )}
          </div>
        ...
        <footer className="annotation-card__footer">
          <span className="annotation-card__editor-meta">
            ...
            {savedAt && Date.now() - savedAt < 2000 && (
              <span className="annotation-card__saved" aria-live="polite">{t('saved')}</span>
            )}
          </span>
        </footer>
      )}
...
function MarkdownToolbar({ textareaRef, setContent }: ToolbarProps) {
  ...
  return (
    <div className="annotation-toolbar" role="toolbar" aria-label={t('toolbar_aria')}>
      {buttons.map(b => {
        const label = t(b.key);
        return (
        <button
...
function relativeTime(t: AnnT, ts: number): string {
  ...
  if (s < 86_400) return t('rel_hours', { count: Math.floor(s / 3600) });
  const d = Math.floor(s / 86_400);
  if (d === 1) return t('rel_yesterday');
  if (d < 30) return t('rel_days', { count: d });
