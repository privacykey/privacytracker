...
export default function TaskListIcon() {
  ...
  const visible = useMemo(() => {
    ...
    for (const t of kept) (t.state === 'completed' ? done : open).push(t);
    return [...open, ...done];
  }, [tasks]);

  const readyCount = useMemo(
    () => tasks.filter(t => t.state === 'ready' || t.state === 'in_progress').length,
    [tasks],
  );
  ...
  const allDone =
    ready && visible.length > 0 && visible.every(t => t.state === 'completed');
    ...
    <>
      <div className="task-list-icon-wrap" ref={wrapRef}>
        <button
          ...
          onClick={() => setOpen(v => !v)}
        >
          <span className="task-list-icon-glyph" aria-hidden="true">
            ✓
          </span>
          {readyCount > 0 && (
            <span className="task-list-icon-badge" aria-hidden="true">
              {readyCount}
            </span>
          )}
        </button>
      ...
      {open && isMobile && (
        ...
        >
          ...
          >
            <div className="task-list-sheet-handle" aria-hidden="true" />
            <TaskListDropdownContent
              tasks={visible}
              allDone={allDone}
              onTaskClick={handleTaskClick}
...
function TaskListDropdownContent({ tasks, allDone, onTaskClick }: TaskListDropdownContentProps) {
  const t = useTranslations('tasks');
  if (allDone) {
    return (
...
function TaskRow({ task, onClick }: { task: ResolvedTask; onClick: () => void }) {
  const t = useTranslations('tasks');
  return (
    <li className={`task-list-row task-list-row-${task.state}`}>
      <button type="button" className="task-list-row-btn" onClick={onClick}>
