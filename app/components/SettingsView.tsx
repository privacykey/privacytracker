 ...
 * status.
 */
type ItemStatusFilter = 'unmatched' | 'error' | 'removed' | 'queued' | 'problems';
...
function itemMatchesFilter(
  ...
  if (filter === 'problems') return status === 'unmatched' || status === 'error';
...
const STATUS_META: Record<ImportItemStatus, { label: string; tone: string; icon: string }> = {
...
function fmtRelativeTime(t: TimeT, tDate: DateT, ts: number): string {
  const now = Date.now();
  const diff = now - ts;
  if (diff < 0) return fmtDate(tDate, ts);
  if (diff < 45_000) return t('rel_just_now');
  if (diff < 90_000) return t('rel_one_min');
  if (diff < 60 * 60_000) return t('rel_mins_ago', { count: Math.round(diff / 60_000) });
  if (diff < 2 * 60 * 60_000) return t('rel_one_hr');
  if (diff < 24 * 60 * 60_000) return t('rel_hrs_ago', { count: Math.round(diff / (60 * 60_000)) });
  if (diff < 2 * 24 * 60 * 60_000) return t('rel_yesterday');
  if (diff < 7 * 24 * 60 * 60_000) return t('rel_days_ago', { count: Math.round(diff / (24 * 60 * 60_000)) });
...
function fmtRelative(t: TimeT, ts: number | null) {
  ...
  if (h > 24) return t('in_days', { count: Math.floor(h / 24) });
  if (h > 0)  return t('in_hours_minutes', { hours: h, minutes: m });
...
function fmtQueueCountdown(t: TimeT, ts: number | null | undefined): string {
  if (!ts) return t('queue_now');
  const diff = ts - Date.now();
  if (diff <= 0) return t('queue_now');
  const secs = Math.ceil(diff / 1000);
  if (secs < 60) return t('queue_seconds', { seconds: secs });
  const m = Math.floor(secs / 60);
  const s = secs % 60;
  if (m < 60) return s > 0 ? t('queue_minutes_seconds', { minutes: m, seconds: s }) : t('queue_minutes', { minutes: m });
  const h = Math.floor(m / 60);
  const mm = m % 60;
  return t('queue_hours_minutes', { hours: h, minutes: String(mm).padStart(2, '0') });
}
...
function pickSourceIcon(
  ...
  if (has('iphone'))     return { glyph: '📱', title: 'iPhone' };
  if (has('ipad'))       return { glyph: '📱', title: 'iPad' };
  if (has('ipod'))       return { glyph: '🎵', title: 'iPod' };
  if (has('appletv') || has('apple tv')) return { glyph: '📺', title: 'Apple TV' };
  if (has('applewatch') || has('apple watch')) return { glyph: '⌚️', title: 'Apple Watch' };
  ...
  if (source === 'configurator') return { glyph: '📱', title: 'Apple Configurator' };
  if (source === 'file')         return { glyph: '📄', title: 'File upload' };
  if (source === 'manual')       return { glyph: '⌨️', title: 'Manual entry' };
  if (source === 'screenshot' || source === 'screenshots') return { glyph: '📷', title: 'Screenshot OCR' };
...
function describeBulkPhase(t: BulkPhaseT, phase: string | undefined | null, note?: string): string {
  if (!phase) return t('working');
  const base = (() => {
    switch (phase) {
      case 'fetching':        return t('fetching');
      case 'fetch':           return t('fetching');
      case 'parse':           return t('parse');
      case 'archive':         return t('archive');
      case 'archive-existing':return t('archive_existing');
      case 'summarise':       return t('summarise');
      case 'chunk':           return t('chunk');
      case 'chunk_summarise': return t('chunk_summarise');
      case 'ai-direct':       return t('summarise');
      case 'ai-chunked':      return t('chunk');
      case 'merge':           return t('merge');
      case 'persist':         return t('persist');
      case 'throttled':       return t('throttled');
      case 'same':            return t('same');
      case 'cache-hit':       return t('cache_hit');
      case 'skip':            return t('skip');
      case 'needs-config':    return t('needs_config');
      case 'changelog':       return t('changelog');
      case 'version-store':   return t('version_store');
      case 'ready':           return t('ready');
      default:                return phase.replace(/_/g, ' ');
    }
  })();
  if (note) return `${base} — ${note}`.slice(0, 120);
  return `${base}…`;
}
...
export default function SettingsView({ viewMode = 'all', focusCard }: SettingsViewProps = {}) {
  ...
  useEffect(() => {
    try {
      setAutosaveLogToTaskCenter(localStorage.getItem(AUTOSAVE_LOG_KEY) === 'true');
    } catch {
  ...
  const tDevActivityTypes = useTranslations('settings.dev_options.activity_types');
  ...
  const tImpFilterBanner = useTranslations('settings.import_history.filter_banner');
  ...
  const tImpChangeMatch = useTranslations('settings.import_history.change_match');
  const tImpLegacy = useTranslations('settings.import_history.legacy');
  const tImpCouldntLoad = useTranslations('settings.import_history.couldnt_load');
  ...
  const settingsSyncScheduleOn = useFlag('flag.settings.sync.schedule') === 'on';
  ...
  const settingsPoliciesThrottleOn = useFlag('flag.settings.policies.throttle') === 'on';
  const settingsPoliciesWaybackOn = useFlag('flag.settings.policies.wayback_import') === 'on';
  const settingsNotificationsPrefsOn = useFlag('flag.settings.notifications.prefs') === 'on';
  const settingsProfilesPrivacyOn = useFlag('flag.settings.profiles.privacy') === 'on';
  const settingsProfilesAccessibilityOn = useFlag('flag.settings.profiles.accessibility') === 'on';
  ...
  // is exercised.
  const settingsAdminExportAuditPdfOn = useFlag('flag.settings.admin.export.audit_pdf') === 'on';
  ...
  const settingsDateFormatPrefOn = useFlag('flag.settings.date_format.user_preference') === 'on';
  ...
  const devActivityLogRetentionDaysOn = useFlag('flag.devopts.activity_log.retention_days') === 'on';
  ...
  // lands later.
  const devFeatureFlagPresetsOn = useFlag('flag.devopts.feature_flag_presets') === 'on';
  ...
  // card visible.
  const settingsAiProviderSelectorOn = useFlag('flag.settings.ai.provider_selector') === 'on';
  const settingsAiTimeoutConfigOn = useFlag('flag.settings.ai.timeout_config') === 'on';
  const settingsAiSummarizeOnImportOn = useFlag('flag.settings.ai.summarize_on_import') === 'on';
  const settingsAiDebugLoggingOn = useFlag('flag.settings.ai.debug_logging') === 'on';
  ...
  const [deploymentDiagnosticsLoading, setDeploymentDiagnosticsLoading] = useState(false);
  const [deploymentDiagnosticsError, setDeploymentDiagnosticsError] = useState('');
  const [copyingDeploymentDiagnostics, setCopyingDeploymentDiagnostics] = useState(false);
  ...
  const [scrapeThrottleEnabled, setScrapeThrottleEnabled] = useState<boolean>(true);
  const [scrapeThrottleMinutes, setScrapeThrottleMinutes] = useState<string>('60');
  ...
  const [activitySortBy, setActivitySortBy] = useState<'started_at' | 'ended_at' | 'duration_ms'>('started_at');
  ...
  const [policyBulkRunning, setPolicyBulkRunning] = useState<null | 'fetch' | 'all'>(null);
  const [policyBulkForce, setPolicyBulkForce] = useState(false);
  const [policyBulkSummary, setPolicyBulkSummary] = useState<string | null>(null);
  ...
  const [savedWaybackShowImported, setSavedWaybackShowImported] = useState(true);
  ...
  const [waybackInitiator, setWaybackInitiator] = useState<'manual' | 'resume' | null>(null);
  ...
  const [profile, setProfile] = useState<PrivacyProfile>({ ...DEFAULT_PROFILE });
  ...
  const pushProfileUndo = useCallback((op: ProfileUndoOp) => {
    ...
    stack.push(op);
    if (stack.length > MAX_PROFILE_UNDO_OPS) stack.shift();
  }, []);
  ...
  const [a11yProfile, setA11yProfile] = useState<AccessibilityProfile>({ ...DEFAULT_A11Y_PROFILE });
  const [savedA11yProfile, setSavedA11yProfile] = useState<AccessibilityProfile | null>(null);
  ...
  const [notificationPrefs, setNotificationPrefs] =
    useState<Record<NotificationTypeKey, boolean>>({ ...DEFAULT_NOTIFICATION_PREFS });
  ...
  const [expandedItems, setExpandedItems] = useState<Record<string, ImportItemRow[]>>({});
   ...
   */
  const [pendingItemRemoval, setPendingItemRemoval] = useState<
    | null
    | { importRow: ImportRow; item: ImportItemRow; appId: string }
  >(null);
  ...
  const [aiSampleResult, setAiSampleResult] = useState<AiSamplePolicyResult | null>(null);
  ...
  const [discoveredModels, setDiscoveredModels] = useState<{ value: string; label: string }[]>([]);
  ...
  const loadDeploymentDiagnostics = async () => {
    ...
    try {
      const res = await fetch('/api/deployment/diagnostics', { cache: 'no-store' });
  ...
  const copyDeploymentSupportBundle = async () => {
    if (copyingDeploymentDiagnostics) return;
    setCopyingDeploymentDiagnostics(true);
    try {
      const res = await fetch('/api/deployment/support-bundle', { cache: 'no-store' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const applyBackupSnapshotPayload = (payload: BackupSnapshotsPayload) => {
    setBackupSnapshotSettings(payload.settings ?? DEFAULT_BACKUP_SNAPSHOT_SETTINGS);
    setBackupSnapshotDirectory(payload.directory ?? '');
    setBackupSnapshots(Array.isArray(payload.snapshots) ? payload.snapshots : []);
  };
  ...
  const handleCreateBackupSnapshot = async () => {
    if (creatingBackupSnapshot) return;
  ...
  const loadSettings = async () => {
    ...
    const maskedKey = data.ai_api_key === '__SET__' ? '__SET__' : (data.ai_api_key ?? '');
    const nextAi: StoredAiSettings = {
      provider,
      apiKey: maskedKey,
      baseUrl: provider === 'disabled' ? '' : (data.ai_base_url ?? '') || resolveDefaultBaseUrl(provider),
      model: provider === 'disabled' ? '' : (data.ai_model ?? '') || resolveDefaultModel(provider),
    ...
    const parsedAlert = parseInt(rawAlert, 10);
    setPolicyDiffAlertDays(
      Number.isFinite(parsedAlert) && parsedAlert >= 0 ? String(parsedAlert) : '90',
    );
    ...
    setScrapeThrottleMinutes(
      Number.isFinite(parsedMinutes) && parsedMinutes >= 0 ? String(parsedMinutes) : '60',
    );
  ...
  const loadNotificationPrefs = async () => {
    try {
      ...
      if (!res.ok) return;
      const data = await res.json();
      const resolved = data?.prefs && typeof data.prefs === 'object' ? data.prefs : null;
      if (resolved) {
  ...
  const notificationPrefsAutoSave = useSettingsAutoSave<NotificationPrefs>({
    ...
    onSaved: (value, response) => {
      const fromResponse = (response as { prefs?: NotificationPrefs } | null)?.prefs;
   ...
   */
  const notificationPrefsSaveTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const scheduleNotificationPrefsSave = useCallback((next: NotificationPrefs) => {
  ...
  const privacyProfileAutoSave = useSettingsAutoSave<PrivacyProfile | null>({
    ...
    onSaved: (value) => {
      ...
      setSavedProfile(prev => {
        ...
        if (isDifferent) pushProfileUndo({ kind: 'privacy', prior: prev });
        return value;
      });
      router.refresh();
    },
  ...
  const runPrivacyProfileSave = useCallback(
    (nextEnabled: boolean, nextProfile: PrivacyProfile) => {
      ...
      if (!isDirty) return;
      const emptyEnabled =
        nextEnabled && Object.values(nextProfile).every(v => typeof v !== 'string');
  ...
  const privacyProfileSaveTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const a11yProfileAutoSave = useSettingsAutoSave<AccessibilityProfile | null>({
    ...
    onSaved: (value) => {
      ...
      setSavedA11yProfile(prev => {
        const isDifferent = JSON.stringify(prev) !== JSON.stringify(value);
        if (isDifferent) pushProfileUndo({ kind: 'accessibility', prior: prev });
        return value;
      });
      router.refresh();
    },
  ...
  const runA11yProfileSave = useCallback(
    (nextEnabled: boolean, nextProfile: AccessibilityProfile) => {
      const payload: AccessibilityProfile | null = nextEnabled
        ...
        : null;
      const isDirty = JSON.stringify(payload) !== JSON.stringify(savedA11yProfile);
      if (!isDirty) return;
      const emptyEnabled =
        nextEnabled && Object.values(nextProfile).every(v => typeof v !== 'string');
      if (emptyEnabled) return;
  ...
  const a11yProfileSaveTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    const handler = () => { handleProfileUndo(); };
  ...
  const policyDiffAlertDaysAutoSave = useSettingsAutoSave<number>({
    ...
    successMessage: (value) =>
      value === 0 ? 'Policy alert disabled' : `Policy alert set to ${value} days`,
  ...
  const handlePolicyDiffAlertBlur = useCallback(() => {
    ...
    if (trimmed === '') return; // empty is "no change"; let user keep typing
  ...
  const scrapeThrottleAutoSave = useSettingsAutoSave<{
    ...
    taskLabel: ({ enabled, minutes }) => `Policy throttle → ${enabled ? `${minutes} min` : 'off'}`,
  ...
  const saveScrapeThrottle = useCallback((next: { enabled: boolean; minutes: number }) => {
  ...
  const saveBackupSnapshots = useCallback((next: {
    enabled: boolean;
    intervalHours: number;
    retentionCount: number;
  }) => {
  ...
  const aiTimeoutDirectAutoSave = useSettingsAutoSave<string>({
    ...
    successMessage: (value) =>
      value === '' ? 'Direct timeout reset to default' : `Direct timeout set to ${value} ms`,
  ...
  const aiTimeoutChunkAutoSave = useSettingsAutoSave<string>({
    ...
    successMessage: (value) =>
      value === '' ? 'Chunk timeout reset to default' : `Chunk timeout set to ${value} ms`,
  ...
  const aiTimeoutMergeAutoSave = useSettingsAutoSave<string>({
    ...
    successMessage: (value) =>
      value === '' ? 'Merge timeout reset to default' : `Merge timeout set to ${value} ms`,
  ...
  const validateAiTimeout = useCallback((raw: string): string | null => {
    ...
    if (trimmed === '') return null; // empty = default, allowed
    const parsed = Number.parseInt(trimmed, 10);
  ...
  const makeAiTimeoutBlurHandler = (
    raw: string,
    saver: (value: string) => Promise<unknown>,
  ) => () => {
  ...
  useEffect(() => {
    ...
    const valid: ItemStatusFilter[] = ['unmatched', 'error', 'removed', 'queued', 'problems'];
  ...
  useEffect(() => {
    ...
    if (!deepImportId) return;
    const key = `${deepImportId}|${deepItemId ?? ''}`;
    if (deepLinkTargetRef.current === key) return;

    if (!imports) return; // still loading — try again after loadImports resolves
  ...
  useEffect(() => {
    ...
    const deepImportId = searchParams.get('importId');
    const deepItemId = searchParams.get('item');
    if (!deepImportId || !deepItemId) return;
    const items = expandedItems[deepImportId];
    if (!items || items.length === 0) return;
    if (!items.some(it => it.id === deepItemId)) return;
  ...
  const countItemsMatchingFilter = useCallback(
    (row: ImportRow, filter: ItemStatusFilter | null): number => {
      if (!filter) return row.total;
      const errored = row.errored ?? 0;
      const removed = row.removed ?? 0;
      const unmatchedOnly = Math.max(0, (row.unmatched ?? 0) - errored - removed);
      if (filter === 'unmatched') return unmatchedOnly;
      if (filter === 'error')     return errored;
      if (filter === 'removed')   return removed;
      if (filter === 'queued')    return row.queued ?? 0;
      if (filter === 'problems')  return unmatchedOnly + errored;
      return 0;
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    if (autoExpandedForFilter.current === itemStatusFilter) return;
    const target = imports.find(row => countItemsMatchingFilter(row, itemStatusFilter) > 0);
    if (!target) {
  ...
  const handleBadgeClick = (next: ItemStatusFilter) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (resolved) url.searchParams.set('filter', resolved);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activityTypeFilter, activityStatusFilter, activityTimeWindow, activitySortBy, activitySortDir]);
   ...
   * only" view.
   */
  const applyActivityQueryParams = (params: URLSearchParams, overrides?: {
    type?: string;
    status?: string;
    timeWindow?: string;
    sortBy?: string;
  ...
  const loadActivityLog = async (append = false) => {
    ...
    try {
      ...
      const data = (await res.json()) as { rows?: ActivityLogRow[]; total?: number };
      const rows = Array.isArray(data.rows) ? data.rows : [];
      const total = typeof data.total === 'number' ? data.total : rows.length;
      setActivityTotal(total);
      setActivityLog(prev => (append && prev ? [...prev, ...rows] : rows));
      const loadedCount = (append && activityLog ? activityLog.length : 0) + rows.length;
  ...
  useEffect(() => { activityTypeFilterRef.current = activityTypeFilter; }, [activityTypeFilter]);
  const activityStatusFilterRef = useRef<string>(activityStatusFilter);
  useEffect(() => { activityStatusFilterRef.current = activityStatusFilter; }, [activityStatusFilter]);
  const activityTimeWindowRef = useRef<string>(activityTimeWindow);
  useEffect(() => { activityTimeWindowRef.current = activityTimeWindow; }, [activityTimeWindow]);
  const activitySortByRef = useRef<typeof activitySortBy>(activitySortBy);
  useEffect(() => { activitySortByRef.current = activitySortBy; }, [activitySortBy]);
  const activitySortDirRef = useRef<typeof activitySortDir>(activitySortDir);
  useEffect(() => { activitySortDirRef.current = activitySortDir; }, [activitySortDir]);
  const activityLoadingRef = useRef<boolean>(activityLoading);
  useEffect(() => { activityLoadingRef.current = activityLoading; }, [activityLoading]);
  ...
  useEffect(() => {
    if (!activityFlashing) return;
    const t = window.setTimeout(() => setActivityFlashing(false), 1200);
    return () => window.clearTimeout(t);
  }, [activityFlashing]);

  useEffect(() => {
    if (!activityOpen || !activityLogLoaded || activityLivePaused) return;
    const tick = () => {
  ...
  const toggleImportRow = async (importRow: ImportRow) => {
    ...
    if (expandedItems[importRow.id]) return;

    setExpandingId(importRow.id);
    try {
      const res = await fetch(`/api/imports?id=${encodeURIComponent(importRow.id)}`);
      if (!res.ok) {
        showToast(tToast('import_details_load_failed'));
        setExpandingId(null);
        return;
      }
      const data = (await res.json()) as { import: ImportRow; items: ImportItemRow[] };
    ...
    try {
      ...
      if (updated) {
        ...
        setExpandedItems(prev => {
          ...
          return {
            ...prev,
            [importRow.id]: current.map(row => (row.id === item.id ? updated : row)),
          };
        });
      }
      showToast(tToast('reimported', { name: updated?.appName ?? item.appName ?? item.query ?? '' }));
  ...
  const handleRetryAllErrors = async () => {
    ...
    try {
      ...
      const candidateImports = imports.filter(row => {
        if (!itemMatchesFilter('error', scope) && !itemMatchesFilter('unmatched', scope)) {
          return false;
        }
      ...
      if (needFetch.length > 0) {
        const fetched = await Promise.all(
          needFetch.map(async row => {
            try {
              const res = await fetch(`/api/imports?id=${encodeURIComponent(row.id)}`);
              if (!res.ok) return { row, items: [] as ImportItemRow[] };
              const data = (await res.json()) as { items?: ImportItemRow[] };
              return { row, items: data.items ?? [] };
            } catch (error) {
              console.warn('[settings] retry-all fetch failed for', row.id, error);
      ...
      if (failed === 0) {
        ...
        showToast(
          `⚠ Retried ${succeeded} — ${failed} still failing`,
        );
      }
    } catch (error) {
   ...
   */
  const handleRetrySingleItem = async (importRow: ImportRow, item: ImportItemRow) => {
    if (retryingItemId !== null) return;
    setRetryingItemId(item.id);
    try {
      ...
      showToast(tToast('save_failed_with_message', { message: 'Retry failed. Check your connection.' }));
    } finally {
      setRetryingItemId(null);
    }
  };
  ...
  const confirmRemoveItemFromDashboard = async () => {
    ...
    try {
      const res = await fetch(
        `/api/apps?id=${encodeURIComponent(appId)}`,
        { method: 'DELETE' },
      );
      if (!res.ok) {
  ...
  const runChangeMatchSearch = async () => {
    ...
    if (!name) {
      setChangeMatch(prev => (prev ? { ...prev, error: 'Enter an app name to search.' } : prev));
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setChangeMatch(prev =>
          prev
            ? { ...prev, searching: false, error: data?.error ?? 'Search failed.' }
            : prev,
        );
        return;
      }
      const results = Array.isArray(data.results) && data.results[0]?.candidates
      ...
      setChangeMatch(prev =>
        prev
          ? { ...prev, searching: false, error: 'Search failed. Check your connection.' }
          : prev,
      );
    }
  };
  ...
  const applyChangeMatch = async (
    ...
    if (!changeMatch) return;
    setChangeMatch(prev => (prev ? { ...prev, applyingAppleId: candidate.appleId, error: '' } : prev));
    try {
      ...
      const res = await fetch('/api/imports/items/change-match', {
        ...
        body: JSON.stringify({
          itemId: item.id,
          url: candidate.url,
          editedQuery: editedQuery && editedQuery !== item.query ? editedQuery : undefined,
        }),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok) {
        const msg = data?.error ?? `Change-match failed (HTTP ${res.status})`;
        setChangeMatch(prev => (prev ? { ...prev, applyingAppleId: null, error: msg } : prev));
        showToast(tToast('save_failed_with_message', { message: msg }));
        return;
      }
      ...
      setChangeMatch(prev =>
        prev ? { ...prev, applyingAppleId: null, error: 'Could not apply match.' } : prev,
      );
  ...
  const confirmDeleteImport = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/imports?${query.toString()}`, { method: 'DELETE' });
      const data = await res.json().catch(() => null);
      if (!res.ok) {
        showToast(tToast('save_failed_with_message', { message: data?.error ?? tToast('delete_failed_fallback') }));
        setDeleting(false);
        return;
      }
      ...
      if (deleteTarget.mode === 'with-apps') {
        ...
        showToast(count ? `✓ Import removed · ${count} app${count !== 1 ? 's' : ''} deleted` : '✓ Import removed');
      } else {
        showToast(tToast('import_entry_removed'));
      }

      setDeleteTarget(null);
      setExpandedImportId(prev => (prev === deleteTarget.importRow.id ? null : prev));
      setExpandedItems(prev => {
        if (!prev[deleteTarget.importRow.id]) return prev;
        const next = { ...prev };
      ...
      if (deleteTarget.mode === 'with-apps') router.refresh();
    } catch (error) {
  ...
  const loadWaybackProgress = async (): Promise<{
    ...
    try {
      ...
      const rawInitiator = (data?.state as { initiator?: unknown } | null)?.initiator;
      const initiator: 'manual' | 'resume' | null =
        rawInitiator === 'manual' || rawInitiator === 'resume' ? rawInitiator : null;
      let progress: {
        index: number;
        total: number;
        currentAppName: string | null;
      ...
      if (data?.summary && data?.state) {
        ...
        const totals = (data.state as { totals?: Record<string, unknown> }).totals ?? {};
        progress = {
          index: Number(summary.done ?? 0) + Number(summary.inProgress ?? 0),
          total: Number(summary.total ?? 0),
          currentAppName:
            typeof data.currentAppName === 'string' ? data.currentAppName : null,
  ...
  const loadWaybackLastRun = async () => {
    try {
      ...
      const detailTotals =
        (summaryRow.detail as { totals?: Record<string, unknown> } | null)?.totals ??
        null;
      ...
      setWaybackLastRun({
        ...
        summary: typeof summaryRow.summary === 'string' ? summaryRow.summary : null,
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(ADMIN_TOKEN_CHANGED_EVENT, refreshAdminUnlockState);
  }, []);
  ...
  const hasQueuedItems = importQueue.state.queued > 0 ||
    Object.values(expandedItems).some(list => list.some(i => i.status === 'queued'));
  useEffect(() => {
    if (!hasQueuedItems) return;
    const id = setInterval(() => setNowTick(t => (t + 1) & 0xffff), 1000);
  ...
  const scheduleAutoSave = useSettingsAutoSave<Schedule>({
    ...
    successMessage: (value) =>
      value === 'manual' ? 'Sync set to manual' :
      value === 'daily' ? 'Sync scheduled daily' :
      'Sync scheduled weekly',
    taskLabel: (value) => `Sync schedule → ${value}`,
    onSaved: () => {
      void Promise.all([loadStatus(), loadSettings(), taskCenter.refreshScheduler()]);
    },
  });
  ...
  const runAiSampleSummaryTest = async () => {
    if (!canRunSamplePolicyTest || aiSampleStatus === 'testing') return;
      ...
      if (data.ok && Array.isArray(data.models)) {
        const cleaned = data.models
          .map(m => ({
            ...
            label: typeof m.label === 'string' && m.label ? m.label : (typeof m.id === 'string' ? m.id : ''),
          }))
          .filter(m => m.value);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { void refreshDiscoveredModels(); }, 600);
  ...
  const triggerSync = async () => {
    ...
    try {
      const res  = await fetch('/api/sync/trigger', { method: 'POST', signal: controller.signal });
      const data = await res.json();
      if (data.skipped) {
        ...
        showToast(tToast('sync_done', { synced: data.synced, changes: data.changes }));
  ...
  const runBulkPolicySync = async (phase: 'fetch' | 'all') => {
    if (policyBulkRunning) return;
    setPolicyBulkRunning(phase);
    setPolicyBulkSummary(null);

    const controller = new AbortController();
    const taskTitle = phase === 'all'
    ...
    let totals: { attempted: number; succeeded: number; failed: number; throttled: number; skipped: number } | null = null;
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        ...
        const message = errBody?.error ?? `Bulk policy sync failed (${res.status})`;
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (event.type === 'batch-start') {
            ...
            const label = describeBulkPhase(tBulkPhase, inner.phase, inner.note);
  ...
  const runBulkWaybackImport = async () => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        const errBody = await res.json().catch(() => null);
        const message = errBody?.error ?? `Wayback import failed (${res.status})`;
        showToast(tToast('save_failed_with_message', { message }));
        handle.complete('error', message);
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (event.type === 'batch-start') {
            ...
            const badge = event.error || failed > 0 ? '⚠' : imported > 0 ? '⟳' : '✓';
            handle.update({ subtitle: `${n}/${total} · ${badge} ${event.name}` });
            setWaybackProgress(prev =>
              prev
                ? {
                    ...prev,
                    index: n,
                    imported: prev.imported + Number(event.result?.imported ?? 0),
                    unchanged: prev.unchanged + Number(event.result?.unchanged ?? 0),
  ...
  const trackAccessibilityAutoSave = useSettingsAutoSave<boolean>({
    ...
    taskLabel: (value) => `Accessibility labels → ${value ? 'visible' : 'hidden'}`,
  ...
  const resetAllData = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(tToast('save_failed_with_message', { message: data.error ?? tToast('reset_failed_fallback') }));
        setResetting(false);
        return;
      }
  ...
  const mergedModelValues = useMemo(() => {
    ...
    if (aiProvider === 'custom') {
      for (const m of discoveredModels) set.add(m.value);
    } else {
      for (const m of discoveredModels) set.add(m.value);
  ...
  const selectedModelPreset = mergedModelValues.has(aiModel) ? aiModel : '__custom__';
  ...
  useEffect(() => {
    if (selectedModelPreset !== '__custom__') return;
    if (!focusCustomModelOnNextRender.current) return;
  ...
  const onProviderChange = (nextProvider: AIProvider) => {
    ...
    const previousDefaultModel = aiProvider === 'disabled' ? '' : resolveDefaultModel(aiProvider);
    const previousDefaultBaseUrl = aiProvider === 'disabled' ? '' : resolveDefaultBaseUrl(aiProvider);
  ...
  return (
    <div className="page-container">
      ...
      <div className="page-header">
        {viewMode === 'import-history' ? (
          <div>
            ...
            <p className="page-subtitle">{tSettings('import_history_subtitle')}</p>
          </div>
        ) : (
          <div>
      ...
      <div className={`settings-layout${viewMode === 'import-history' ? ' settings-layout-standalone' : ''}`}>
      ...
      {desktopAppSectionOn && (
        <div
          ...
          style={{ background: 'rgba(99, 102, 241, 0.06)', border: '1px dashed rgba(99, 102, 241, 0.35)' }}
        >
          <h2 className="settings-section-title">{tSections('desktop_app')}</h2>
          <p className="settings-section-subtitle">{tSub('desktop_app')}</p>
        </div>
      )}
      ...
      {settingsDateFormatPrefOn && (
        <div id="date-format" className="settings-section">
          <DateFormatPicker />
        </div>
      )}
      <h3 className="settings-group-heading">{tSettings('sidebar.group_you')}</h3>
      ...
      <div id="language" className="settings-section">
        ...
        <p className="settings-section-subtitle">{tSettings('language_section.subtitle')}</p>
        <LocaleSwitcher />
      </div>
      ...
      {settingsProfilesPrivacyOn && <div id="privacy-profile" className="settings-section privacy-profile-section">
        <h2 className="settings-section-title">{tSections('privacy_profile')}</h2>
        <p className="settings-section-subtitle">{tSub('privacy_profile')}</p>
        ...
        <div className="privacy-profile-toggle-row">
          ...
          <div className="privacy-profile-toggle-label">
            <div className="privacy-profile-toggle-title">{tPrivProfile('toggle_title')}</div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginTop: 12 }}>
          {(() => {
            const currentPayload = profileEnabled ? sanitizeProfile(profile) : null;
            const isDirty = JSON.stringify(currentPayload) !== JSON.stringify(savedProfile);
            const emptyProfile =
              profileEnabled && Object.values(profile).every(v => typeof v !== 'string');
              ...
              <>
                {!isDirty && savedProfile && (
                  <span style={{ fontSize: 13, color: 'var(--text-2)' }}>
                    {tPrivProfile('saved_count', { count: Object.values(savedProfile).filter(v => typeof v === 'string').length })}
                  </span>
                )}
                {!isDirty && !savedProfile && (
                  <span style={{ fontSize: 13, color: 'var(--text-2)' }}>
      ...
      {settingsProfilesAccessibilityOn && <div id="accessibility-profile" className="settings-section privacy-profile-section">
        <h2 className="settings-section-title">{tSections('accessibility_profile')}</h2>
        <p className="settings-section-subtitle">{tSub('accessibility_profile')}</p>
        ...
        <div className="privacy-profile-toggle-row">
          ...
          <div className="privacy-profile-toggle-label">
            <div className="privacy-profile-toggle-title">{tA11yProfile('toggle_title')}</div>
            <div className="privacy-profile-toggle-hint">
              {a11yProfileEnabled
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginTop: 12 }}>
          {(() => {
            const currentPayload = a11yProfileEnabled ? sanitizeA11yProfile(a11yProfile) : null;
            const isDirty = JSON.stringify(currentPayload) !== JSON.stringify(savedA11yProfile);
            const emptyProfile =
              a11yProfileEnabled && Object.values(a11yProfile).every(v => typeof v !== 'string');
            ...
            return (
              <>
                {!isDirty && savedA11yProfile && (
                  <span style={{ fontSize: 13, color: 'var(--text-2)' }}>
                    {tA11yProfile('saved_count', { count: Object.values(savedA11yProfile).filter(v => typeof v === 'string').length })}
                  </span>
                )}
                {!isDirty && !savedA11yProfile && (
                  <span style={{ fontSize: 13, color: 'var(--text-2)' }}>
      ...
      {settingsNotificationsPrefsOn && <div id="notifications" className="settings-section">
        <h2 className="settings-section-title">{tSections('notifications')}</h2>
        <p className="settings-section-subtitle">{tSub('notifications')}</p>

        <div className="notification-prefs-list" role="group" aria-label={tAria('notification_types')}>
          {NOTIFICATION_TYPE_KEYS.map(key => {
            ...
            return (
              <label key={key} htmlFor={inputId} className="notification-prefs-row">
                <input
                  id={inputId}
                  type="checkbox"
                  ...
                  checked={enabled}
                  onChange={event => {
                    const next = { ...notificationPrefs, [key]: event.target.checked };
                ...
                <span className="notification-prefs-copy">
                  <span className="notification-prefs-label">{tNotifPrefs(`${tKey}_label`)}</span>
                  <span className="notification-prefs-description">{tNotifPrefs(`${tKey}_desc`)}</span>
                  <span className="notification-prefs-example">{tNotifPrefs(`${tKey}_example`)}</span>
                </span>
              </label>
            );
          })}
          ...
          <button
            ...
            disabled={
              ...
              NOTIFICATION_TYPE_KEYS.every(
                key => notificationPrefs[key] === DEFAULT_NOTIFICATION_PREFS[key],
              )
            }
          >
      ...
      <h3 className="settings-group-heading">{tSettings('sidebar.group_data_sync')}</h3>

      {/* Sync Schedule */}
      {settingsSyncScheduleOn && <div id="sync-schedule" className="settings-section">
        ...
        >
          {SCHEDULE_OPTIONS.map(opt => {
            ...
            const localisedLabel = (() => {
              try { return tSchedule(`${opt.value}_label`); }
              catch { return opt.label; }
            })();
            const localisedDesc = (() => {
              try { return tSchedule(`${opt.value}_desc`); }
              catch { return opt.desc; }
            })();
            return (
              <button
                key={opt.value}
      ...
      {/* App Store Region */}
      {settingsSyncRegionOn && <div id="region" className="settings-section">
        <h2 className="settings-section-title">{tSections('app_store_region')}</h2>
        <p className="settings-section-subtitle">{tSub('app_store_region')}</p>

        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center' }}>
          <label htmlFor="app-country" style={{ fontSize: 14, color: 'var(--text-2)' }}>
      ...
      */}
      <div id="accessibility-labels" className="settings-section">
        <h2 className="settings-section-title">{tSections('accessibility_labels')}</h2>
        ...
        <label className="settings-checkbox-row">
          <input
            ...
            onChange={event => void saveTrackAccessibility(event.target.checked)}
      ...
      {settingsSyncScheduleOn && <div id="sync-status" className="settings-section">
        <h2 className="settings-section-title">{tSections('app_store_sync_status')}</h2>
        <p className="settings-section-subtitle">{tSub('app_store_sync_status')}</p>
        ...
        {status ? (
          <div className="settings-status-grid">
            <div className="settings-status-item">
              <div className="settings-status-label">{tSyncStatus('last_sync')}</div>
              <div className="settings-status-value">{fmtDate(tSettings, status.lastRun)}</div>
            </div>
            <div className="settings-status-item">
              <div className="settings-status-label">{tSyncStatus('next_auto_sync')}</div>
            ...
            <div className="settings-status-item">
              <div className="settings-status-label">{tSyncStatus('status_label')}</div>
              <div className="settings-status-value" style={{ color: status.isRunning ? 'var(--orange)' : 'var(--green)' }}>
      ...
      <h3 className="settings-group-heading">{tSettings('sidebar.group_policies_ai')}</h3>

      {settingsAiEnabledOn && <div id="ai-summaries" className="settings-section ai-settings-section">
        <div className="settings-section-heading">
          <div>
            <h2 className="settings-section-title">{tSections('ai_summaries')}</h2>
            <p className="settings-section-subtitle" style={{ marginBottom: 0 }}>{tSub('ai_summaries')}</p>
          </div>
          <span className={`ai-status-pill ai-status-pill-${aiProvider === 'disabled' ? 'off' : 'on'}`}>
            <span className="ai-status-dot" />
            {aiProvider === 'disabled'
              ? tAi('status_off')
              : tAi('status_using', {
                  provider: (() => {
                    const opt = AI_PROVIDER_OPTIONS.find(o => o.value === aiProvider);
                    if (!opt) return aiProvider;
        ...
        {/* 1. Provider */}
        {settingsAiProviderSelectorOn && <section className="settings-subsection">
          <header className="settings-subsection-header">
            <h3 className="settings-subsection-title">{tAiProvider('title')}</h3>
            <p className="settings-subsection-desc">{tAiProvider('desc')}</p>
          </header>

          <label className="settings-field">
            <span className="settings-field-label">{tAiProvider('backend_label')}</span>
            <select
          ...
          {aiProvider === 'custom' && (
            <details className="settings-help-card settings-help-details">
              <summary className="settings-help-summary">
                <span className="settings-help-title">{tOllamaHelp('summary_title')}</span>
              ...
              <div style={{ marginTop: 10 }}>
                <p className="settings-help-copy">
                  {tOllamaHelp.rich('intro', { own: chunks => <strong>{chunks}</strong> })}
                </p>
...
ollama serve`}
                ...
                <p className="settings-help-copy">
                  <strong>{tOllamaHelp('long_policies_lead')}</strong>{tOllamaHelp('long_policies_body')}
                </p>
                <pre className="settings-code-block">
{`OLLAMA_CONTEXT_LENGTH=131072 ollama serve`}
                ...
                <div className="settings-help-subnote">
                  <strong>{tOllamaHelp('memory_guide_lead')}</strong>{tOllamaHelp('memory_guide_body')}
                ...
                <p className="settings-help-copy" style={{ marginBottom: 0 }}>
                  {tOllamaHelp.rich('verify', { em: chunks => <em>{chunks}</em> })}
                </p>
              </div>
            </details>
          )}
        ...
        {aiProvider !== 'disabled' && (
          <section className="settings-subsection">
            ...
            <label className="settings-field">
              <span className="settings-field-label">{tAiConn('base_url_label')}</span>
              <input
              ...
              <label className="settings-field">
                <span className="settings-field-label">
                  ...
                  {providerRequiresApiKey(aiProvider) && (
                    <span className="settings-field-required">{tAiConn('api_key_required')}</span>
                  )}
                </span>
                <input
                  className="settings-input"
                  ...
                  spellCheck={false}
                />
                <span className="settings-field-help">{tAiConn('api_key_help')}</span>
              </label>
            )}
              ...
              <div className="settings-field">
                ...
                {customApiKeyEnabled && (
                  <label className="settings-field" style={{ marginTop: 10 }}>
                    <span className="settings-field-label">{tAiConn('api_key_label')}</span>
                    <input
                      className="settings-input"
                      type="password"
                      value={aiApiKey}
            ...
            <div className="ai-test-row">
              ...
              {(aiTestMessage || aiTestLatency !== null) && (
                <div className={`ai-test-message ai-test-message-${aiTestStatus}`}>
                  {aiTestStatus === 'ok' ? '✓ ' : aiTestStatus === 'fail' ? '⚠ ' : ''}
                  {aiTestMessage || (aiTestStatus === 'ok' ? tAiConn('test_reachable_fallback') : '')}
                  {aiTestLatency !== null && (
                    <span className="ai-test-latency">{tAiConn('test_latency', { ms: aiTestLatency })}</span>
                  )}
                </div>
              )}
            </div>
        ...
        {aiProvider !== 'disabled' && (
          <section className="settings-subsection">
            ...
            <label className="settings-field">
              ...
              >
                {discoveredModels.length > 0 && (
                  <optgroup label={tAiModel('available_optgroup', { count: discoveredModels.length })}>
                ...
                {aiProvider !== 'custom' && providerOptions
                  .filter(option => !discoveredModels.some(discovered => discovered.value === option.value))
                  .map(option => (
                    <option key={option.value} value={option.value}>
                      {option.label}
                    </option>
              ...
              {modelsStatus === 'error' && (
                <span className="settings-field-help settings-field-help-warn">
                  {tAiModel('error_lead')} {modelsError || tAiModel('error_fallback')} {tAiModel('error_after')}
                </span>
              )}
              {modelsStatus === 'ok' && discoveredModels.length > 0 && (
                <span className="settings-field-help">
                  {tAiModel('found_count', { count: discoveredModels.length })}
                  {selectedModelPreset === '__custom__'
                    ? tAiModel('found_custom_extra')
                    : ''}
                </span>
              )}
              {aiProvider === 'custom' && modelsStatus === 'ok' && discoveredModels.length === 0 && (
                <span className="settings-field-help settings-field-help-warn">
                  {tAiModel.rich('no_models', {
                    code: () => <code>ollama pull {resolveDefaultModel('custom')}</code>,
                  })}
                </span>
              )}
              {(modelsStatus === 'idle' || modelsStatus === 'loading') && (
                <span className="settings-field-help">
                  {selectedModelPreset === '__custom__'
                    ...
                    : (() => {
                        const opt = providerOptions.find(option => option.value === aiModel);
                        if (opt) return tAiOptions(opt.descKey);
                        return aiProvider === 'custom' && modelsStatus === 'loading'
            ...
            {selectedModelPreset === '__custom__' && (
              <label className="settings-field">
                <span className="settings-field-label">{tAiModel('custom_id_label')}</span>
                <input
        ...
        {aiProvider !== 'disabled' && (
          <section className="settings-subsection ai-sample-section">
            ...
            <div className="ai-sample-framing" aria-label={tAiSample('frame_aria')}>
              <div>
                <span className="ai-sample-frame-label">{tAiSample('policy_label')}</span>
                <p>{tAiSample('policy_body')}</p>
              </div>
              <div>
                <span className="ai-sample-frame-label">{tAiSample('judging_label')}</span>
                <p>{tAiSample('judging_body')}</p>
              </div>
              <div>
                <span className="ai-sample-frame-label">{tAiSample('look_for_label')}</span>
            ...
            <div className="ai-test-row">
              ...
              >
                <span className={`ai-test-dot ai-test-dot-${aiSampleStatus}`}>
                  {aiSampleStatus === 'testing' ? <span className="spinner-sm" /> : null}
                </span>
              ...
              {(aiSampleMessage || !canRunSamplePolicyTest) && (
                <div className={`ai-test-message ai-test-message-${aiSampleStatus}`}>
                  {aiSampleStatus === 'ok' ? '✓ ' : aiSampleStatus === 'fail' ? '⚠ ' : ''}
            ...
            {aiSampleResult ? (
              <div className="ai-sample-result">
                <div className="ai-sample-result-header">
                  <div>
                    <div className="ai-sample-kicker">{tAiSample('result_kicker')}</div>
                    <h4 className="ai-sample-title">{aiSampleResult.sample.appName}</h4>
                    <p className="ai-sample-scenario">{aiSampleResult.sample.scenario}</p>
                  </div>
                ...
                <h5 className="ai-sample-output-heading">{tAiSample('overview_title')}</h5>
                <p className="ai-sample-overview">{aiSampleResult.summary.overview}</p>
                ...
                <h5 className="ai-sample-output-heading">{tAiSample('lenses_title')}</h5>
                <p className="ai-sample-lenses-help">{tAiSample('lenses_help')}</p>
                  ...
                  {aiSampleResult.summary.lenses.map(entry => (
                    ...
                    >
                      <div className="ai-sample-lens-top">
                        ...
                        <span className={`policy-rating-badge policy-rating-${entry.rating as PolicyRating}`}>
              ...
              <p className="settings-field-help ai-sample-empty">{tAiSample('empty_help')}</p>
            )}
          </section>
        )}
        ...
        <section className="settings-subsection">
          ...
          {settingsAiSummarizeOnImportOn && <label className="settings-checkbox-row">
            <input
              className="settings-checkbox"
              type="checkbox"
        ...
        <footer className="settings-actions-footer">
          <div className="settings-actions-status">
            {storedAi && !aiConfigChanged ? (
              ...
              <span className="settings-actions-unsaved">{tAiFooter('unsaved')}</span>
            ) : null}
      ...
      */}
      {settingsPoliciesThrottleOn && <div id="privacy-policies-bulk" className="settings-section">
        <h2 className="settings-section-title">{tSections('privacy_policies')}</h2>
        ...
        <label className="settings-checkbox-row" style={{ marginTop: 14 }}>
          <input
            ...
            onChange={event => setPolicyBulkForce(event.target.checked)}
            disabled={policyBulkRunning !== null}
          />
          <span>
      ...
      <div id="policy-alerts" className="settings-section">
        <h2 className="settings-section-title">{tSections('policy_change_alerts')}</h2>
        <p className="settings-section-subtitle">{tSub('policy_change_alerts')}</p>

        <label className="settings-field" style={{ maxWidth: 320 }}>
          <span className="settings-field-label">{tPolicyAlerts('field_label')}</span>
          <input
            className="settings-input"
            type="number"
            min={0}
            ...
            onChange={event => setPolicyDiffAlertDays(event.target.value)}
          ...
          >
            {tPolicyAlerts.rich('field_help', { strong: chunks => <strong>{chunks}</strong> })}
          </span>
        </label>
      </div>

      {/* Policy Scrape Throttle */}
      <div id="policy-scrape-throttle" className="settings-section">
        <h2 className="settings-section-title">{tSections('policy_scrape_throttle')}</h2>
        <p className="settings-section-subtitle">{tSub('policy_scrape_throttle')}</p>

        <label
          className="settings-field"
          style={{ maxWidth: 420, display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 10 }}
        >
          <input
            type="checkbox"
            ...
            disabled={scrapeThrottleAutoSave.saving}
          />
        ...
        <label className="settings-field" style={{ maxWidth: 320, marginTop: 12 }}>
          <span className="settings-field-label">{tPolicyThrottle('cooldown_label')}</span>
          <input
            className="settings-input"
            type="number"
            min={0}
            ...
            disabled={!scrapeThrottleEnabled || scrapeThrottleAutoSave.saving}
          ...
          <span
            ...
            style={{ display: 'block', marginTop: 4 }}
          >
            {tPolicyThrottle.rich('cooldown_help', { strong: chunks => <strong>{chunks}</strong> })}
          </span>
        </label>
      </div>
      </>)}
      {viewMode === 'import-history' && settingsImportHistoryOn && (
      <div id="import-history" className="settings-section">
        ...
        {importQueue.state.queued > 0 && (() => {
          ...
          <div className="import-queue-banner" role="status">
            <div className="import-queue-banner-body">
              <span className="import-queue-banner-icon" aria-hidden>⏳</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="import-queue-banner-title">
                  {drainState
                    ? tImpQueue('draining_title', {
                        done: Math.min(drainState.processed, drainState.initialTotal),
                        total: drainState.initialTotal,
                      })
                    : tImpQueue('title', { count: importQueue.state.queued })}
                </div>
                <div className="import-queue-banner-sub">
                  {drainState && drainState.pausedUntil && drainState.pausedUntil > Date.now()
                    ? tImpQueue('drain_paused', {
                        countdown: fmtQueueCountdown(tTime, drainState.pausedUntil),
                      })
                    : drainState
                      ? tImpQueue('drain_running')
                      : importQueue.state.pausedUntil && importQueue.state.pausedUntil > Date.now()
                        ? tImpQueue('paused', { countdown: fmtQueueCountdown(tTime, importQueue.state.pausedUntil) })
                        : importQueue.state.soonestNextAttemptAt && importQueue.state.soonestNextAttemptAt > Date.now()
                          ? tImpQueue('next_retry', { countdown: fmtQueueCountdown(tTime, importQueue.state.soonestNextAttemptAt) })
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              {drainState ? (
                ...
                >
                  {drainState.cancelled ? tImpQueue('cancelling') : tImpQueue('cancel_drain')}
                </button>
              ) : (
                <button
                  type="button"
        ...
        {itemStatusFilter && (() => {
          ...
          >
            <div className="import-history-filter-banner-body">
              <span className="import-history-filter-banner-icon" aria-hidden>🔎</span>
              <div>
                <div className="import-history-filter-banner-title">
                  {tImpFilterBanner('showing', { label: tImpFilterMeta(itemStatusFilter) })}
                </div>
                <div className="import-history-filter-banner-sub">
                  {retryingAll && retryAllProgress
                    ? tImpFilterBanner('retrying', { done: retryAllProgress.done, total: retryAllProgress.total })
                ...
                >
                  {retryingAll && retryAllProgress ? (
                    <>
                      ...
                      {' '}{tImpFilterBanner('retry_all_busy', { done: retryAllProgress.done, total: retryAllProgress.total })}
                    </>
                  ) : (
        ...
        {imports === null ? (
          ...
          const visibleImports = itemStatusFilter
            ? imports.filter(r => countItemsMatchingFilter(r, itemStatusFilter) > 0)
            : imports;
          if (visibleImports.length === 0) {
            return (
              <div className="import-history-empty">
                {tImpHistory('empty_filter_lead')} <button
                  type="button"
                ...
                </button>{tImpHistory('empty_filter_post')}
              </div>
            );
          }
          return (
          <ul className="import-history-list">
            {visibleImports.map(importRow => {
              ...
              const sourceIcon = pickSourceIcon(importRow.sourceLabel, importRow.source);
              ...
              return (
                <li key={importRow.id} className={`import-history-row${isExpanded ? ' is-open' : ''}`}>
                  <div className="import-history-summary">
                    <div className="import-history-meta">
                      <span className="import-history-date">{fmtShortDate(importRow.createdAt, dateMode)}</span>
                      <span className="import-history-sep">·</span>
                      <span className="import-history-count">
                        {tImpHistory('meta_apps_count', { count: importRow.total })}
                      </span>
                      <span className="import-history-sep">·</span>
                    ...
                    {hasProblems && (
                      <div className="import-history-stats">
                        {hasQueued && (
                          ...
                          >
                            {tImpHistory('stat_queued', { count: queuedCount })}
                          </button>
                        )}
                        {hasUnmatched && (
                          <button
                          ...
                          >
                            {tImpHistory('stat_unmatched', { count: unmatchedOnly })}
                          </button>
                        )}
                        {hasErrored && (
                          <button
                          ...
                          >
                            {tImpHistory('stat_error', { count: erroredCount })}
                          </button>
                        )}
                        {hasRemoved && (
                          <button
                          ...
                          >
                            {tImpHistory('stat_removed', { count: removedCount })}
                          </button>
                        )}
                      </div>
                    )}
                    ...
                    <div className="import-history-actions">
                      {hasQueued && (
                        ...
                        >
                          {retryingQueue ? (
                            <><span className="spinner-sm" /> {tImpActions('resuming')}</>
                          ) : (
                      ...
                      >
                        {isExpanded ? tImpActions('hide') : tImpActions('view')}
                      </button>
                      <button
                        type="button"
                        className="pill-button pill-button-danger"
                        onClick={() => setDeleteTarget({ importRow, mode: 'history-only' })}
                      >
                        {tImpActions('delete')}
                      </button>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="import-history-detail">
                      {loadingItems ? (
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-3)', padding: '12px 2px' }}>
                          <span className="spinner-sm" /> {tImpHistory('loading_items')}
                        </div>
                        ...
                        const visibleItems = itemStatusFilter
                          ? items.filter(it => itemMatchesFilter(it.status, itemStatusFilter))
                          : items;
                        if (visibleItems.length === 0) {
                          return (
                            <div className="import-history-empty" style={{ margin: 0 }}>
                        ...
                        <ul className="import-item-list">
                          {visibleItems.map(item => {
                            ...
                            const displayQuery = item.editedQuery || item.query;
                            ...
                            const canEditRetry = item.status === 'unmatched' || item.status === 'error';
                            const canQueueRetry = item.status === 'queued';
                            const canChangeMatch =
                              item.status === 'matched' || item.status === 'imported';
                            const canReAdd = item.status === 'removed';
                            const editing = changeMatch?.itemId === item.id;
                            ...
                            const canRetryImport = canEditRetry && Boolean(item.url);
                            const retryingThisItem = retryingItemId === item.id;
                            const applying = editing && changeMatch?.applyingAppleId !== null;
                            ...
                            const dashboardHref = item.status === 'imported' && item.appId
                              ? `/apps/${encodeURIComponent(item.appId)}`
                              : null;
                            const isDeepLinkTarget = highlightItemId === item.id;
                            return (
                              <li
                                key={item.id}
                              ...
                              >
                                <span className={`import-item-chip import-item-chip-${meta.tone}`}>
                                  <span aria-hidden>{meta.icon}</span> {tImpStatusMeta(item.status)}
                                </span>
                                <div className="import-item-body">
                                  <div className="import-item-query-line">
                                    ...
                                    <div className="import-item-query">{displayQuery}</div>
                                  </div>
                                  {item.editedQuery && item.editedQuery !== item.query && (
                                    <div className="import-item-sub">{tImpItem('originally_prefix')}<em>{item.query}</em></div>
                                  )}
                                  {hasMatch ? (
                                    <div className="import-item-sub">
                                      {item.status === 'removed' ? tImpItem('was_prefix') : tImpItem('arrow_prefix')}
                                      {dashboardHref ? (
                                        <Link href={dashboardHref} className="import-item-match-link">
                                          {item.appName}
                                        </Link>
                                      ) : item.url ? (
                                        <a
                                      ...
                                      {item.developer ? (
                                        <span style={{ color: 'var(--text-3)' }}> · {item.developer}</span>
                                      ) : null}
                                    ...
                                    <div className="import-item-sub" style={{ color: 'var(--text-3)' }}>
                                  ...
                                  {item.status === 'queued' && (
                                    <div className="import-item-sub import-item-queued-note">
                                      ...
                                      {item.nextAttemptAt && item.nextAttemptAt > Date.now()
                                        ? tImpItem('queued_next', { countdown: fmtQueueCountdown(tTime, item.nextAttemptAt) })
                                        : tImpItem('queued_retry_now')}
                                      {typeof item.attemptCount === 'number' && item.attemptCount > 0 && (
                                        <span style={{ color: 'var(--text-3)' }}>
                                          {tImpItem('queued_attempt', { n: item.attemptCount + 1 })}
                                        </span>
                                      )}
                                    </div>
                                  )}
                                  ...
                                  {item.scrapeError && item.status !== 'queued' && (
                                    <div className="import-item-error">{item.scrapeError}</div>
                                  )}
                                </div>
                                <div className="import-item-actions">
                                  ...
                                  {canRetryImport && !editing && (
                                    <button
                                      type="button"
                                      className="pill-button pill-button-primary"
                                      onClick={() => void handleRetryImport(importRow, item)}
                                      disabled={retryingThisItem}
                                    >
                                      {retryingThisItem ? (
                                        <><span className="spinner-sm" /> {tImpActions('retrying')}</>
                                      ) : (
                                  ...
                                  {canChangeMatch && !editing && (
                                    <button
                                      type="button"
                                      className="pill-button"
                                      onClick={() => openChangeMatch(item, 'change')}
                                    >
                                  ...
                                  {!editing && item.appId && item.status !== 'removed' && (
                                    <button
                                      type="button"
                                      className="pill-button pill-button-danger"
                                      onClick={() =>
                                        void handleRemoveItemFromDashboard(importRow, item)
                                      }
                                      disabled={removingItemId === item.id}
                                    >
                                      {removingItemId === item.id ? (
                                        <><span className="spinner-sm" /> {tImpActions('removing')}</>
                                      ) : (
                                  ...
                                  {canReAdd && !editing && (
                                    <button
                                      type="button"
                                      className="pill-button"
                                      onClick={() => openChangeMatch(item, 'readd')}
                                    >
                                      {tImpActions('re_add')}
                                    </button>
                                  )}
                                  {canQueueRetry && !editing && (
                                    <button
                                      ...
                                      onClick={() => void handleRetrySingleItem(importRow, item)}
                                      disabled={retryingItemId === item.id || retryingQueue}
                                    >
                                  ...
                                  {canEditRetry && !editing && (
                                    <button
                                      type="button"
                                      className="pill-button"
                                      onClick={() => handleRetryItem(importRow, item)}
                                    >
                                      {tImpActions('change_match')}
                                    </button>
                                  )}
                                  ...
                                  {canEditRetry && !editing && (
                                    <Link
                                      className="pill-button"
                                      href={{
                                        pathname: '/dashboard/manual-apps',
                                        query: { prefillName: displayQuery },
                                      }}
                                    >
                                  ...
                                  <div className="change-match-panel">
                                    ...
                                    <div className="change-match-search">
                                      <label className="change-match-field">
                                        <span className="change-match-label">{tImpChangeMatch('app_name_label')}</span>
                                        <input
                                          type="text"
                                          value={changeMatch.query}
                                          onChange={event =>
                                            setChangeMatch(prev =>
                                              prev ? { ...prev, query: event.target.value, error: '' } : prev,
                                            )
                                          }
                                          onKeyDown={event => {
                                            if (event.key === 'Enter') {
                                      ...
                                      <label className="change-match-field">
                                        <span className="change-match-label">{tImpChangeMatch('seller_label')}</span>
                                        <input
                                          type="text"
                                          value={changeMatch.developer}
                                          onChange={event =>
                                            setChangeMatch(prev =>
                                              prev ? { ...prev, developer: event.target.value, error: '' } : prev,
                                            )
                                          }
                                          onKeyDown={event => {
                                            if (event.key === 'Enter') {
                                      ...
                                      <button
                                        ...
                                        onClick={() => void runChangeMatchSearch()}
                                        disabled={
                                          applying ||
                                      ...
                                      >
                                        {changeMatch.searching ? (
                                          <><span className="spinner-sm" /> {tImpChangeMatch('searching')}</>
                                        ) : (
                                    ...
                                    {changeMatch.error && (
                                      <div className="import-item-error">{changeMatch.error}</div>
                                    )}

                                    {changeMatch.results !== null && !changeMatch.searching && (
                                      changeMatch.results.length === 0 ? (
                                        <div className="change-match-empty">
                                          {tImpChangeMatch('empty')}
                                        </div>
                                      ) : (
                                        <ul className="change-match-results">
                                          {changeMatch.results.map(candidate => {
                                            const isCurrent =
                                              item.appId === candidate.appleId ||
                                              item.removedAppId === candidate.appleId;
                                            const isApplying =
                                              changeMatch.applyingAppleId === candidate.appleId;
                                            return (
                                              <li key={candidate.appleId} className="change-match-result">
                                                {candidate.iconUrl ? (
                                                  // eslint-disable-next-line @next/next/no-img-element
                                                  <img
                                                ...
                                                <div className="change-match-result-body">
                                                  ...
                                                  >
                                                    {tImpChangeMatch('view_app_store')}
                                                  </a>
                                                </div>
                                                <button
                                                  type="button"
                                                  className="pill-button"
                                                  onClick={() =>
                                                    void applyChangeMatch(importRow, item, candidate)
                                                  }
                                                  disabled={applying}
                                                >
                                                  {isApplying ? (
                                                    <><span className="spinner-sm" /> {tImpChangeMatch('applying')}</>
                                                  ) : isCurrent && changeMatch.mode === 'change' ? (
                                                    tImpChangeMatch('rescrape')
                                                  ) : changeMatch.mode === 'readd' ? (
                                                    tImpChangeMatch('re_add')
                                                  ) : (
                                                    tImpChangeMatch('use_this')
                                                  )}
                                                </button>
                                              </li>
                                            );
                        ...
                        importRow.itemCount === 0 ? (
                          ...
                          <div className="import-history-items-empty">
                            ...
                            <div style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 8 }}>
                              {tImpCouldntLoad('body', { count: importRow.itemCount })}
                            </div>
                            <button
                              type="button"
                              className="pill-button"
                              onClick={() => {
                                ...
                                setExpandedImportId(null);
                                setTimeout(() => void toggleImportRow(importRow), 0);
                              }}
                            >
      ...
      {viewMode === 'all' && (<>
      <h3 className="settings-group-heading">{tSettings('sidebar.group_admin')}</h3>
        ...
        <div id="import-history" className="settings-section">
          ...
          {importQueue.state.queued > 0 && (
            <p style={{ fontSize: 12, color: 'var(--orange)', marginTop: 10 }}>
              {tImpHistoryCard('queue_note', { count: importQueue.state.queued })}
            </p>
          )}
        </div>
      )}

      <div id="deployment-diagnostics" className="settings-section">
        <h2 className="settings-section-title">{tSections('deployment_diagnostics')}</h2>
        <p className="settings-section-subtitle">{tSub('deployment_diagnostics')}</p>
        ...
        {deploymentDiagnosticsLoading && !deploymentDiagnostics ? (
          ...
          <>
            <div className="settings-status-grid deployment-diagnostics-grid">
              <div className="settings-status-item">
                ...
                <div className="settings-status-value">{deploymentDiagnostics.app.version}</div>
              </div>
              <div className="settings-status-item">
              ...
              <div className="settings-status-item">
                ...
                <div
                  className="settings-status-value"
                  style={{ color: deploymentDiagnostics.database.writable ? 'var(--green)' : 'var(--danger)' }}
                >
            ...
            <div className="deployment-admin-card">
              <div>
                <div className="deployment-admin-title">{tDeploy('admin_unlock_title')}</div>
              ...
              {deploymentDiagnostics.security.adminTokenConfigured ? (
                <div className="deployment-admin-controls">
                  <label className="settings-field" style={{ gap: 6 }}>
                    <span className="settings-field-label">{tDeploy('admin_token_input')}</span>
                    <input
                      type="password"
                      className="settings-input"
            ...
            <div className="deployment-check-list" aria-label={tDeploy('checks_aria')}>
              {deploymentDiagnostics.checks.map(check => (
                <div key={check.id} className={`deployment-check deployment-check-${check.status}`}>
                  <div className="deployment-check-main">
                    <span className="deployment-check-dot" aria-hidden="true" />
                    <div>
                      <div className="deployment-check-title">{check.label}</div>
                      <div className="deployment-check-detail">{check.detail}</div>
                    </div>
                  </div>
            ...
            <div className="deployment-detail-grid">
              ...
              <div className="deployment-detail-row">
                ...
                <strong>{fmtBytes(deploymentDiagnostics.database.sizeBytes)}</strong>
              </div>
              <div className="deployment-detail-row">
                <span>{tDeploy('host')}</span>
                <code>{deploymentDiagnostics.network.host ?? tDeploy('unknown')}</code>
              </div>
              <div className="deployment-detail-row">
          ...
          <div className="settings-help-card" role="status">
            <div className="settings-help-title">{tDeploy('unavailable_title')}</div>
            <p className="settings-help-copy">{deploymentDiagnosticsError || tDeploy('unavailable_body')}</p>
            <button
              type="button"
              className="btn btn-secondary"
              onClick={() => void loadDeploymentDiagnostics()}
      ...
      {/* Backup & Restore */}
      {settingsAdminBackupOn && <div id="backup" className="settings-section">
        ...
        <div className="backup-grid">
          <div className="backup-card">
            <div className="backup-card-title">{tBackupCard('download_title')}</div>
            ...
            >
              {exportingBackup ? tBackupCard('download_busy') : tBackupCard('download_button')}
            </button>
          </div>

          <div className="backup-card">
            <div className="backup-card-title">{tBackupCard('restore_title')}</div>
            ...
            <label
              ...
              style={{ cursor: status?.isRunning ? 'not-allowed' : 'pointer' }}
            >
              {restoreStage === 'previewing' ? tBackupCard('restore_busy') : tBackupCard('restore_choose')}
              <input
                type="file"
                accept="application/json,.json"
                style={{ display: 'none' }}
                disabled={Boolean(status?.isRunning) || restoreStage === 'previewing' || restoreStage === 'applying'}
                onChange={event => {
        ...
        <div className="backup-snapshot-panel">
          <div className="backup-snapshot-header">
            <div>
              <div className="backup-card-title">{tBackupCard('snapshots_title')}</div>
              <p className="backup-snapshot-copy">{tBackupCard('snapshots_copy')}</p>
            </div>
            <span className={`backup-snapshot-state${backupSnapshotSettings.enabled ? ' is-on' : ''}`}>
          ...
          <label className="settings-checkbox-row backup-snapshot-toggle">
            <input
              ...
              onChange={event => {
                const enabled = event.target.checked;
                setBackupSnapshotSettings(prev => ({ ...prev, enabled }));
                saveBackupSnapshots({
                  enabled,
            ...
            />
            <span>
              <span className="settings-field-label">{tBackupCard('snapshots_enabled_label')}</span>
              <span className="settings-field-help" style={{ display: 'block', marginTop: 4 }}>
          ...
          <div className="settings-field-grid backup-snapshot-controls">
            <label className="settings-field">
              <span className="settings-field-label">{tBackupCard('snapshots_interval_label')}</span>
              <select
                className="settings-input settings-select"
                ...
                onChange={event => {
                  ...
                  setBackupSnapshotSettings(prev => ({ ...prev, intervalHours }));
                  saveBackupSnapshots({
              ...
              >
                ...
                <option value={12}>{tBackupCard('snapshots_interval_12h')}</option>
                <option value={24}>{tBackupCard('snapshots_interval_24h')}</option>
                <option value={168}>{tBackupCard('snapshots_interval_168h')}</option>
              </select>
            </label>

            <label className="settings-field">
              <span className="settings-field-label">{tBackupCard('snapshots_retention_label')}</span>
              <input
                className="settings-input"
                type="number"
                min={1}
                ...
                onChange={event => {
                  ...
                  const retentionCount = Number.isFinite(raw)
                    ...
                    : 1;
                  setBackupSnapshotSettings(prev => ({ ...prev, retentionCount }));
                }}
            ...
            <div>
              ...
              <dd>
                <code title={backupSnapshotDirectory}>
                  {backupSnapshotDirectory || tBackupCard('snapshots_directory_empty')}
                </code>
              </dd>
            </div>
            <div>
          ...
          <div className="backup-snapshot-list-heading">{tBackupCard('snapshots_latest')}</div>
          {backupSnapshots.length === 0 ? (
            <p className="backup-snapshot-empty">{tBackupCard('snapshots_none')}</p>
          ) : (
      ...
      */}
      {settingsPoliciesWaybackOn && <div id="wayback-import" className="settings-section">
        <h2 className="settings-section-title">
          <span aria-hidden="true" className="wayback-icon-inline" style={{ marginRight: 8 }}>🕰</span>
          {tWayback('title')}
        </h2>
        ...
        <label className="settings-checkbox-row" style={{ marginTop: 14 }}>
          <input
            ...
            onChange={event => void saveWaybackShowImported(event.target.checked)}
            ...
            {waybackProgress ? (
                ...
                */}
                {waybackInitiator === 'resume' ? (
                  ...
                  >
                    ...
                    <span>
                      <strong style={{ marginRight: 4 }}>{tWayback('resume_label')}</strong>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, color: 'var(--text-2)' }}>
                  <span>
                    <strong style={{ color: 'var(--text-1)' }}>{waybackProgress.imported}</strong> {tWayback('stat_imported')}
                  </span>
                  {waybackProgress.unchanged > 0 ? (
                    <span>
                      <strong style={{ color: 'var(--text-1)' }}>{waybackProgress.unchanged}</strong> {tWayback('stat_no_op')}
                    </span>
                  ) : null}
                  {waybackProgress.skipped > 0 ? (
                    <span>
                      <strong style={{ color: 'var(--text-1)' }}>{waybackProgress.skipped}</strong> {tWayback('stat_skipped')}
                    </span>
                  ) : null}
                  <span style={{ color: waybackProgress.failed > 0 ? 'var(--danger, #b91c1c)' : undefined }}>
                    <strong style={{ color: waybackProgress.failed > 0 ? 'var(--danger, #b91c1c)' : 'var(--text-1)' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                  <span
                    aria-label={tWayback('status_aria', { status: waybackLastRun.status })}
                    style={{
                      display: 'inline-flex',
                      alignItems: 'center',
                      gap: 4,
                  ...
                  <strong style={{ color: 'var(--text-1)' }}>{tWayback('last_run')}</strong>
                  {waybackLastRun.startedAt ? (
                    <span style={{ color: 'var(--text-3)' }}>
                      · {fmtRelativeTime(tTime, tSettings, waybackLastRun.startedAt)}
                    </span>
                  ) : null}
                </div>
                {waybackLastRun.totals ? (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
                    <span>
                      <strong style={{ color: 'var(--text-1)' }}>{waybackLastRun.totals.imported}</strong> {tWayback('stat_imported')}
                    </span>
                    {waybackLastRun.totals.unchanged > 0 ? (
                      <span>
                        <strong style={{ color: 'var(--text-1)' }}>{waybackLastRun.totals.unchanged}</strong> {tWayback('stat_no_op')}
                      </span>
                    ) : null}
                    {waybackLastRun.totals.skipped > 0 ? (
                      <span>
                        <strong style={{ color: 'var(--text-1)' }}>{waybackLastRun.totals.skipped}</strong> {tWayback('stat_skipped')}
                      </span>
                    ) : null}
                    <span
                      style={{
                        color:
                          waybackLastRun.totals.failed > 0 ? 'var(--danger, #b91c1c)' : undefined,
                      }}
                    >
                      <strong
                        style={{
                    ...
                    <span style={{ color: 'var(--text-3)' }}>
                      {tWayback('across_apps', { count: waybackLastRun.totals.appsAttempted })}
                    </span>
                  </div>
              ...
              <div>{tWayback('summary_lead')} {waybackSummary}</div>
            ) : null}
          </div>
        ) : null}
      </div>}
      {/* Data Export */}
      {settingsAdminExportOn && <div id="export" className="settings-section">
        ...
        <p style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 12 }}>
          CSV includes one row per data type. JSON includes the full nested structure.
        </p>
      ...
      {(settingsAdminResetOn || settingsAdminStartOverOn) && (
        <div id="reset" className="settings-section settings-section-danger">
      ...
      {devOptsVisible && <div id="developer" className="settings-section">
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 16 }}>
          ...
          >
            {debugLoading ? <><span className="spinner-sm" /> {tDevAiDebug('loading')}</> : debugLog === null ? tDevAiDebug('load') : tDevAiDebug('refresh')}
          </button>
        ...
        {debugLog !== null && (
          <div style={{ marginTop: 16 }}>
            {debugLog.length === 0 ? (
              <div style={{ fontSize: 13, color: 'var(--text-3)', padding: '12px 2px' }}>
                {debugLogging
                  ...
                  return (
                    <li
                      ...
                      style={{
                        ...
                        borderRadius: 8,
                        padding: 12,
                        background: row.error ? 'rgba(255, 80, 80, 0.04)' : 'var(--surface-2)',
                      }}
                    >
                      <button
                        type="button"
                        onClick={() =>
                          setDebugExpandedId(prev => (prev === row.id ? null : row.id))
                        }
                        style={{
                          all: 'unset',
                          cursor: 'pointer',
                      ...
                      >
                        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                          ...
                          <span style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--text-3)' }}>
                            ...
                            {typeof row.durationMs === 'number' && tDevAiDebug('duration_suffix', { ms: row.durationMs })}
                          </span>
                        </div>
        ...
        {devActivityLogRetentionDaysOn && (
          <div
            className="settings-field"
            style={{ marginBottom: 8, padding: '6px 0', fontSize: 12, color: 'var(--text-3)' }}
          >
            <strong>{tDevActivity('retention_lead')}</strong>{tDevActivity('retention_body')}
          </div>
        )}
        {devActivityLogOn && <details
          id="activity-log"
        ...
        >
          <summary style={{ cursor: 'pointer', fontSize: 14, fontWeight: 600, userSelect: 'none' }}>
            {tDevActivity('summary')}
          </summary>
          <p className="settings-field-help" style={{ marginTop: 12, marginBottom: 12 }}>
            {tDevActivity('help')}
          </p>
          ...
          <div className="activity-log-toolbar">
            <label className="activity-log-filter">
              ...
              >
                ...
                <option value="policy_summary">{tDevActivityTypes('policy_summary')}</option>
                <option value="scheduled_sync">{tDevActivityTypes('scheduled_sync')}</option>
                <option value="manual_sync">{tDevActivityTypes('manual_sync')}</option>
                <option value="import">{tDevActivityTypes('import')}</option>
                <option value="backup_export">{tDevActivityTypes('backup_export')}</option>
                <option value="backup_restore">{tDevActivityTypes('backup_restore')}</option>
            ...
            <label className="activity-log-filter">
              ...
              >
                ...
                <option value="partial">{tDevActivity('status_partial')}</option>
                <option value="cancelled">{tDevActivity('status_cancelled')}</option>
              </select>
            </label>
            <label className="activity-log-filter">
            ...
            <label className="activity-log-filter">
              ...
              >
                <option value="started_at:desc">{tDevActivity('sort_started_desc')}</option>
                <option value="started_at:asc">{tDevActivity('sort_started_asc')}</option>
                <option value="ended_at:desc">{tDevActivity('sort_ended_desc')}</option>
                <option value="ended_at:asc">{tDevActivity('sort_ended_asc')}</option>
                <option value="duration_ms:desc">{tDevActivity('sort_duration_desc')}</option>
                <option value="duration_ms:asc">{tDevActivity('sort_duration_asc')}</option>
              </select>
            </label>
            <button
              type="button"
            ...
            {activityLog !== null && (
              ...
              >
                <span className="activity-log-live-dot" aria-hidden />
                {activityLivePaused ? tDevActivity('paused') : tDevActivity('live')}
              </button>
            )}
          </div>
            ...
            <div style={{ marginTop: 12 }}>
              {activityLog.length === 0 ? (
                <div className="activity-log-empty">
                  {(() => {
                    const parts: string[] = [];
                    if (activityStatusFilter) parts.push(`${activityStatusFilter}`);
                    if (activityTypeFilter) parts.push(activityTypeFilter.replace(/_/g, ' '));
                    if (parts.length > 0) {
                      return tDevActivity('empty_filter', { filter: parts.join(' ') });
                    }
                ...
                <>
                  <ul className="activity-log-list">
                    {activityLog.map(row => {
                      ...
                      const typeLabel = ACTIVITY_TYPE_LABELS[row.type]
                        ? tDevActivityTypes(row.type as Parameters<typeof tDevActivityTypes>[0])
                        : row.type;
                      const typeIcon = ACTIVITY_TYPE_ICONS[row.type] ?? '·';
                      const statusClass = `activity-status-pill activity-status-${row.status}`;
                      return (
                        <li key={row.id} className={`activity-log-row activity-status-row-${row.status}`}>
                          <button
                            type="button"
                            className="activity-log-header"
                            onClick={() =>
                              setActivityExpandedId(prev => (prev === row.id ? null : row.id))
                            }
                            aria-expanded={isExpanded}
                          >
                            <span className="activity-log-icon" aria-hidden>{typeIcon}</span>
                            <span className="activity-log-title">
                              <span className="activity-log-type">{typeLabel}</span>
                              {row.appName && (
                                <span className="activity-log-appname"> · {row.appName}</span>
                              )}
                            </span>
                          ...
                          {row.summary && (
                            <div className="activity-log-summary">{row.summary}</div>
                          )}
                  ...
                  <div className="activity-log-footer">
                    <span className="activity-log-count">
                      {tDevActivity('showing', { current: activityLog.length, total: activityTotal })}
                    </span>
                    {activityHasMore && (
                      <button
                        type="button"
                        ...
                        disabled={activityLoading}
                      >
                        {activityLoading ? <><span className="spinner-sm" /> {tDevActivity('loading')}</> : tDevActivity('load_more')}
                      </button>
                    )}
                  </div>
                </>
        ...
        {/* Advanced — per-phase AI request timeouts. Collapsed by default
            ...
            sprawling open. */}
        {devAdvancedAccordionOn && settingsAiTimeoutConfigOn && <details
          id="ai-timeouts"
          className="settings-advanced-details"
          open={devAdvancedAccordionFlag === 'on' || advancedAiOpen}
          onToggle={event => setAdvancedAiOpen((event.target as HTMLDetailsElement).open)}
          style={{ marginTop: 24, borderTop: '1px solid var(--border)', paddingTop: 16 }}
        >
          <summary style={{ cursor: 'pointer', fontSize: 14, fontWeight: 600, userSelect: 'none' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input
                ...
                onBlur={makeAiTimeoutBlurHandler(aiTimeoutDirectMs, aiTimeoutDirectAutoSave.save)}
                disabled={aiProvider === 'disabled' || aiTimeoutDirectAutoSave.saving}
              />
              <span className="settings-field-help">
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input
                ...
                onBlur={makeAiTimeoutBlurHandler(aiTimeoutChunkMs, aiTimeoutChunkAutoSave.save)}
                disabled={aiProvider === 'disabled' || aiTimeoutChunkAutoSave.saving}
              />
              <span className="settings-field-help">
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input
                ...
                onBlur={makeAiTimeoutBlurHandler(aiTimeoutMergeMs, aiTimeoutMergeAutoSave.save)}
                disabled={aiProvider === 'disabled' || aiTimeoutMergeAutoSave.saving}
              />
              <span className="settings-field-help">
        ...
        <div style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid var(--border)' }}>
          {devFeatureFlagPresetsOn && (
            ...
            >
              <strong>{tDevPresets('lead')}</strong>{tDevPresets('body')}
            </div>
          )}
      ...
      {(restoreStage === 'confirm' || restoreStage === 'applying') && restorePreview && (
        ...
        >
          ...
          >
            ...
            <h2 id="restore-backup-title" className="modal-title">
              {tModalRestore('title')}
            </h2>
            <p className="modal-copy">
              {pendingRestoreFilename ? (
                <>
                  ...
                  {restorePreview.exportedAt ? tModalRestore('exported_suffix', { date: fmtShortDate(restorePreview.exportedAt, dateMode) }) : null}
            ...
            <div className="backup-preview-table" role="table" aria-label={tAria('rows_per_table')}>
            ...
            <div className="modal-warning" style={{ marginTop: 12 }}>
              <strong>{tModalRestore('warning_lead')}</strong>{tModalRestore('warning_body')}
            </div>
            ...
            <div className="destructive-backup-offer">
              ...
              >
                {exportingBackup ? tBackupCard('download_busy') : tBackupCard('download_before_destructive')}
              </button>
            </div>

            <label className="modal-confirm-label" htmlFor="restore-confirm-input">
      ...
      {deleteTarget && (
        ...
        >
          ...
          >
            ...
            <h2 id="delete-import-title" className="modal-title">
              {tModalDelete('title')}
            </h2>
            <p className="modal-copy">
              {tModalDelete('meta', {
            ...
            <div className="delete-import-options">
              <label className={`delete-import-option${deleteTarget.mode === 'history-only' ? ' is-active' : ''}`}>
                <input
                  type="radio"
                  name="delete-import-mode"
                  value="history-only"
                  checked={deleteTarget.mode === 'history-only'}
                  onChange={() => setDeleteTarget({ ...deleteTarget, mode: 'history-only' })}
                  disabled={deleting}
                />
                <div>
                  <div className="delete-import-option-label">{tModalDelete('option_history_only_label')}</div>
              ...
              <label className={`delete-import-option${deleteTarget.mode === 'with-apps' ? ' is-active' : ''}`}>
                <input
                  type="radio"
                  name="delete-import-mode"
                  value="with-apps"
                  checked={deleteTarget.mode === 'with-apps'}
                  onChange={() => setDeleteTarget({ ...deleteTarget, mode: 'with-apps' })}
                  disabled={deleting}
                />
                <div>
                  ...
                  <div className="delete-import-option-desc">
                    {tModalDelete('option_with_apps_desc', { count: deleteTarget.importRow.imported })}
                  </div>
                </div>
              </label>
            </div>
      ...
      {waybackRemoveOpen && (
        <div className="modal-overlay" onClick={closeWaybackRemoveModal}>
          <div
            ...
            onClick={event => event.stopPropagation()}
            onKeyDown={event => { if (event.key === 'Escape') closeWaybackRemoveModal(); }}
          >
            <div className="modal-badge">{tWaybackRemove('badge')}</div>
            <h2 id="wayback-remove-title" className="modal-title">
              {tWaybackRemove('title')}
            </h2>
            <p id="wayback-remove-copy" className="modal-copy">
              {tWaybackRemove('body')}
            </p>
            <div className="modal-actions">
              <button
      ...
      */}
      {pendingItemRemoval && (() => {
        ...
        return (
          <div
            className="modal-overlay"
            onClick={() => { if (!closing) setPendingItemRemoval(null); }}
          >
            <div
              className="modal-card"
              role="dialog"
              ...
              onClick={event => event.stopPropagation()}
              onKeyDown={event => {
                if (event.key === 'Escape' && !closing) setPendingItemRemoval(null);
              }}
            >
              <div className="modal-badge">{tModalRemoveApp('badge')}</div>
              <h2 id="remove-item-title" className="modal-title">
                {tModalRemoveApp('title', { name: label })}
              </h2>
              <p id="remove-item-copy" className="modal-copy">
              ...
              <div className="modal-actions">
                ...
                >
                  {closing
                    ? <><span className="spinner-sm" aria-hidden="true" /> {tModalRemoveApp('removing')}</>
      ...
      {resetStep > 0 && (
        <div className="modal-overlay" onClick={closeResetModal}>
          <div
            ...
            onClick={event => event.stopPropagation()}
            onKeyDown={event => { if (event.key === 'Escape') closeResetModal(); }}
          >
            <div className="modal-badge">{tResetCard('modal_badge')}</div>
            <h2 id="reset-app-title" className="modal-title">
              {resetStep === 1
                ? tResetCard('modal_title_step_1')
                : tResetCard('modal_title_step_2')}
            </h2>
            <p id="reset-app-copy" className="modal-copy">
              {resetStep === 1
...
function ActivityRowDetail({ row }: { row: ActivityLogRow }) {
  ...
  if (fetchDiag) {
    if (typeof fetchDiag.httpStatus === 'number') {
      diagnosticLines.push({ label: 'HTTP status', value: String(fetchDiag.httpStatus) });
    }
    if (typeof fetchDiag.origin === 'string') {
      ...
      diagnosticLines.push({
        ...
        value: ORIGIN_LABELS[fetchDiag.origin as string] ?? String(fetchDiag.origin),
      });
    }
    if (typeof fetchDiag.contentType === 'string' && fetchDiag.contentType) {
      diagnosticLines.push({ label: 'Content-Type', value: fetchDiag.contentType as string });
    }
    if (typeof fetchDiag.networkHint === 'string' && fetchDiag.networkHint) {
      ...
      diagnosticLines.push({
        label: 'Network',
        value: NETWORK_HINT_LABELS[fetchDiag.networkHint as string] ?? String(fetchDiag.networkHint),
      });
    }
  }

  const requestedUrl = fetchDiag && typeof fetchDiag.requestedUrl === 'string'
    ...
    : (typeof (detail as Record<string, unknown>).url === 'string'
      ...
      : null);
  const finalUrl = fetchDiag && typeof fetchDiag.finalUrl === 'string'
    ? (fetchDiag.finalUrl as string)
    : null;
  const troubleshoot = fetchDiag && Array.isArray(fetchDiag.troubleshoot)
    ? (fetchDiag.troubleshoot as unknown[]).filter(x => typeof x === 'string') as string[]
    : [];

  const showTroubleshoot =
    row.status === 'error' &&
    (fetchDiag || errorMessage) &&
    (diagnosticLines.length > 0 || troubleshoot.length > 0 || requestedUrl || errorMessage);
    ...
    <div className="activity-log-detail-wrap">
      {showTroubleshoot && (
        <div className="activity-log-troubleshoot">
          ...
          {errorMessage && (
            <div className="activity-log-troubleshoot-message">{errorMessage}</div>
          )}
          {diagnosticLines.length > 0 && (
            <dl className="activity-log-troubleshoot-facts">
              {diagnosticLines.map(line => (
                <div key={line.label} className="activity-log-troubleshoot-fact">
                  <dt>{line.label}</dt>
                  <dd>{line.value}</dd>
                </div>
              ))}
            ...
            <dl className="activity-log-troubleshoot-facts">
              {requestedUrl && (
                <div className="activity-log-troubleshoot-fact">
                  <dt>Requested URL</dt>
                  <dd>
                    <a href={requestedUrl} target="_blank" rel="noreferrer noopener">
                      {requestedUrl}
                    </a>
                  </dd>
                </div>
              ...
              {finalUrl && finalUrl !== requestedUrl && (
                <div className="activity-log-troubleshoot-fact">
                  <dt>Final URL</dt>
                  <dd>
                    <a href={finalUrl} target="_blank" rel="noreferrer noopener">
                      {finalUrl}
                    </a>
                  </dd>
                </div>
