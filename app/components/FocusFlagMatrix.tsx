...
export default function FocusFlagMatrix({ rows }: FocusFlagMatrixProps) {
  ...
  // Stable ref so useModalFocus can call cancel() without closing over a
  // stale `applying` value (the ref is updated each render in the hook).
...
function FocusMatrixSurface({
  ...
  // Same lookup pattern as DevOptionsFeatureFlagPanel: the English map
  // documents the known surfaces and guards the translator call; the
  // visible label comes from `dev_options.feature_flags.surfaces.*`.
