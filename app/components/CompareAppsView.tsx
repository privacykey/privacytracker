...
export default function CompareAppsView({
  ...
  const [specA, setSpecA] = useState<string | null>(() => {
    if (initialSpec && pinnedSlot === "A") {
      return initialSpec;
    }
    if (initialSpecOther && pinnedSlot === "B" && !lockPinned) {
      return initialSpecOther;
    }
    return null;
  });
  const [specB, setSpecB] = useState<string | null>(() => {
    if (initialSpec && pinnedSlot === "B") {
      return initialSpec;
    }
    if (initialSpecOther && pinnedSlot === "A" && !lockPinned) {
      return initialSpecOther;
    }
    return null;
  });
  ...
  useEffect(() => {
    ...
    if (pinnedSlot === "A") {
      setSpecA(initialSpec);
    } else {
      setSpecB(initialSpec);
    }
  ...
  useEffect(() => {
    fetch("/api/apps")
      ...
      .catch(() => {
        /* keeps slots usable via App Store search even if DB is empty */
      });
  }, []);
  ...
  useEffect(() => {
    ...
    fetch("/api/privacy-profile")
      ...
      .catch(() => {
        /* optional — the grid still renders without a profile */
      });
    return () => {
      live = false;
    };
  ...
  useEffect(() => {
    ...
    fetch("/api/accessibility-profile")
      .then((r) =>
        r.ok
          ? (r.json() as Promise<{ profile: AccessibilityProfile | null }>)
          : null
      )
      .then((body) => {
        if (!(live && body)) {
  ...
  const refreshShortlist = useCallback(async () => {
    try {
      ...
      /* non-fatal: the UI falls back to "Shortlist" with no checkmark */
    }
  }, []);
  useEffect(() => {
    refreshShortlist();
  ...
  const sourceIdForA = useMemo(() => {
    ...
    if (specA?.startsWith("id:")) {
      return null;
    }
    if (specB?.startsWith("id:")) {
      return specB.slice(3);
    }
    return null;
  }, [specA, specB]);
  ...
  const shortlistSet = useMemo(() => {
    ...
    for (const p of shortlistPairs) {
      s.add(`${p.sourceAppId}::${p.candidateAppleId}`);
    }
    return s;
  }, [shortlistPairs]);
  ...
  const isShortlistedFor = useCallback(
    (sourceId: string | null) => (candidateId: string) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/compare?${qs}`, { signal: ctrl.signal })
      ...
      .then(({ a, b }) => {
        setSlotA({ kind: "ready", data: a });
        setSlotB({ kind: "ready", data: b });
      })
      .catch((e: Error) => {
    ...
    <div>
      ...
      <div className="compare-toolbar">
        {shortlistCount === 0 && (
          <span className="compare-toolbar-hint">
            Save candidates with the <strong>+ Shortlist</strong> button on the
            right to review them later.
          </span>
        )}
        <div className="compare-toolbar-right">
          ...
          >
            ...
            <span className="compare-shortlist-cta-label">
              {tCompare("shortlist_view_label")}
            </span>
            {shortlistCount > 0 && (
              <span aria-hidden="true" className="compare-shortlist-cta-count">
                {shortlistCount}
              </span>
            )}
          </Link>
      ...
      {(() => {
        const sourceForRelated =
          sourceIdForB ??
          sourceIdForA ??
          (initialSpec?.startsWith("id:") ? initialSpec.slice(3) : null);
        if (!sourceForRelated) {
          return null;
        }
        return (
        ...
        return (
          ...
          >
            <div className="compare-source-shortlist-head">
              <span
                aria-hidden="true"
                className="compare-source-shortlist-icon"
              >
                ★
              </span>
 ...
 */
function parseAppStoreUrlInput(
  raw: string
): { url: string; appleId: string } | null {
  ...
  if (!/^(apps|itunes)\.apple\.com$/i.test(parsed.hostname)) {
    return null;
  }
...
function deriveRiskLevelFromPrivacyTypes(
  ...
  if (!privacyTypes || privacyTypes.length === 0) {
    return null;
  }
  let trackCount = 0;
  let linkedCount = 0;
  ...
  for (const type of privacyTypes) {
    ...
    if (type.identifier === "DATA_USED_TO_TRACK_YOU") {
      trackCount += cats;
    } else if (type.identifier === "DATA_LINKED_TO_YOU") {
      linkedCount += cats;
    } else if (type.identifier === "DATA_NOT_LINKED_TO_YOU") {
      unlinkedCount += cats;
    }
  }
  if (trackCount === 0 && linkedCount === 0 && unlinkedCount === 0) {
    return null;
  }
  if (trackCount >= 1) {
    return "high";
  ...
  if (linkedCount >= 1 || unlinkedCount >= 1) {
    return "low";
  }
  return "minimal";
}
...
function SlotCard(props: {
  ...
  const riskLevel = liveData
    ? deriveRiskLevelFromPrivacyTypes(liveData.privacyTypes)
    : null;
  ...
  return (
    <>
      <button
        aria-label={
          isPicked
            ? tCompare("slot_card_change_aria", {
                label: props.label,
        ...
        {isPicked ? (
          <span className="compare-slot-card-picked-body">
            {displayIcon ? (
              ...
              <span
                aria-hidden="true"
                className="compare-slot-card-icon-placeholder"
              />
            )}
            <span className="compare-slot-card-text">
              <span className="compare-slot-card-name">
                {displayName || tCompare("slot_card_loading")}
              </span>
              <span className="compare-slot-card-developer">
                {displayDev || "—"}
              </span>
              {riskLevel && (
          ...
          <span className="compare-slot-card-empty-body">
            <span aria-hidden="true" className="compare-slot-card-plus">
              +
            </span>
            <span className="compare-slot-card-cta">
              {tCompare("slot_card_pick_cta", { label: props.label })}
            </span>
            <span className="compare-slot-card-hint">
              {tCompare("slot_card_pick_hint")}
            </span>
          </span>
        )}
        {isPicked && (
...
function SlotPicker({
  ...
  const currentLabel = useMemo(() => {
    ...
    if (spec.startsWith("url:")) {
      return tCompare("appstore_candidate");
    }
    return spec;
    // eslint-disable-next-line react-hooks/exhaustive-deps -- t* is a stable next-intl translator; including it forces a re-run on every render
  }, [spec, library]);
  ...
  return (
    <Wrapper {...wrapperProps}>
      {!hideHeader && (
        ...
        >
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 4 }}>
            ...
            >
              {tCompare("mode_library")}
            </ModeButton>
            <ModeButton
              active={mode === "search"}
              onClick={() => setMode("search")}
            >
              {tCompare("mode_appstore")}
            </ModeButton>
          </div>
        </div>
      )}
      ...
      {mode === "library" && (
        <select
          onChange={(e) =>
            onChange(e.target.value ? `id:${e.target.value}` : null)
          }
          style={selectStyle}
        ...
        <div>
          ...
          {pastedUrl &&
            (() => {
              ...
              const shortlisted =
                enableShortlistOnResults && isShortlisted(pastedUrl.appleId);
                ...
                >
                  ...
                  >
                    <button
                      ...
                      onClick={() =>
                        onChange(selected ? null : `url:${pastedUrl.url}`)
                      }
                      style={{
                        flex: 1,
                        minWidth: 0,
                      ...
                      title={
                        selected ? tCompare("result_deselect_title") : undefined
                      }
                      type="button"
                    >
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: 6,
                          background:
                            "color-mix(in srgb, var(--blue, #0a84ff) 18%, transparent)",
                    ...
                    {enableShortlistOnResults && sourceAppId && (
                      <button
                        ...
                        onClick={() =>
                          onToggleShortlist({
                            ...
                            name: tCompare("app_store_id_name", {
                              id: pastedUrl.appleId,
                            }),
                            url: pastedUrl.url,
                          })
                        }
                      ...
                      >
                        {shortlisted
                          ? tCompare("shortlist_saved")
                          : tCompare("shortlist_add")}
                      </button>
                    )}
                  </div>
                </div>
          ...
          {!(pastedUrl || searching || searchError) &&
            ...
            (() => {
              ...
              return (
                <>
                  <div
                    style={{
                      maxHeight: resultsExpanded ? 340 : 220,
                      overflow: "auto",
                      marginTop: 8,
                      display: "flex",
                      flexDirection: "column",
                  ...
                  >
                    {visible.map((m) => {
                      ...
                      const shortlisted =
                        enableShortlistOnResults && isShortlisted(m.appleId);
                      ...
                      return (
                        ...
                        >
                          <button
                            ...
                            onClick={() =>
                              onChange(selected ? null : `url:${m.url}`)
                            }
                            style={{
                              flex: 1,
                              minWidth: 0,
                            ...
                            title={
                              selected
                                ? tCompare("result_deselect_title")
                                : undefined
                            }
                            type="button"
                          >
                  ...
                  {hasMore && (
                    ...
                    >
                      {resultsExpanded
                        ? tCompare("show_less")
                        : tCompare("show_n_more", {
                            count: fullList.length - INITIAL_RESULT_LIMIT,
                          })}
                    </button>
                  )}
                </>
...
function RelatedAppsPanel({
  ...
  return (
    <section aria-label={tCompare("related_aria")} className="compare-related">
      <header className="compare-related-head">
        <button
          ...
          type="button"
        >
          <span aria-hidden="true" className="compare-related-toggle-icon">
            {open ? "▾" : "▸"}
          </span>
                  ...
                  <li key={c.appleId}>
                    <button
                      ...
                      title={
                        picked
                          ...
                          : tCompare("related_chip_pick_title", {
                              name: c.name,
                            })
                      }
                      type="button"
                    >
                      ...
                      <span className="compare-related-chip-body">
                        <span className="compare-related-chip-name">
                          {c.name}
                        </span>
                        {c.developer && (
                          <span className="compare-related-chip-dev">
                            {c.developer}
                          </span>
                        )}
                      </span>
                      {picked && (
                        <span
                          aria-hidden="true"
                          className="compare-related-chip-tick"
                        >
                          ✓
                        </span>
                      )}
...
function ComparisonBody({
  ...
  toggleShortlistFor: (
    sourceId: string | null
  ) => (c: ShortlistCandidate) => void | Promise<void>;
  ...
  if (a.kind === "loading" && b.kind === "loading") {
    return (
      <div className="empty-state" style={{ padding: 24 }}>
        <span className="spinner-sm" /> {tCompare("loading_comparison")}
      </div>
    );
  }
  if (a.kind === "error") {
...
function ComparisonTable({
  ...
  const aShortlistAction =
    sourceIdForA && a.id
      ? {
          isShortlisted: isShortlistedFor(sourceIdForA)(a.id),
          onClick: () =>
            toggleShortlistFor(sourceIdForA)({
              appleId: a.id,
              name: a.name,
      ...
      : null;
  const bShortlistAction =
    sourceIdForB && b.id
      ? {
          isShortlisted: isShortlistedFor(sourceIdForB)(b.id),
          onClick: () =>
            toggleShortlistFor(sourceIdForB)({
              appleId: b.id,
              name: b.name,
  ...
  const profileCountFor = (
    map: Map<string, CellSev>
  ): { mismatches: number; total: number } | null => {
    if (!profile) {
      return null;
    }
    ...
    for (const [catId, cell] of map) {
      ...
      if (TIER_RANK[tier] > TIER_RANK[allowed]) {
        mismatches += 1;
      }
    }
  ...
  return (
    <div>
      ...
      >
        <SlotHeader
          shortlistAction={aShortlistAction}
          slot={a}
          status={aStatus}
        />
        <SlotHeader
          shortlistAction={bShortlistAction}
          slot={b}
          status={bStatus}
        />
      </div>
      ...
      {!(aEmpty && bEmpty) && (
        ...
        >
          ...
          {hasActiveProfile && (
            <span
              aria-label={tCompare("profile_match_aria")}
              className="compare-profile-strip"
            >
              <span aria-hidden="true">🎯</span>
              {profileA && (
                <ProfileSummaryPill counts={profileA} label={a.name} />
              )}
              {profileB && (
                <ProfileSummaryPill counts={profileB} label={b.name} />
              )}
            </span>
          )}
          {refreshing && (
...
function NoDataNotice({
  ...
  return (
    <div className="compare-unfilled-banner" role="status">
      <span aria-hidden="true" className="compare-unfilled-banner-icon">
        ⚠️
      </span>
      <div className="compare-unfilled-banner-body">
        ...
        <div className="compare-unfilled-banner-detail">
          ...
          {bUnfilled && (
            <div style={{ marginTop: aUnfilled ? 4 : 0 }}>{reasonFor(b)}</div>
          )}
        </div>
        ...
        <div className="compare-unfilled-banner-foot">
          Privacy policies carry the authoritative disclosures until Apple
          captures labels — read the policy to confirm what the app actually
          collects.
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="compare-slot-header-row">
        ...
        >
          {slot.source === "library"
            ? tCompare("slot_label_tracked")
            : tCompare("slot_label_preview")}
        </span>
      </div>
      ...
      {hasFooter && (
        <div className="compare-slot-header-empty">
          ...
          {isUnfilled && (
            <span
              className="compare-slot-chip compare-slot-chip-warn"
              title={tCompare("chip_no_details_chip_title_alt")}
            >
              <span aria-hidden="true">⚠</span>
          ...
          {(isNoData || isUnfilled) &&
            (slot.privacyPolicyUrl ? (
              ...
              <span
                className="compare-slot-chip-muted"
                title={tCompare("no_policy_url_title")}
              >
...
function SeverityCell({
  ...
  const color =
    cell.severity === "DATA_USED_TO_TRACK_YOU"
      ? "var(--red)"
...
function AccessibilityComparisonTable({
  ...
  const canonicalIds = CANONICAL_ACCESSIBILITY_FEATURES.map(
    (f) => f.identifier
  );
  ...
  const onlyACount = ordered.filter(
    (id) => mapA.has(id) && !mapB.has(id)
  ).length;
  const onlyBCount = ordered.filter(
    (id) => !mapA.has(id) && mapB.has(id)
  ).length;
  ...
  const totalMissingPreferred =
    prefStats.required.missing + prefStats.nice.missing;
  ...
  const statusOf = (slot: SlotData): A11yStatus => {
    if (slot.accessibilityFeatures.length > 0) {
      return "labeled";
    }
    if (slot.hasAccessibilityLabels === 0) {
      return "none";
    }
    return "unknown";
  };
    ...
    <div>
            ...
            <span className="a11y-profile-key-summary">
              {totalMissingPreferred === 0 ? (
                <>
                  ...
                  {totalPreferred === 1 ? "" : "s"} covered by at least one app
                </>
              ) : (
                <>
                  {totalMissingPreferred} of {totalPreferred} missing from both
                  apps
                </>
              )}
            </span>
        ...
        {refreshing && (
          <span
            ...
            style={{
              ...
              borderRadius: 999,
              background:
                "color-mix(in srgb, var(--blue, #0a84ff) 14%, transparent)",
              border:
                "1px solid color-mix(in srgb, var(--blue, #0a84ff) 35%, transparent)",
              color: "var(--blue, #0a84ff)",
              fontWeight: 600,
            }}
          >
      ...
      >
        ...
        {ordered.map((featureId, i) => {
          ...
          const title =
            inA?.title ?? inB?.title ?? canonical?.title ?? featureId;
          const description =
            inA?.description ??
            inB?.description ??
          ...
          const prefLabelCell: React.CSSProperties = preference
            ? {
                ...prefCellShared,
                borderLeft: `2px ${prefBorderStyle} ${prefBorderColor}`,
              }
            : {};
          const prefRightmostCell: React.CSSProperties = preference
            ? {
                ...prefCellShared,
                borderRight: `2px ${prefBorderStyle} ${prefBorderColor}`,
              }
            : {};
          return (
      ...
      {(aStatus !== "labeled" || bStatus !== "labeled") && (
        ...
        >
          ...
          <div className="compare-unfilled-banner-body">
            {aStatus !== "labeled" && (
              <div style={{ marginBottom: bStatus === "labeled" ? 0 : 8 }}>
                <div className="compare-unfilled-banner-title">
                  {aStatus === "none"
                    ? tCompare("a11y_unfilled_no_features_title", {
                        name: a.name,
                      })
                    : tCompare("a11y_unfilled_unavailable_title", {
                        name: a.name,
                      })}
                </div>
                <div className="compare-unfilled-banner-detail">
...
function AccessibilitySlotHeader({
  ...
  const chip =
    status === "labeled" ? (
      ...
      <span
        className="compare-slot-chip compare-slot-chip-warn"
        title={tCompare("a11y_filed_no_features_title")}
      >
        <span aria-hidden="true">⚠</span>
      ...
      <span
        className="compare-slot-chip-muted"
        title={tCompare("a11y_shelf_absent_title")}
      >
  ...
  return (
    ...
    >
      ...
      {chip && <div className="compare-slot-header-empty">{chip}</div>}
    </div>
  );
}
      ...
      >
        {preferenceMissing ? (
          <>
            ...
            <span>
              Missing{" "}
              {preference === "required" ? "(required)" : "(nice-to-have)"}
            </span>
          </>
        ) : (
          <span>—</span>
