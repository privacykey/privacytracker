...
export function ImportQueueProvider({ children }: { children: ReactNode }) {
  ...
  const buildProgressLabel = useCallback((snap: ImportQueueSnapshot): string => {
    if (snap.queued === 0) return 'Draining…';
    if (snap.pausedUntil) {
      const secs = Math.max(1, Math.ceil((snap.pausedUntil - Date.now()) / 1000));
    ...
    if (snap.soonestNextAttemptAt && snap.soonestNextAttemptAt > Date.now()) {
      const secs = Math.max(1, Math.ceil((snap.soonestNextAttemptAt - Date.now()) / 1000));
      return `${snap.queued} queued · next retry in ${secs}s`;
    }
    return `${snap.queued} queued · retrying now`;
  }, []);

  const syncTaskHandle = useCallback((snap: ImportQueueSnapshot) => {
    if (snap.queued > 0) {
      ...
      if (!taskHandleRef.current) {
        ...
        taskHandleRef.current.setProgress(done, total, buildProgressLabel(snap));
      }
  ...
  const fetchStatus = useCallback(async (): Promise<ImportQueueSnapshot | null> => {
    try {
      ...
      const snap: ImportQueueSnapshot = {
        ...
        oldestNextAttemptAt: typeof data?.oldestNextAttemptAt === 'number' ? data.oldestNextAttemptAt : null,
        soonestNextAttemptAt: typeof data?.soonestNextAttemptAt === 'number' ? data.soonestNextAttemptAt : null,
        items: Array.isArray(data?.items) ? data.items : [],
        pausedUntil: typeof data?.pausedUntil === 'number' ? data.pausedUntil : null,
  ...
  const retryNow = useCallback(async (signal?: AbortSignal): Promise<ImportQueueTickResult> => {
    ...
    const rateLimited = typeof data?.rateLimited === 'number' ? data.rateLimited : 0;
    const pausedUntil = typeof data?.pausedUntil === 'number' ? data.pausedUntil : null;
    const skipped = data?.skipped === 'paused' || data?.skipped === 'busy' || data?.skipped === 'empty'
      ? data.skipped
      : null;
    ...
    void (async () => {
      ...
      try {
        while (!drainCancelRef.current) {
          ...
          if (drainCancelRef.current) break;

          processed += result.processed;
          setDrainState(prev =>
            prev ? { ...prev, processed, pausedUntil: result.pausedUntil } : prev,
          );
          ...
          for (const cb of tickSubscribersRef.current) {
            try { cb(result); } catch (e) { console.warn('[import-queue] tickComplete subscriber threw:', e); }
          }

          if (result.skipped === 'empty') break;
          ...
          if (result.skipped === 'busy') {
            ...
            await sleepCancellable(3_000, drainCancelRef);
            continue;
          }
