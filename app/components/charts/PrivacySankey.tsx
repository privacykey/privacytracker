...
function prettyKind(t: SankeyT, nodeName: string): string {
  if (nodeName.startsWith(PREFIX_APP)) {
    return t("kind_app");
  }
  if (nodeName.startsWith(PREFIX_SEV)) {
    return t("kind_severity");
  }
  if (nodeName.startsWith(PREFIX_CAT)) {
    return t("kind_category");
  }
  return "";
}
...
export default function PrivacySankey() {
  ...
  const labelsByName = useMemo(() => {
    ...
    for (const app of data.apps) {
      m.set(`${PREFIX_APP}${app.id}`, app.name);
    }
    for (const s of data.severities) {
      m.set(`${PREFIX_SEV}${s.identifier}`, s.label);
    }
    for (const c of data.categories) {
      m.set(`${PREFIX_CAT}${c.identifier}`, c.label);
    }
    return m;
  }, [data]);

  const prettyLabel = useCallback(
    (name: string): string => labelsByName.get(name) ?? name,
    [labelsByName]
  );
  ...
  const { option, hiddenCount } = useMemo(() => {
    ...
    const rawNodes = [
      ...
      ...data.severities
        .filter((s) => usedSevs.has(s.identifier))
        .map((s) => sevNode(s.identifier, s.label)),
      ...data.categories
        .filter((c) => usedCats.has(c.identifier))
        .map((c) => catNode(c.identifier, c.label)),
    ];
    ...
    const links = rawLinks.map((l) => {
      ...
      if (adjacentLinkKey.has(`${l.source}→${l.target}`)) {
        return l;
      }
        ...
        {lockedLabel ? (
          <>
            <span style={{ color: "var(--text-2)" }}>
              Highlighting:{" "}
              <strong style={{ color: "var(--text)" }}>{lockedLabel}</strong>
            </span>
            <button
          ...
          <span>
            Click any node to lock its highlight. Hover still works as normal.
          </span>
        )}
        {hiddenCount > 0 && (
          <span style={{ marginLeft: "auto" }}>
            Showing top {MAX_APPS} apps by category count · {hiddenCount} more
            not shown
          </span>
        )}
      </div>
