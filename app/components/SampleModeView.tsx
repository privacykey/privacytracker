...
import { readSampleApps, clearSampleApps, type SampleApp } from '@/lib/sample-apps';
...
export default function SampleModeView() {
  ...
  if (!loaded) return <p style={{ padding: 32 }}>{tSample('loading')}</p>;
  ...
  if (apps.length === 0) {
    return (
      <div className="page-container" style={{ padding: 32 }}>
        ...
        <Link href="/welcome" className="btn btn-primary" style={{ marginTop: 16, display: 'inline-block' }}>
  ...
  return (
    <div className="page-container home-page sample-mode-view" style={{ padding: 32 }}>
      {sampleBannerOn && (
        <div
        ...
        >
          <div>
            <strong>{tSample('banner_lead')}</strong> {tSample('banner_body', { count: apps.length })}
          </div>
      ...
      >
        {apps.map((app) => (
          <SampleAppCard key={app.id} app={app} />
        ))}
      </div>
    </div>
  );
...
function SampleAppCard({ app }: { app: SampleApp }) {
  ...
  const riskColour =
    app.riskTier === 'high' ? '#d04040' :
    app.riskTier === 'moderate' ? '#e69020' :
    app.riskTier === 'low' ? '#5aa860' :
    '#888';
  ...
  return (
    ...
    >
      <header style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
        ...
        <div style={{ flex: 1 }}>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 600 }}>{app.name}</h2>
          <p style={{ margin: 0, fontSize: 12, color: 'var(--text-3)' }}>{app.developer}</p>
        </div>
        <span
          style={{
            fontSize: 10,
