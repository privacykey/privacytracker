...
export default function InfoTooltip({
  ...
  const computePlacement = useCallback((): BubblePlacement | null => {
    ...
    const preferTop = side === 'top' ? spaceAbove >= bHeight + VIEWPORT_MARGIN : false;
    const flow: BubblePlacement['flow'] =
      side === 'top'
        ? (preferTop ? 'top' : (spaceBelow >= bHeight + VIEWPORT_MARGIN ? 'bottom' : (spaceAbove >= spaceBelow ? 'top' : 'bottom')))
        : (spaceBelow >= bHeight + VIEWPORT_MARGIN ? 'bottom' : 'top');
