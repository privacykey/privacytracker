...
browserFlow(
  "AppGrid profile-mismatch filter narrows the visible apps to those that exceed the profile",
  async ({ page }) => {
    ...
    // grid wiring is sane.
    const expectedAfter = Number.parseInt(
      (await mismatchToggle.locator(".mismatch-toggle-count").textContent()) ??
        "0",
      10
    );
    ...
    await expect(cards.filter({ has: page.getByText(/^Notes$/) })).toHaveCount(
      0
    );
  }
);
