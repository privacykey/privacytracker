...
browserFlow(
  "large onboarding import batches item status writes instead of fan-out updates",
  async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Importing apps/i })
    ).toBeVisible();
...
browserFlow(
  "onboarding import drains queued apps chunk-after-chunk until empty",
  async ({ page }) => {
    ...
    await page.route("**/api/imports**", async (route) => {
      ...
      if (url.pathname === "/api/imports/queue") {
        if (method === "POST") {
          const processed =
            processedPlan[Math.min(queuePostCalls, processedPlan.length - 1)];
          queuePostCalls += 1;
    ...
    await expect
      .poll(() => queuePostCalls, { timeout: 5000 })
...
browserFlow(
  "onboarding import drain waits through a 429 cooldown and resumes",
  async ({ page }) => {
    ...
    await expect
      .poll(() => queuePostCalls, { timeout: 8000 })
      .toBeGreaterThanOrEqual(4);
  }
);
