...
browserFlow('audit-bundle dedup: importing the same bundle twice surfaces the re-import modal', async ({ page, request }) => {
  ...
  const exportRes = await request.post('/api/export/audit-bundle', {
    ...
    data: { recommenderName: RECOMMENDER_NAME, includeRecommenderProfile: false },
  });
  ...
  await expect(modal.locator('.modal-title')).toContainText(/already imported/i);

  // The primary action is now "Re-import anyway" — verify the
  // copy-button label has shifted.
  await expect(modal.getByRole('button', { name: /re.import anyway/i })).toBeVisible();
