...
test.beforeEach(async ({ request }) => {
  ...
  const resetRes = await request.post('/api/reset', { headers: sameOriginHeaders });
...
browserFlow('app detail: seeded sample app renders hero, categories, timeline, and mismatch flags', async ({ page, request }) => {
  ...
  const seedBody = (await seedRes.json()) as { apps?: SeedResult[]; results?: SeedResult[] };
  ...
  expect(instagram, 'expected Instagram to land in the seeded apps').toBeDefined();
  expect(instagram?.id, 'seed result must include the synthetic app id').toBeTruthy();
