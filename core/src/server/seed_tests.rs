//! Replays `core/tests/fixtures/seed-cases.json` the way `imports_tests`
//! replays the import pipeline, with the network canned: each case's
//! replies are served in order by the Phase 3 test fetcher, and the raw
//! calls it saw are compared with the ones Node's stub recorded — so a
//! chart request for the wrong region or the wrong number of apps fails
//! here, not on Apple. Foreign keys stay ON.
use super::body::BodyOutcome;
use super::{
...
#[test]
fn seed_matches_node_wire_calls_stream_and_rows() {
    ...
    let fixture: Value =
        serde_json::from_str(include_str!("../../tests/fixtures/seed-cases.json")).unwrap();
    ...
    for case in fixture["cases"].as_array().unwrap() {
        ...
        let actual = json!({"status": status, "body": body, "type": content_type, "retryAfter": retry_after});
