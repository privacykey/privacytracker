...
**The oracle — `core/scripts/extract-device-routes-cases.mjs`.** 146
...
**Node's behaviour, kept, and one bug filed.** It has its own follow-up
to fix Node and the core together: the direct-child test accepts the
MobileSync root's own parent, whose relative form `..` is one segment
with no separator, so a non-empty `Manifest.db` beside `Backup/`
verifies. Also kept: a
