...
**Node's behaviour, kept, and a bug filed.** A resumed run keeps the
blob's original initiator, so `state.initiator === "resume"` is never
true for a real resume, in this runner or the other two: TaskCenter's
"Resumed after restart" card and the Wayback section's pill never
appear, and "(resumed after restart)" never reaches the activity log. It
has its own follow-up to fix Node and the core together. Also kept: a
blob that does not parse is left in place when the resume heals a stale
lock, since it is not a state.
