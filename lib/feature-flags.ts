...
export const FLAG_REGISTRY: Record<FlagKey, FlagRegistration> =
  Object.fromEntries(
...
function computeFlag(key: FlagKey, ctx: ResolverContext): FlagValue {
  ...
  if (
    key === "flag.desktop.app_section" &&
    ctx.runtimeEnvironment === "desktop"
  ) {
    value = "on";
  }
  if (
    key === "flag.onboarding.method.configurator" &&
    ctx.runtimeEnvironment === "desktop"
  ) {
    value = "on";
  }
...
export function getFocusState(read: (key: string) => string): FocusState {
  ...
  const aiConfigured =
    read("ai_provider") !== "" && read("ai_provider") !== "disabled";
 ...
 */
export function getResolverContext(
  ...
  const runtimeEnvironment =
    read("runtime_environment") === "desktop" ? "desktop" : undefined;
...
export function enabledForAudience(
  audience: Audience,
  ctx?: ResolverContext
): boolean {
  const context = ctx ?? requireContext();
...
export function enabledForGoal(
  goal: PrimaryGoal | Modifier,
  ctx?: ResolverContext
): boolean {
  const context = ctx ?? requireContext();
...
export function resetSurfaceOverrides(surfacePrefix: string) {
  ...
  for (const key of newOverrides.keys()) {
    if (key.startsWith(`flag.${surfacePrefix}.`)) {
      newOverrides.delete(key);
    }
  }
  setResolverContext({ ...activeCache.context, overrides: newOverrides });
...
export { activeGoalsFrom, TOUR_STEPS };
