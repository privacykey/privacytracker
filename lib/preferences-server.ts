...
export function getUserIntent(): UserIntent | null {
  ...
  if (audience) {
    if (audience === 'guardian') return 'family';
    if (audience === 'loved_one') return 'curious';
    ...
    if (minimal) return 'curious';
    if (understand && declutter) return 'hygiene';
    if (declutter) return 'cleanup';
    if (understand) return 'curious';
    return 'curious';
  }
...
export function setManualAppsBannerDismissed(dismissed: boolean): void {
  setSetting(MANUAL_APPS_BANNER_DISMISSED_KEY, dismissed ? String(Date.now()) : '');
}
