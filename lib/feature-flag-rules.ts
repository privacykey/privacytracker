...
export const AUDIENCE_RULES: Record<Audience, Partial<Record<FlagKey, FlagValue>>> = {
...
export const GOAL_RULES: Record<PrimaryGoal, Partial<Record<FlagKey, FlagValue>>> = {
...
export const FLAG_DEPENDENCIES: Partial<Record<FlagKey, FlagKey>> = {
  ...
  'flag.detail.timeline.matches_live_sync_badge':  'flag.detail.timeline.wayback_rows',
  'flag.detail.timeline.policy_diff_toggle':       'flag.detail.timeline.live_rows',
  'flag.detail.timeline.policy_preview_toggle':    'flag.detail.timeline.live_rows',
  ...
  'flag.detail.timeline.review_snapshot_chips':    'flag.detail.timeline.review_rows',
  ...
  'flag.onboarding.method.import_audit_bundle':    'flag.onboarding.step.choose_method',
  ...
  'flag.onboarding.goals_picker.accessibility_modifier':'flag.onboarding.goals_picker',
  'flag.onboarding.audience_picker.skip':               'flag.onboarding.audience_picker',

  // Confirm-step sub-toggle
  'flag.onboarding.confirm.hide_tracked_toggle':   'flag.onboarding.step.confirm_matches',
  'flag.onboarding.import.rate_limit_handoff':     'flag.onboarding.step.import_progress',
...
export function activeGoalsFrom(input: {
  ...
  if (input.minimal) {
    ...
    if (input.understand) goals.add('understand');
    if (input.declutter) goals.add('declutter');
    if (goals.size === 0) goals.add('understand'); // silent default per §4.2
  }
  if (input.accessibility) goals.add('accessibility');
  return goals;
}
