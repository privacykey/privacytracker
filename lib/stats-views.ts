...
import type { PolicyLensKey, PolicyRating, PolicySummary } from './policy-summary-meta';
import { POLICY_LENSES, POLICY_RATINGS } from './policy-summary-meta';
import type {
  SeverityId, MatrixApp, MatrixCategory, MatrixData,
  TimelineBucket, TimelinePoint, TimelineData,
  RadarLens, RadarApp, RadarData,
...
const SEVERITY_ORDER: SeverityId[] = ['DATA_USED_TO_TRACK_YOU', 'DATA_LINKED_TO_YOU', 'DATA_NOT_LINKED_TO_YOU'];
...
export function getMatrixData(): MatrixData {
  ...
  for (const { appId, sev, cat } of rows) {
    if (!SEVERITY_ORDER.includes(sev as SeverityId)) continue;
    const s = sev as SeverityId;
    const appCells = (cells[appId] ??= {});
    const prev = appCells[cat];
    if (!prev || sevRank[s] > sevRank[prev]) appCells[cat] = s;

    if (!catCounts.has(cat)) catCounts.set(cat, new Set());
  ...
  const categoryOrder = [...canonical, ...extras].filter(c => catCounts.has(c));
...
function bucketKey(ts: number, kind: TimelineBucket): string {
  ...
  if (kind === 'month') return `${yyyy}-${mm}-01`;
  if (kind === 'week') {
    // ISO week: shift to Monday 00:00 UTC
    const dow = d.getUTCDay(); // 0=Sun
    const diff = (dow + 6) % 7;
    const monday = new Date(Date.UTC(yyyy, d.getUTCMonth(), d.getUTCDate() - diff));
  ...
  for (const row of rows) {
    ...
    for (const e of entries) {
      ...
      if (e.category === 'accessibility') {
        if (e.type === 'added') point.accessibilityAdded = (point.accessibilityAdded ?? 0) + 1;
        else if (e.type === 'removed') point.accessibilityRemoved = (point.accessibilityRemoved ?? 0) + 1;
        else continue;
      } else if (e.type === 'added') point.added++;
      else if (e.type === 'removed') point.removed++;
      else if (e.type === 'modified') point.modified++;
      else if (e.type === 'policy') point.policy++;
      else continue;
      total++;
    }
  }
...
export function getRadarData(appIds?: string[]): RadarData {
  const placeholders = (appIds && appIds.length) ? appIds.map(() => '?').join(',') : null;
  const rows = placeholders
    ? db.prepare(`
      ...
      `).all(...appIds!) as { id: string; name: string; iconUrl: string; summary_json: string | null; status: string | null }[]
    : db.prepare(`
        SELECT a.id, a.name, a.iconUrl, p.summary_json, p.status
        FROM apps a
        LEFT JOIN privacy_policy_analyses p ON p.app_id = a.id
        ...
        LIMIT 6
      `).all() as { id: string; name: string; iconUrl: string; summary_json: string | null; status: string | null }[];

  const apps: RadarApp[] = rows.map(r => {
    let summary: PolicySummary | null = null;
    if (r.summary_json) {
      try { summary = JSON.parse(r.summary_json) as PolicySummary; } catch { /* ignore malformed */ }
    }
    const byKey = new Map<PolicyLensKey, { rating: PolicyRating; summary: string }>();
    for (const lens of summary?.lenses ?? []) byKey.set(lens.key, lens);
