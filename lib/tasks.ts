...
import type { Audience, FocusState, Modifier, PrimaryGoal } from './feature-flag-rules';
...
const has = (focus: FocusState, goal: PrimaryGoal | Modifier) => focus.goals.has(goal);
...
export const TASK_DEFS: UserTaskDef[] = [
  ...
  },
  {
    ...
    includedWhen: focus => has(focus, 'understand') || has(focus, 'declutter'),
...
export function resolveTaskState(
  ...
  if (def.completionCheck(ctx)) return 'completed';
  if (userState?.dismissed_at) return 'dismissed';
  ...
  if (userState?.started_at) {
    ...
    if (now - userState.started_at < STALE_MS) return 'in_progress';
  }
  return 'ready';
}
...
export function resolveTasks(
  ...
  const visibleDefs = TASK_DEFS.filter(d => {
    if (!d.includedWhen(focus, env)) return false;
    if (d.optInOnly && !blob.tasks[d.id]?.opted_in_at) return false;
    return true;
  });
  ...
  return visibleDefs.map(def => {
    ...
    const state = resolveTaskState(
      ...
      ctx,
      userState,
      TASK_DEFS.map(d => ({ id: d.id, isCompleted: completionMap.get(d.id) ?? false })),
      now,
    );
    return {
      id: def.id,
...
export function getOptInCandidates(
  ...
  return TASK_DEFS.filter(d => {
    if (!d.optInOnly) return false;
    if (!d.includedWhen(focus, env)) return false;
    if (blob.tasks[d.id]?.opted_in_at) return false;
    // Don't offer to opt into a task you're already done with — that's
    // confusing UX. Derived-completion still applies.
    if (d.completionCheck(ctx)) return false;
    return true;
