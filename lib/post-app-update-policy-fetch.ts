...
async function drainPolicyFetchQueue(): Promise<void> {
  ...
  try {
    const { canStartPolicyManualRun, runBulkPolicySync } = await import('./policy-bulk-runner');
    ...
    busyRetries = 0;
    console.info(`[PolicyFetch] Starting deferred ${reasonLabel} policy source fetch`);
    ...
    console.warn(`[PolicyFetch] Deferred ${reasonLabel} policy source fetch failed:`, error);
  }
}
