...
export const POLICY_RATINGS = ['favorable', 'mixed', 'concerning', 'unclear'] as const;
...
export const POLICY_SOURCE_ORIGINS = ['direct', 'browser_retry', 'wayback'] as const;
export type PolicySourceOrigin = (typeof POLICY_SOURCE_ORIGINS)[number];

export const POLICY_SOURCE_ORIGIN_META: Record<PolicySourceOrigin, { label: string; hint: string }> = {
  direct: { label: 'Direct fetch', hint: 'Fetched straight from the developer\u2019s privacy-policy link.' },
  browser_retry: { label: 'Retried as browser', hint: 'The first fetch was blocked, so we retried with a desktop-browser header bundle.' },
  wayback: { label: 'From Wayback Machine', hint: 'The live policy page blocked us, so this summary uses the most recent archived copy.' },
};
...
export const POLICY_RATING_META: Record<PolicyRating, { label: string; cls: string }> = {
