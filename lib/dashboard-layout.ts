...
export const DASHBOARD_PRESET_META: Record<DashboardPresetKey, DashboardPresetMeta> = {
  default: {
    key: 'default',
    label: 'Default',
    shortLabel: 'Default',
    description: 'The canonical order with every card visible. A safe baseline.',
    icon: '🏠',
  ...
  minimal: {
    ...
    label: 'Minimal',
    shortLabel: 'Minimal',
    description: 'Just the essentials — hero, risk, review, and at-a-glance stats.',
    icon: '🪶',
  ...
  caretaker: {
    ...
    description: 'Family-focused. Surface privacy concerns and review queue first.',
    icon: '🫶',
  ...
  watchdog: {
    ...
    label: 'Watchdog',
    shortLabel: 'Watchdog',
    description: 'Risk-first. Quickly spot the apps you might want to replace or sync.',
    icon: '🕵️',
  ...
  at_a_glance: {
    ...
    description: 'Stats up top. For users who want the high-level picture first.',
    icon: '📊',
    severityCls: 'severity-unlinked',
  },
};
...
function buildPreset(
  visibleFirstClass: DashboardCardId[],
): DashboardLayout {
  ...
  for (const id of visibleFirstClass) {
    if (!CANONICAL_SET.has(id)) continue;
    order.push(id);
    consumed.add(id);
  ...
  for (const id of FIRST_CLASS_CARDS) {
    if (!visibleSet.has(id)) hidden.push(id);
  }
  ...
  hidden.sort((a, b) => CANONICAL_ORDER.indexOf(a) - CANONICAL_ORDER.indexOf(b));
...
function normaliseLayout(layout: DashboardLayout): DashboardLayout {
  ...
  const hidden = layout.hidden
    .filter(id => {
      if (!FIRST_CLASS_CARDS.has(id)) return false;
...
function layoutsEqual(a: DashboardLayout, b: DashboardLayout): boolean {
  ...
  if (an.order.length !== bn.order.length) return false;
  for (let i = 0; i < an.order.length; i += 1) {
    if (an.order[i] !== bn.order[i]) return false;
  }
  if (an.hidden.length !== bn.hidden.length) return false;
  for (let i = 0; i < an.hidden.length; i += 1) {
    if (an.hidden[i] !== bn.hidden[i]) return false;
  }
  return true;
}
...
export function reconcileLayout(
  ...
  const known = (Array.isArray(s.order) ? s.order : [])
    .filter((id): id is DashboardCardId => typeof id === 'string' && canonicalSet.has(id as DashboardCardId));
  ...
  const hidden = (Array.isArray(s.hidden) ? s.hidden : [])
    .filter((id): id is DashboardCardId => typeof id === 'string' && FIRST_CLASS_CARDS.has(id as DashboardCardId));
...
export function isCardVisible(
  ...
  if (flagResolved && flagResolved !== 'on' && flagResolved !== 'collapsed') return false;
  if (FIRST_CLASS_CARDS.has(id) && layout.hidden.includes(id)) return false;
  return true;
}
