...
export const NOTIFICATION_TYPE_META: Record<NotificationTypeKey, NotificationTypeMeta> = {
  labelChanges: {
    ...
    description: 'When a tracked app adds, removes, or changes a privacy label on the App Store.',
    example: 'e.g. "Instagram now collects Health & Fitness data (Linked to You)."',
    defaultOn: true,
  },
  profileMismatch: {
    key: 'profileMismatch',
    label: 'Privacy profile mismatches',
    description: 'When a tracked app starts collecting data at a tier beyond what your privacy profile allows.',
  ...
  policyUpdates: {
    ...
    description: 'When the developer\u2019s linked privacy policy document changes materially since the last sync.',
    example: 'e.g. "Spotify\u2019s privacy policy has been updated \u2014 new section on third-party sharing."',
    defaultOn: true,
  },
  versionUpdates: {
    key: 'versionUpdates',
    label: 'App version updates',
    description: 'When Apple reports a new App Store version for a tracked app \u2014 useful for cross-referencing a release with a privacy-label change.',
    example: 'e.g. "Instagram updated from v287.0 to v288.0 (released 14 Apr 2026)."',
    defaultOn: true,
  },
  importCompleted: {
    key: 'importCompleted',
    label: 'Import finished',
    description: 'A one-shot confirmation each time a bulk import finishes, including partial/queued runs.',
  ...
  manualAppsPrompt: {
    ...
    description: 'A 24h-debounced nudge after an import to review rows that didn\u2019t match an App Store listing (e.g. TestFlight or web clips).',
    example: 'e.g. "3 rows didn\u2019t match the App Store \u2014 track them under Manual apps."',
    defaultOn: true,
  },
  aiTimeout: {
    key: 'aiTimeout',
    label: 'AI policy summary timeouts',
    description: 'When an AI-powered privacy-policy summary call aborts because it exceeded the configured timeout.',
...
export const DEFAULT_NOTIFICATION_PREFS: Record<NotificationTypeKey, boolean> =
  NOTIFICATION_TYPE_KEYS.reduce((acc, key) => {
 ...
 * shapes; never throws.
 */
export function parseStoredPrefs(raw: string | null | undefined): NotificationPrefs {
  if (!raw) return {};
  let parsed: unknown;
  try {
 ...
 */
export function resolvePrefs(stored: NotificationPrefs | null | undefined): Record<NotificationTypeKey, boolean> {
...
export function classifyNotificationType(
  ...
  if (!entries || entries.length === 0) return 'labelChanges';
  const firstType = entries[0]?.type;
  if (firstType === 'ai_timeout') return 'aiTimeout';
  if (firstType === 'manual_apps_prompt') return 'manualAppsPrompt';
  if (firstType === 'import_completed') return 'importCompleted';
  if (firstType === 'profile_mismatch') return 'profileMismatch';
  if (firstType === 'version_update') return 'versionUpdates';
  ...
  if (entries.some(e => e?.type === 'policy' || e?.type === 'policy_summary')) {
