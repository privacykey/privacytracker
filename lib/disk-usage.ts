...
function dataDirSize(dir: string): {
  total: number;
  backupsBytes: number;
  backupCount: number;
} {
  if (!existsSync(dir)) {
    return { total: 0, backupsBytes: 0, backupCount: 0 };
  }
  let total = 0;
  let backupsBytes = 0;
  let backupCount = 0;
...
export function snapshotDisk(): DiskSnapshot {
  ...
  const freePct =
    totalBytes > 0 ? Math.round((freeBytes / totalBytes) * 100) : 0;
  ...
  const lastBackupSnapshotAt = lastBackupRaw
    ? Number(lastBackupRaw) || null
    : null;
