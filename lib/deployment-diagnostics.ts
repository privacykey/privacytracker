  ...
  try {
    ...
    if (!row || row.ok !== 1) {
      return {
        status: "degraded",
        dbPingMs,
        error: "Database ping returned an unexpected result.",
      };
    }
...
function buildChecks(
  ...
  const checks: DeploymentDiagnosticCheck[] = [
    ...
    },
    {
      ...
      label: "Transport",
      status:
        network.protocol === "https" || network.localOnlyHost ? "ok" : "warn",
      detail:
...
export function buildDeploymentDiagnostics(
  ...
  const runtimeEnvironment =
    getSetting("runtime_environment", "") === "desktop" ? "desktop" : "web";
...
function redactFetchDiagnostics(
...
export function buildDeploymentSupportBundle(
  headers: Headers
): DeploymentSupportBundle {
