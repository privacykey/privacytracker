...
export function compareVersions(a: string, b: string): number {
  const [aCore, aPre] = stripBuild(a).split("-", 2) as [
    string,
    string | undefined,
  ];
  const [bCore, bPre] = stripBuild(b).split("-", 2) as [
    string,
    string | undefined,
  ];
  ...
  for (let i = 0; i < len; i++) {
    ...
    const xn = /^\d+$/.test(x) ? Number.parseInt(x, 10) : Number.NaN;
    const yn = /^\d+$/.test(y) ? Number.parseInt(y, 10) : Number.NaN;
