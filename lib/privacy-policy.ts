...
function stashFetchDiagnostics(appId: string, diag: PolicyFetchDiagnostics): void {
 ...
 */
function hintsForHttpStatus(status: number, finalUrl?: string | null): string[] {
  const hints: string[] = [];
  const urlLabel = finalUrl ? safeUrlLabel(finalUrl) : 'the developer\'s site';
  if (status === 401) {
    hints.push('The site requires authentication before serving the policy page — likely an intranet link that shouldn\'t be public.');
    hints.push('Ask the developer to host the policy at a public URL.');
  } else if (status === 403) {
    hints.push(`${urlLabel} is rejecting automated requests. The Chrome-header retry and Wayback fallback both failed.`);
    hints.push('Open the URL in a real browser — if it loads there, the site is specifically blocking server traffic (CloudFlare bot-fight, Akamai, etc.).');
    hints.push('If the URL is still correct, submit it to the Wayback Machine (web.archive.org/save) and re-try the scrape in an hour.');
  } else if (status === 404) {
    hints.push('The policy URL returned Not Found. The developer may have moved or renamed the page.');
    hints.push('Verify the developer\'s current privacy-policy link on the App Store listing.');
  } else if (status === 405 || status === 406) {
    hints.push('The site rejected our request method or Accept header. Usually a CDN quirk — try again after a minute.');
  } else if (status === 410) {
    hints.push('The policy URL is explicitly marked Gone by the server. The developer has retired this page — App Store listing may be out of date.');
  } else if (status === 429) {
    hints.push('Rate-limited. Wait a few minutes and re-sync, or stagger bulk syncs with a longer delay.');
  } else if (status === 451) {
    hints.push('Content blocked for legal reasons in this jurisdiction. Try fetching from a different region.');
  } else if (status === 500 || status === 502 || status === 503 || status === 504) {
    hints.push('Upstream server is unhealthy. Usually transient — retry in 5–10 minutes.');
  } else if (status >= 500) {
    hints.push('Upstream server error. Usually transient.');
  } else if (status >= 400) {
    hints.push(`Server returned HTTP ${status}. Verify the URL still works in a real browser.`);
  }
  return hints;
}
...
function classifyNetworkError(error: unknown): {
  ...
  if (/timeout|ETIMEDOUT|aborted/i.test(message)) {
    hints.push('Server took too long to respond — retry later, or the site may block server traffic entirely.');
    return { networkHint: 'timeout', troubleshoot: hints };
  }
  if (/ENOTFOUND|EAI_AGAIN/i.test(message)) {
    hints.push('Hostname did not resolve. Verify the policy URL is still live on the developer\'s site.');
    return { networkHint: 'dns', troubleshoot: hints };
  }
  if (/ECONNRESET|ECONNREFUSED|socket hang up/i.test(message)) {
    hints.push('Connection was reset mid-request. The site may be rate-limiting or require a proxy.');
    return { networkHint: 'connection_reset', troubleshoot: hints };
  }
  if (/fetch failed|network/i.test(message)) {
    hints.push('Generic network failure. Check the container has outbound internet access.');
...
const KNOWN_LANGUAGE_CODES = new Set([
  ...
  'gu', 'he', 'hi', 'hr', 'hu', 'hy', 'id', 'is', 'it', 'iw', 'ja', 'ka',
  'kk', 'km', 'kn', 'ko', 'ky', 'lo', 'lt', 'lv', 'mk', 'ml', 'mn', 'mr',
  'ms', 'my', 'ne', 'nl', 'nn', 'no', 'pa', 'pl', 'ps', 'pt', 'ro', 'ru',
...
function normalizePolicyUrlLanguage(urlString: string): string {
  ...
  for (let i = 0; i < segments.length; i += 1) {
    ...
    if (!KNOWN_LANGUAGE_CODES.has(base)) continue;
    if (base === PREFERRED_POLICY_LANGUAGE) continue;
  ...
  for (const key of LOCALE_QUERY_KEYS) {
    ...
    if (!KNOWN_LANGUAGE_CODES.has(primary)) continue;
    if (primary === PREFERRED_POLICY_LANGUAGE) continue;
...
const POLICY_TOPIC_GUIDES: Array<{ key: PolicyLensKey; label: string; keywords: string[] }> = [
  { key: 'collection_scope', label: 'Collection Scope', keywords: ['collect', 'information we collect', 'personal information', 'device information', 'usage information', 'automatically collect'] },
  { key: 'product_use', label: 'Product Use', keywords: ['use your information', 'provide', 'operate', 'improve', 'personalize', 'security', 'support'] },
  { key: 'ads_marketing', label: 'Ads & Marketing', keywords: ['advertising', 'marketing', 'promotional', 'remarketing', 'newsletter', 'interest-based'] },
  { key: 'third_party_sharing', label: 'Third-Party Sharing', keywords: ['share', 'disclose', 'service providers', 'vendors', 'partners', 'affiliates', 'law enforcement'] },
  { key: 'tracking_analytics', label: 'Tracking & Analytics', keywords: ['analytics', 'cookies', 'sdk', 'tracking', 'identifier', 'advertising id', 'pixel'] },
  { key: 'user_controls', label: 'User Controls', keywords: ['access', 'delete', 'deletion', 'opt out', 'opt-out', 'choices', 'rights', 'request'] },
  { key: 'data_retention', label: 'Data Retention', keywords: ['retain', 'retention', 'store your information', 'keep your information', 'as long as necessary'] },
  { key: 'children_minors', label: 'Children & Minors', keywords: ['children', 'child', 'under 13', 'under 16', 'minor', 'age'] },
];
...
export async function syncPrivacyPolicyAnalysis(
  ...
  if (!request.policyUrl) {
    db.prepare('DELETE FROM privacy_policy_analyses WHERE app_id = ?').run(request.appId);
    return null;
  }
  ...
  try {
    ...
    if (phase === 'fetch') {
      result = await fetchAndStorePolicySource(request, logger, { forceResummarise, bypassThrottle });
    } else if (phase === 'summarise') {
      result = await summariseStoredPolicy(request, logger, { forceResummarise });
    } else {
      ...
      const afterFetch = await fetchAndStorePolicySource(request, logger, { forceResummarise, bypassThrottle });
      if (!afterFetch) {
        result = afterFetch;
    ...
    if (!result) {
      ...
      activityStatus = 'ok';
      summaryLine = phase === 'fetch' ? 'Policy source fetched (cached)' : 'Policy summary ready';
    } else if (resultStatus === 'source_ready') {
      activityStatus = 'ok';
      summaryLine = 'Policy source fetched';
    } else if (resultStatus === 'fetch_error') {
      activityStatus = 'error';
      summaryLine = result.error ? `Fetch failed: ${result.error}`.slice(0, 200) : 'Policy fetch failed';
    } else if (resultStatus === 'analysis_error') {
      activityStatus = 'error';
      summaryLine = result.error ? `Summary failed: ${result.error}`.slice(0, 200) : 'Policy summary failed';
    } else if (resultStatus === 'too_short' || resultStatus === 'unsupported_content_type') {
    ...
    const fetchDiagnostics =
      activityStatus === 'error' ? consumeFetchDiagnostics(request.appId) : null;
    // Even on success, make sure the stash doesn't linger for the next run.
    if (activityStatus !== 'error') consumeFetchDiagnostics(request.appId);
...
async function fetchAndStorePolicySource(
  ...
  const throttleEnabled = getSetting('policy_scrape_throttle_enabled', 'true') !== 'false';
  const throttleMinutesRaw = parseInt(getSetting('policy_scrape_throttle_minutes', '60'), 10);
  const throttleMinutes = Number.isFinite(throttleMinutesRaw) && throttleMinutesRaw > 0 ? throttleMinutesRaw : 0;
  if (
    !bypassThrottle &&
    throttleEnabled &&
    throttleMinutes > 0 &&
    ...
    if (elapsedMs >= 0 && elapsedMs < windowMs) {
      ...
      const remainingMin = Math.max(1, Math.ceil((windowMs - elapsedMs) / 60_000));
  ...
  try {
    ...
    if (error instanceof PolicyFetchError) {
      stashFetchDiagnostics(appId, { ...error.diagnostics, requestedUrl: error.diagnostics.requestedUrl ?? policyUrl });
    } else {
    ...
    try {
      ...
      logger.event('changelog', { note: 'Recorded failed rescrape in History.' });
    } catch (logError) {
  ...
  if (source.status !== 'ready') {
    ...
    try {
      ...
      logger.event('changelog', { note: `Recorded unusable rescrape (${source.status}) in History.` });
    } catch (logError) {
      logger.event('changelog', { error: getErrorMessage(logError) });
    }
    return hydratePolicyAnalysis(row);
  ...
  const hadPriorVersion = hasAnyPolicyVersion(appId) || !!existing?.content_hash;
  ...
  if (
    ...
    try {
      const seedAt =
        (typeof existing.source_fetched_at === 'number' && existing.source_fetched_at > 0
          ? existing.source_fetched_at
  ...
  if (versionId) {
    ...
    try {
      ...
      if (existing?.url) {
        ...
        logger.event('archive-existing', { note: 'No existing Wayback snapshot found.' });
      }
    } catch (error) {
    ...
    void (async () => {
      try {
        ...
        if (fresh.ok && fresh.snapshot.url) {
          setPolicyVersionArchiveUrl(capturedVersionId, fresh.snapshot.url, Date.now());
        }
      } catch {
  ...
  try {
    ...
    logger.event('changelog', { note: `Recorded policy ${policyEvent} event in History.` });
  } catch (error) {
  ...
  if (!forceResummarise && existing?.content_hash === contentHash && existing.summary_json) {
    logger.event('cache-hit', { note: 'Policy text is unchanged since the last summary.' });
    const row = persistPolicyAnalysis({
      appId,
      policyUrl,
      status: 'ready',
...
async function summariseStoredPolicy(
  ...
  if (!existing) {
    ...
    logger.event('restart', { note: 'No stored source; running full fetch + summarise.' });
    return syncPrivacyPolicyAnalysis(request, { phase: 'all', phaseStream: undefined });
  }
  ...
  if (!forceResummarise && existing.status === 'ready' && existing.summary_json && existing.source_text) {
  ...
  if (!aiConfig) {
    logger.event('needs-config', { note: 'No AI provider configured in Settings.' });
    const row = persistPolicyAnalysis({
      appId,
      policyUrl,
      ...
      previousSummaryAt: existing.previous_summary_at ?? null,
      model: null,
      error: 'Configure an AI provider in Settings to enable privacy-policy summaries.',
      updatedAt: now,
      lastRunLogJson: logger.toJson(),
    });
    return hydratePolicyAnalysis(row);
  ...
  try {
    ...
    try {
      const focusMod = require('./feature-flag-storage') as typeof import('./feature-flag-storage');
    ...
    logger.startPhase('summarising', `Using ${aiConfig.label} (${aiConfig.model}).`);
    ...
    const previousSummaryAt =
      existing.previous_summary_at ?? (existing.summary_json ? existing.updated_at : null);
...
function finishAiDebugCapture(
  ...
  if (!capture.enabled || !capture.id || !capture.createdAt) return;
...
function traceEvent(
  ...
  if (opts.error) {
    console.warn(`[policy] ${phase}: ${opts.error}${opts.note ? ` (${opts.note})` : ''}`);
  } else if (opts.note) {
...
async function fetchPolicyRaw(
  ...
  if (normalized !== policyUrl) {
    ...
    try {
      return await fetchPolicyRawAttempt(maybePinGoogleLocale(normalized, logger), logger);
    } catch (err) {
...
async function fetchPolicyRawAttempt(
  ...
  if (!verdict.ok) {
    throw new Error(`Refusing to fetch policy URL: ${verdict.error} (${verdict.detail ?? policyUrl})`);
  }

  // Tier 1: direct fetch with the Safari UA (what the scraper uses for apple.com).
  traceEvent(logger, 'fetch:direct', { note: `GET ${safeUrlLabel(policyUrl)}` });
  try {
    ...
    if (direct.ok) {
      return { res: wrapResponse(direct, directBody), origin: 'direct', fetchedUrl: finalUrl };
    }
    if (!HTTP_BLOCK_CODES.has(direct.status)) {
      throw new PolicyFetchError(`HTTP ${direct.status} fetching privacy policy`, {
    ...
    if (!isRetryableFetchError(error)) {
      traceEvent(logger, 'fetch:direct-error', { error: getErrorMessage(error) });
    ...
    traceEvent(logger, 'fetch:direct-retryable', { note: getErrorMessage(error) });
  }

  // Tier 2: retry with a Chrome-desktop header bundle + App Store referer.
  traceEvent(logger, 'fetch:browser-retry', { note: 'Retrying with Chrome-desktop headers.' });
  try {
    const { response: retried, body: retriedBody } = await safeFetch(policyUrl, {
    ...
    if (retried.ok) {
      return { res: wrapResponse(retried, retriedBody), origin: 'browser_retry', fetchedUrl: finalUrl };
    }
  } catch (error) {
    if (!isRetryableFetchError(error)) {
      traceEvent(logger, 'fetch:browser-retry-error', { error: getErrorMessage(error) });
      if (error instanceof PolicyFetchError) throw error;
      const { networkHint, troubleshoot } = classifyNetworkError(error);
      throw new PolicyFetchError(getErrorMessage(error), {
        requestedUrl: policyUrl,
    ...
    traceEvent(logger, 'fetch:browser-retryable', { note: getErrorMessage(error) });
  }

  // Tier 3: fall back to the Internet Archive Wayback Machine's most recent snapshot.
  traceEvent(logger, 'fetch:wayback', { note: 'Direct + browser retry both blocked; resolving Wayback snapshot.' });
  const waybackUrl = await resolveWaybackUrl(policyUrl);
  if (!waybackUrl) {
    traceEvent(logger, 'fetch:wayback-miss', { error: 'No Wayback snapshot available.' });
  ...
  traceEvent(logger, 'fetch:wayback-snapshot', { note: safeUrlLabel(waybackUrl) });

  const { response: waybackRes, body: waybackBody } = await safeFetch(waybackUrl, {
  ...
  if (!waybackRes.ok) {
    traceEvent(logger, 'fetch:wayback-error', { error: `HTTP ${waybackRes.status}` });
    throw new PolicyFetchError(`Wayback fetch failed (HTTP ${waybackRes.status}) for ${policyUrl}`, {
...
export async function fetchPrivacyPolicySource(
  ...
  if (contentType.includes('text/plain')) {
    ...
    traceEvent(logger, 'fetch:plain-text', { note: `${text.length.toLocaleString()} chars, no HTML follow-up.` });
    return validateSource({ title: titleFromUrl, contentType, text, origin, finalUrl: fetchedUrl });
  }
  ...
  if (consentRewrite) {
    ...
    try {
      ...
      const bypassType = (bypass.res.headers.get('content-type') ?? '').toLowerCase();
      if (bypassType.includes('text/html') || bypassType.includes('application/xhtml+xml') || bypassType === '') {
        res = bypass.res;
      ...
      traceEvent(logger, 'fetch:consent-bypass', { error: getErrorMessage(err) });
    }
  }
  ...
  for (let hop = 0; hop < MAX_META_HOPS; hop += 1) {
    ...
    if (!redirect || visited.has(redirect.target)) break;
    ...
    try {
      ...
      const nextType = (next.res.headers.get('content-type') ?? '').toLowerCase();
      // If the redirected page is no longer HTML, bail and keep the last HTML we had.
      if (!nextType.includes('text/html') && !nextType.includes('application/xhtml+xml') && nextType !== '') {
      ...
      traceEvent(logger, 'fetch:redirect-failed', { error: getErrorMessage(err) });
  ...
  const enriched = await maybeFollowPolicyLink({ html, baseUrl: fetchedUrl, currentText: text, logger });
  if (enriched !== null) {
 ...
 */
function extractScriptLocationTarget(html: string, baseUrl: string): string | null {
...
function extractHtmlRedirectTarget(
  ...
  if (meta) return { kind: 'meta-refresh', target: meta };
  const script = extractScriptLocationTarget(html, baseUrl);
  if (script) return { kind: 'js-redirect', target: script };
  return null;
}
 ...
 */
function detectGoogleConsentHandoff(html: string, fetchedUrl: string): string | null {
  let parsed: URL;
  try {
  ...
  const host = parsed.hostname.toLowerCase();
  const isConsentHost = host === 'consent.google.com' || host.endsWith('.consent.google.com');
  ...
  if (!isConsentHost && !looksLikeConsentMarkup) return null;
...
function validateSource({
  ...
  if (wordCount < POLICY_MIN_WORDS || text.length < POLICY_MIN_CHARS) {
    return {
      ...
      origin,
      finalUrl,
      error: 'The fetched privacy-policy text was too short to summarize reliably.',
    };
  }
  ...
  if (topicHits < POLICY_MIN_TOPIC_HITS) {
    return {
      ...
      origin,
      finalUrl,
      error: 'Source page does not look like a privacy policy (no privacy clauses found in the extracted text).',
    };
  }
...
async function maybeFollowPolicyLink({
  ...
  if (currentText.length >= POLICY_MIN_CHARS) return null;
  ...
  try {
    ...
    if (!followVerdict.ok) {
      traceEvent(logger, 'fetch:follow-link-rejected', { error: followVerdict.error ?? 'URL validation failed' });
      return null;
    }
    ...
    if (!res.ok) {
      traceEvent(logger, 'fetch:follow-link-http', { error: `HTTP ${res.status}` });
      return null;
    }

    const nextHtml = bodyBuf.toString('utf8');
    const { text } = extractPolicyTextFromHtml(nextHtml, safeUrlLabel(href));
    if (text.length > currentText.length) return text;
    ...
    traceEvent(logger, 'fetch:follow-link-error', { error: getErrorMessage(err) });
    return null;
  }
...
const CHROME_CLASS_PATTERN = /(cookie|consent|banner|navbar|nav-|menu|footer|subscribe|signup|breadcrumb|hero-|cta-|sidebar|social|related|share|toolbar|modal|popup)/i;
...
function stripChromeTags(html: string): string {
  return html
    ...
    .replace(/<[^>]+\srole="(navigation|banner|contentinfo|complementary|search)"[^>]*>[\s\S]*?<\/[^>]+>/gi, ' ')
    .replace(
      /<(div|section|aside|header|footer|ul|ol)\b[^>]*\sclass="[^"]*"[^>]*>[\s\S]*?<\/\1>/gi,
      (full) => {
        const classMatch = full.match(/\sclass="([^"]*)"/i);
        if (classMatch && CHROME_CLASS_PATTERN.test(classMatch[1])) return ' ';
        return full;
      },
    );
}
...
function htmlBlockToText(html: string): string {
  const stripped = html
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/<\/(p|div|li|section|article|main|header|h[1-6]|tr|td|blockquote|ul|ol)>/gi, '\n')
    .replace(/<(p|div|li|section|article|main|header|h[1-6]|tr|td|blockquote|ul|ol)[^>]*>/gi, '\n')
    .replace(/<[^>]+>/g, ' ');
...
function extractPolicyTextFromHtml(html: string, fallbackTitle: string) {
  ...
  while ((match = containerRegex.exec(html)) !== null) {
    ...
    if (CHROME_CLASS_PATTERN.test(attrValue)) continue;
    const innerText = htmlBlockToText(stripChromeTags(match[3]));
    if (innerText.length >= POLICY_MIN_CHARS) candidates.push(innerText);
  }
 ...
 */
function wrapUntrusted(kind: string, raw: string): { block: string; nonce: string } {
  ...
  const cleaned = typeof raw === 'string' ? raw.replace(/\r/g, '') : String(raw ?? '');
...
async function buildPolicySummary({
  ...
  logger.event('ai-chunked', { note: `Splitting source into ${chunks.length} chunks.` });
  ...
  for (let index = chunkNotes.length; index < chunks.length; index += 1) {
    logger.startPhase(`chunk-${index + 1}`, `Summarising chunk ${index + 1} of ${chunks.length}.`);
...
function buildDirectPolicySummaryPrompt({
  ...
  const { block: devBlock } = wrapUntrusted('DEVELOPER', developer || 'Unknown developer');
...
export async function summarizeSamplePrivacyPolicy({
  ...
  logger.startPhase('sample-summary', 'Summarising the built-in sample privacy policy.');
  try {
    const { summary, mode } = await buildPolicySummary({
      aiConfig,
...
async function summarizePolicyChunk({
  ...
  const summary = cleanSentence(result?.summary) || 'This chunk did not add clear privacy-practice details.';
  const highlights = Array.isArray(result?.highlights)
    ? result.highlights.map((item: unknown) => cleanSentence(item)).filter(Boolean).slice(0, 6)
    : [];

  return {
    summary,
    highlights: highlights.length > 0 ? highlights : ['No clearly extractable customer-data practice was stated in this chunk.'],
  };
}
...
async function summarizePolicyFromChunkNotes({
  ...
  const result = await callAiJson<any>({
    ...
    prompt: (() => {
      ...
      // same quarantine.
      const { block: notesBlock } = wrapUntrusted('CHUNK_NOTES', synthesizedNotes);
      return [
        UNTRUSTED_INPUT_PREAMBLE,
        '',
        `App name (untrusted): ${appBlock}`,
 ...
 */
async function readBoundedResponseText(res: Response, maxBytes: number = AI_RESPONSE_MAX_BYTES): Promise<string> {
  if (!res.body) {
    return '';
  }
...
async function readStreamingChatCompletion(
  ...
  if (!res.body) return { rawBody: '', content: '' };
  ...
  const consumeEvent = (dataLine: string) => {
    ...
    try {
      ...
      if (typeof full === 'string' && !delta) content += full;
    } catch {
  ...
  try {
    ...
    if (buffer.trim()) {
      for (const line of buffer.split('\n')) {
        if (line.startsWith('data:')) consumeEvent(line.slice(5));
      }
    }
 ...
 */
function resolveTimeoutForPhase(aiConfig: AiRuntimeConfig, phase: AiTimeoutPhase): number {
...
function surfaceAiTimeout({
  ...
  try {
    ...
    console.warn('[policy] failed to record AI timeout notification:', getErrorMessage(err));
  }
...
async function callAiJson<T>({
  ...
  const invoke = (): Promise<T> => {
    const common = { aiConfig, schemaName, schema, prompt, appId, appName, phase, logger, phaseKind };
...
async function callChatCompletionsJson<T>({
  ...
  const responseFormat = aiConfig.provider === 'custom'
    ? { type: 'json_object' }
    : {
        type: 'json_schema',
        json_schema: {
  ...
  // output shape.
  const userPrompt = aiConfig.provider === 'custom'
  ...
  const endpointVerdict = validateExternalUrl(`${aiConfig.baseUrl}/chat/completions`, {
    maxLength: 1024,
  ...
  try {
    ...
    finishAiDebugCapture(debug, { response: '', durationMs: observedMs, error: message });
    logger?.event('ai-error', { error: message });
    if (abort) {
      surfaceAiTimeout({ aiConfig, phaseKind: phaseKindResolved, logger, appId, appName, observedMs });
    }
  ...
  if (!res.ok) {
    ...
    finishAiDebugCapture(debug, { response: body, durationMs: Date.now() - started, error: message });
    logger?.event('ai-error', { error: message });
    throw new Error(message);
  }
  ...
  try {
    if (useStreaming) {
      ...
      if (refusal) {
        ...
        finishAiDebugCapture(debug, { response: rawBody, durationMs: Date.now() - started, error: message });
        logger?.event('ai-error', { error: message });
        throw new Error(message);
      }
    ...
    finishAiDebugCapture(debug, {
      response: typeof (error as any)?.partialRawBody === 'string' ? (error as any).partialRawBody : '',
  ...
  if (typeof content !== 'string' || !content.trim()) {
    ...
    finishAiDebugCapture(debug, { response: rawBody, durationMs, error: message });
    logger?.event('ai-error', { error: message });
    throw new Error(message);
  }
...
async function callAnthropicJson<T>({
  ...
  const endpointVerdict = validateExternalUrl(`${endpointBaseUrl}/v1/messages`, {
    maxLength: 1024,
    allowPrivateHosts: true,
  });
  if (!endpointVerdict.ok) {
  ...
  try {
    ...
    logger?.event('ai-error', { error: message });
    if (abort) {
      surfaceAiTimeout({ aiConfig, phaseKind, logger, appId, appName, observedMs });
    }
    throw new Error(message);
  }
  ...
  const toolUse = Array.isArray(payload?.content)
    ? payload.content.find((part: any) => part?.type === 'tool_use' && part?.name === schemaName)
    : null;
...
function getAiRuntimeConfig(): AiRuntimeConfig | null {
  ...
  const model = (getSetting('ai_model', resolveDefaultModel(provider)) || resolveDefaultModel(provider)).trim();
  const baseUrl = normalizeBaseUrl(
    getSetting('ai_base_url', resolveDefaultBaseUrl(provider)) || resolveDefaultBaseUrl(provider),
    provider,
  );
...
function normalizePolicySummary(input: any): PolicySummary {
  ...
  const byKey = new Map<PolicyLensKey, { rating: PolicyRating; summary: string }>();
  ...
  for (const entry of lensEntries) {
    ...
    const summary = cleanSentence(entry?.summary) || 'The policy does not address this clearly.';
  ...
  const lenses = POLICY_LENSES.map(({ key }) => ({
    ...
    summary: byKey.get(key)?.summary ?? 'The policy does not address this clearly.',
  }));

  const highlights = Array.isArray(input?.highlights)
    ? uniqueStrings(input.highlights.map((item: unknown) => cleanSentence(item)).filter(Boolean)).slice(0, 5)
    : [];
...
function normalizeSafetySummary(input: any): PolicySummarySafety | undefined {
  ...
  const rawConcerns = Array.isArray((input as any).concerns) ? (input as any).concerns : [];
  const concerns = uniqueStrings(
    rawConcerns.map((item: unknown) => cleanSentence(item)).filter(Boolean),
  ).slice(0, 5);
  return { paragraph: paragraph.slice(0, 1_400), concerns };
}
...
function chunkPolicyText(text: string, maxChars: number): string[] {
  ...
  for (const paragraph of paragraphs) {
    if (paragraph.length > maxChars) {
      ...
      const slices = paragraph.match(new RegExp(`[\\s\\S]{1,${Math.max(1000, maxChars - 1_000)}}(?:\\s|$)`, 'g')) ?? [paragraph];
...
function hydratePolicyAnalysis(row: PolicyAnalysisRow): AppPolicyAnalysis {
  ...
  const chunkNotes =
    storedNotes && row.chunk_notes_hash && row.chunk_notes_hash === (row.content_hash ?? '')
      ? storedNotes
      : undefined;
  ...
  return {
    ...
    previousSummary: row.previous_summary_json ? safeParseSummary(row.previous_summary_json) : null,
    previousSummaryAt: row.previous_summary_at ?? undefined,
    error: row.error ?? undefined,
    sourcePreview: row.source_text ? row.source_text.slice(0, SOURCE_PREVIEW_CHARS) : undefined,
 ...
 */
function parseStoredChunkNotes(raw: string | null | undefined): ChunkNote[] | null {
  if (!raw) return null;
  try {
    ...
    for (const entry of parsed) {
      ...
      const summary = typeof (entry as any).summary === 'string' ? (entry as any).summary : '';
 ...
 */
function persistChunkNotes(appId: string, contentHash: string, notes: ChunkNote[]): void {
  db.prepare(
...
function loadReusableChunkNotes(
  ...
  const row = getPolicyAnalysisRow(appId);
  if (!row) return null;
  if (!row.chunk_notes_json || row.chunk_notes_hash !== contentHash) return null;
  const notes = parseStoredChunkNotes(row.chunk_notes_json);
  if (!notes) return null;
  if (notes.length !== expectedChunkCount) return null;
  return notes;
}

function parseRunLog(raw: string | null | undefined): PolicyRunPhase[] | undefined {
  ...
  try {
    ...
    return parsed
      .map((entry: any) => {
        ...
        if (typeof entry.error === 'string') result.error = entry.error;
        if (typeof entry.ms === 'number' && Number.isFinite(entry.ms)) result.ms = entry.ms;
        return result;
      })
...
export class PolicyRunLogger {
  ...
  startPhase(phase: string, note?: string): void {
    ...
    if (this.currentIdx >= 0) {
      ...
      if (!stale.note) stale.note = 'incomplete';
      this.currentIdx = -1;
    }
    const at = Date.now();
 ...
 */
export function markPolicyRunStart(appId: string, now: number = Date.now()): void {
  const existing = getPolicyAnalysisRow(appId);
  if (existing) {
    db.prepare(
      `UPDATE privacy_policy_analyses
...
function persistPolicyAnalysis(input: PersistPolicyAnalysisInput): PolicyAnalysisRow {
  ...
  const preservedLog = input.lastRunLogJson === undefined
    ? existing?.last_run_log ?? null
    : input.lastRunLogJson;
  const preservedFetchedAt = input.sourceFetchedAt === undefined
  ...
  if (!row) throw new Error('Failed to persist privacy policy analysis');
  return row;
}

function getPolicyAnalysisRow(appId: string): PolicyAnalysisRow | null {
  return (db.prepare('SELECT * FROM privacy_policy_analyses WHERE app_id = ?').get(appId) as PolicyAnalysisRow | undefined) ?? null;
}
 ...
 * doesn't reject them.
 */
function finalSummarySchema(audience: 'self' | 'loved_one' | 'guardian' = 'self') {
...
function resolvePolicyLengthConfig(aiConfig: AiRuntimeConfig): PolicyLengthConfig {
  if (providerLikelyNeedsChunking(aiConfig.provider, aiConfig.model)) {
    return {
      maxDirectChars: 8_000,
      maxChunkChars: 4_000,
    };
  }
...
function normalizeStatus(status: string | null | undefined): PolicyAnalysisStatus | null {
  return POLICY_ANALYSIS_STATUSES.find(value => value === status) ?? null;
}

function normalizeAnalysisMode(mode: string | null | undefined): PolicyAnalysisMode | null {
  if (mode === 'direct' || mode === 'chunked') return mode;
  return null;
}
...
function normalizeExternalReferences(value: unknown): ExternalPolicyReference[] {
  ...
  return value
    .map(item => {
      ...
      if ((source !== 'privacyspy' && source !== 'tosdr') || !label || !url || !summary) return null;
      return { source, label, url, summary, ...(scoreLabel ? { scoreLabel } : {}) } as ExternalPolicyReference;
    })
...
function buildPolicyClueDigest(text: string): string {
  const sections = POLICY_TOPIC_GUIDES.map(guide => {
    ...
    return [guide.label + ':', ...snippets.map(snippet => `- ${snippet}`)].join('\n');
  });

  return sections.join('\n\n');
}

function collectSnippetsForKeywords(text: string, keywords: string[]): string[] {
...
function countPolicyTopicHits(text: string): number {
  ...
  for (const guide of POLICY_TOPIC_GUIDES) {
    if (guide.keywords.some(keyword => lower.includes(keyword.toLowerCase()))) {
      groupHits += 1;
    }
  }
...
function normalizeSourceOrigin(value: string | null | undefined): PolicySourceOrigin | null {
...
function jsonSkeletonForSchema(schema: Record<string, unknown>): unknown {
  ...
  if (type === 'array') {
    ...
    return Array(Math.max(1, minItems)).fill(sample);
  }
