...
export function snapshotDatabaseHealth(): DatabaseHealthSnapshot {
  ...
  const dbPath = (db as unknown as { name: string }).name ?? join(process.cwd(), 'data', 'privacy.db');
  ...
  const walAutocheckpoint = Number(readPragma<number>('wal_autocheckpoint') ?? 0);

  const utilisationPct =
    pageCount > 0 ? Math.round(((pageCount - freelistCount) / pageCount) * 100) : 0;
 ...
 */
export function runIntegrityCheck(): NonNullable<DatabaseHealthSnapshot['integrityCheck']> {
  ...
  try {
    ...
    const messages = rows.map(r => String(r.integrity_check ?? r['integrity_check'] ?? '')).filter(Boolean);
    if (messages.length === 0 || (messages.length === 1 && messages[0] === 'ok')) {
      status = 'ok';
    } else {
      status = 'error';
