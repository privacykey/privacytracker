...
export const PROFILE_PRESET_META: Record<ProfilePresetKey, ProfilePresetMeta> =
  {
    strict: {
      ...
      shortLabel: "Strict",
      description:
        "Restrict identity-linked data wherever possible. Many mainstream apps will mismatch.",
      icon: "🛡️",
      severityCls: "severity-none",
    },
    balanced: {
      ...
      shortLabel: "Balanced",
      description:
        "A sensible default. Strict on health, sensitive, and contacts; lenient on usage and diagnostics.",
      icon: "⚖️",
      severityCls: "severity-unlinked",
    },
    anti_tracking: {
      ...
      description:
        "Only flag apps that share data with third-party trackers. Everything else is fine.",
      icon: "🚫",
      severityCls: "severity-linked",
    },
    permissive: {
      ...
      description:
        "Accept almost anything, but still flag tracking on health, financial, location, and sensitive data.",
      icon: "🌤️",
      severityCls: "severity-track",
    },
  };
...
export function describePresetTransition(
  ...
  const oldHasAny =
    !!oldProfile &&
    Object.values(oldProfile).some((v) => typeof v === "string");
  const newHasAny =
    !!newProfile &&
    Object.values(newProfile).some((v) => typeof v === "string");
  ...
  if (oldHasAny && !newHasAny) {
    return {
      ...
      detail: {
        from: matchPreset(oldProfile ?? null),
        to: null,
        cleared: true,
      },
    };
...
export function matchPreset(
  ...
  const profileKeys = Object.keys(profile).filter(
    (k) => typeof profile[k] === "string"
  );
  if (profileKeys.length !== PROFILE_CATEGORY_KEYS.length) {
    return null;
  }
...
export interface AppProfileFootprint {
   ...
   */
  worstByCategory: Partial<
    Record<string, Exclude<ProfileTier, "not_collected">>
  >;
}
 ...
 */
export function describeWorstMismatch(
...
export function parseStoredProfile(
  ...
  for (const [key, value] of Object.entries(
    ...
    if (!Object.hasOwn(CATEGORY_META, key)) {
      continue;
    }
...
export function summariseBadge(result: ProfileMismatchResult): AppProfileBadge {
  ...
  if (result.count === 0) {
    return {
      ...
      description:
        "Every category this app collects stays within your preferences.",
      worstCategory: null,
      worstCategoryLabel: null,
    };
  }
