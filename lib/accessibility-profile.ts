...
export const A11Y_PREFERENCE_META: Record<AccessibilityPreference, A11yPreferenceMeta> = {
  required: {
    value: 'required',
    label: 'Required',
    shortLabel: 'Required',
  ...
  nice: {
    ...
    label: 'Nice to have',
    shortLabel: 'Nice',
    description: "I'd prefer apps that declare this, but it's not a deal-breaker.",
...
export type AccessibilityProfile = Partial<Record<string, AccessibilityPreference>>;
...
export function computeA11yMismatch(
  ...
  if (!profile) {
    return { missing: [], count: 0, totalGap: 0, profileActive: false, missingRequired: 0 };
  }
  const preferences = Object.entries(profile).filter(
    (entry): entry is [string, AccessibilityPreference] => typeof entry[1] === 'string',
  );
  ...
  for (const [feature, preference] of preferences) {
    if (footprint.declared.has(feature)) continue;
    missing.push({ feature, preference });
    totalGap += A11Y_PREFERENCE_WEIGHT[preference];
    if (preference === 'required') missingRequired += 1;
  }

  // Sort required-first, then alphabetical for stability.
  missing.sort((a, b) => {
    const weightDelta = A11Y_PREFERENCE_WEIGHT[b.preference] - A11Y_PREFERENCE_WEIGHT[a.preference];
 ...
 *     the total
 */
export type A11yBadgeKind = 'no_profile' | 'match' | 'missing_required' | 'missing_nice';
...
export function summariseA11yBadge(result: A11yMismatchResult): A11yProfileBadge {
  ...
  if (result.count === 0) {
    return {
      ...
      kind: 'match',
      label: 'A11y match',
      description: 'This app declares every accessibility feature you asked for.',
      worstFeature: null,
    };
  }
...
export function parseStoredA11yProfile(
  ...
  for (const [key, value] of Object.entries(parsed as Record<string, unknown>)) {
    if (!FEATURE_KEY_SET.has(key)) continue;
    if (typeof value !== 'string' || !PREFERENCE_SET.has(value)) continue;
