...
/**
 * The socket peer, for the admin-token guess limits (lib/admin-token-guard.ts).
 * Next hands the proxy and routes a copy of the headers but not the socket,
 * so the peer travels as a header. A client can send any header, so any
 * copy it sent is removed first, and the value carries a random stamp made
 * once per process: the guard believes the header only when the stamp
 * matches, so a server started without this preloader ignores a peer a
 * client claims instead of trusting it.
 */
const PEER_HEADER = "x-privacytracker-peer";
const PEER_STAMP = Symbol.for("privacytracker.peer-stamp");
globalThis[PEER_STAMP] ??= randomBytes(16).toString("hex");
...
function stampPeer(request) {
  ...
  if (Array.isArray(raw)) {
    for (let i = raw.length - 2; i >= 0; i -= 2) {
      if (String(raw[i]).toLowerCase() === PEER_HEADER) {
        raw.splice(i, 2);
      }
    }
  }
  ...
  if (peer) {
    request.headers[PEER_HEADER] = `${globalThis[PEER_STAMP]} ${peer}`;
  }
}
...
module.exports = { bodyLimit, guardRequest, PEER_HEADER, stampPeer };
