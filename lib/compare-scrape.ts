 ...
 * same HTML.
 */
import { PrivacyTypeSnapshot } from './changelog';
import { safeFetch, validateAppStoreUrl, sanitizePolicyUrl } from './security';
 ...
 */
export async function fetchAndParsePreview(url: string): Promise<ComparePreview> {
  const verdict = validateAppStoreUrl(url);
  if (!verdict.ok || !verdict.url) {
    throw new Error(`Rejected URL (${verdict.error ?? 'invalid_url'}): ${verdict.detail ?? url}`);
  }
  ...
  if (ariaMatch) {
    ...
    if (sectionMatch) privacyPolicyUrl = sectionMatch[1];
  }
  ...
  if (jsonTitle) name = jsonTitle.trim();
  ...
  const accessibilityFeatures: AccessibilityFeatureRecord[] = rawAccessibility ?? [];
        ...
        for (const shelf of viaHeader) {
          ...
          for (const item of (shelf.items ?? [])) {
            if (item.categories?.length) {
              ...
              privacyItems.push({ ...item, categories: [...catMap.values()], purposes: [] });
            }
          }
        }
      }
