...
export type WebhookFrequency = 'immediate' | 'daily_summary' | 'weekly_summary' | 'off';

const VALID_FORMATS: readonly WebhookFormat[] = ['slack', 'discord', 'teams', 'generic'];
const VALID_FREQUENCIES: readonly WebhookFrequency[] = ['immediate', 'daily_summary', 'weekly_summary', 'off'];
 ...
 */
export async function postImmediateWebhook(notification: WebhookNotification): Promise<void> {
  const cfg = readWebhookConfig();
  if (!cfg || cfg.frequency !== 'immediate') return;
  const title = notification.appName
    ? `📱 ${notification.appName}: ${notification.summary}`
    : `📱 ${notification.summary}`;
  await postWebhook(cfg, title, [notification.summary], [notification]).catch(err => {
...
export async function maybePostSummaryWebhook(): Promise<number> {
  const cfg = readWebhookConfig();
  if (!cfg) return 0;
  if (cfg.frequency !== 'daily_summary' && cfg.frequency !== 'weekly_summary') return 0;

  const intervalMs = cfg.frequency === 'daily_summary'
    ? 24 * 60 * 60 * 1000
    : 7 * 24 * 60 * 60 * 1000;
  const now = Date.now();
  const lastSent = Number(getSetting('notification_webhook_last_sent', '0')) || 0;
  if (now - lastSent < intervalMs) return 0;
  ...
  const title = cfg.frequency === 'daily_summary'
  ...
  const lines = rows.map(r => r.app_name
...
async function postWebhook(
  ...
  if (!verdict.ok || !verdict.url) {
    throw new Error(`Blocked webhook URL: ${verdict.error ?? 'invalid_url'} — ${verdict.detail ?? cfg.url}`);
  }
  const body = JSON.stringify(buildPayload(cfg.format, title, lines, notifications));
