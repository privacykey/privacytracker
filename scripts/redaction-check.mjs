...
const { exportBackup, SENSITIVE_SETTING_KEYS, redactSensitiveSettingsRows } =
...
console.log(
  "Stored ai_api_key reads back as:",
  getSetting("ai_api_key", "") === sentinel ? "sentinel ✓" : "WRONG ✗"
);
...
console.log(
  "Sentinel in dump?",
  serialised.includes(sentinel) ? "YES (LEAK ✗)" : "no ✓"
);
...
console.log(
  "  ai_api_key.value =",
  JSON.stringify(scrubbed[0].value),
  scrubbed[0].value === "" ? "✓" : "✗"
);
console.log(
  "  ai_provider.value =",
  JSON.stringify(scrubbed[1].value),
  scrubbed[1].value === "openai" ? "✓ (preserved)" : "✗"
);
console.log(
  "  original input mutated?",
  synthetic[0].value === "should-be-scrubbed" ? "no ✓" : "YES ✗"
);

if (serialised.includes(sentinel) || !redactedOk) {
  process.exit(1);
}
process.exit(0);
