...
// Screenshot import's OCR worker, engine and model live in public/ocr/,
// generated by scripts/stage-ocr-assets.mjs as the first step of
// `pnpm build:standalone`. A tree built without that step would ship with
// screenshot import unable to start, so refuse it here.
if (!existsSync(path.join(stagedPublic, "ocr", "worker.min.js"))) {
  console.error(
    "stage-standalone: public/ocr/ is missing. Run `pnpm build:standalone`, which stages the OCR assets before `next build`."
  );
  process.exit(1);
}
