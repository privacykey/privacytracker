...
**Deliberately NOT scoped:** audit-bundle export (`/api/export`). An export is a record of the install, and the recipient of a bundle has no way to tell a scoped export from a complete one — per-device export should be an explicit choice in the export UI, never a side effect of a nav setting. Bulk actions that *are* scoped say so: "Sync All" renders as "Sync {device}" when a scope is active, because the server pages only in-scope apps and a button claiming "All" would be lying.
