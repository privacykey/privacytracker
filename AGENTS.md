...
- **The desktop shell has a second backend, behind a cargo feature.** By default it spawns the Node sidecar, as it always has. Built with `--features rust-backend` (Phase 6, batch 4a) it serves the app from its own process with the Rust core instead: `just tauri-dev-rust`, which needs no `fetch-node-sidecar` and no standalone tarball, only a `pnpm build` for the frontend it serves. `src-tauri/src/backend.rs` is what the rest of the shell talks to; `sidecar.rs` and `embedded.rs` are the two implementations, each compiled only on its own path. Shipped builds stay on Node until the cutover release, so the feature must stay off by default: `tests/app/rust-core-inert.test.ts` requires the optional dependency, the gated module declaration in `main.rs` and the `#![cfg(feature = "rust-backend")]` at the top of `embedded.rs`, and CI's `rust-check` compiles the shell both ways. The embedded backend also reuses the port it bound last (`<data>/.desktop-port`), which is what keeps the page's origin, and therefore its local storage, across relaunches. A packaged Rust build stages the frontend into the bundle's Resources with `scripts/stage-site.mjs` through the `src-tauri/tauri.rust.conf.json` overlay (`pnpm tauri:build:rust`), rather than extracting a tarball into the data directory, and `scripts/parity/handoff.mjs` (`just handoff`, and CI's core-parity job) proves either backend can open and serve the database the other wrote, which is what a rollback from a Rust release to a Node one depends on. Releases carry the same switch: **macOS desktop release** takes a `backend` input (`node` by default, and `Prepare verified release draft` does not pass one, so tagged releases stay on Node), `rust` builds with the overlay and `entitlements-rust.plist`, and `scripts/verify-macos-bundle.mjs <app> <arch> [backend]` checks a Rust bundle by driving the packaged app through its hidden `--smoke-server <dir>` mode. Keep the Node-only steps in that workflow gated on `env.BACKEND`: fetching the Node sidecar, the staging keychain, `build:standalone` and `STANDALONE_PRE_BUILT` all exist for the sidecar and break or waste time on a Rust build. A Rust bundle also carries its notices under `third-party/` (`scripts/stage-notices.mjs`), and the crate list they include is generated: after changing a Rust dependency run `pnpm notices:rust` and commit `src-tauri/THIRD-PARTY-RUST.md` and `lib/rust-crates.json`, or CI's `rust-check` job fails on the difference.
