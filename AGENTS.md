...
assets in a draft. Each macOS build job packs the updater archive from the
bundle it verified, signs it there and records its SHA-256 in
`build-evidence.json` (`scripts/stage-updater-archive.mjs`); assembly and the
draft check refuse a draft asset with any other digest. Never sign an asset
downloaded from the draft. macOS requires 13.5+; v0.1.2 uses the frozen legacy feed,
...
Docker (production): first configure `AUDITOR_ADMIN_TOKEN` in `.env` (see [secure deployment](docs/SECURE_DEPLOYMENT.md)), then `docker compose up --build -d`. Docker requires authentication for private pages and APIs even when host port publishing is loopback-only. By default the SQLite DB lives in a Docker-managed **named volume** (`privacytracker-data`), so data survives rebuilds and the container's non-root `audit` user (uid 100 / gid 101) can write it on any host with no setup. Back it up with `docker compose cp web:/app/data ./data-backup`. This is a change from the old `./data` bind mount, which broke on a fresh Linux host: Docker auto-creates the bind source as `root:root`, uid 100 can't create `privacy.db`, and `lib/db.ts` throws `SQLITE_CANTOPEN` (macOS Docker Desktop hid this by uid-mapping bind mounts). If you'd rather keep the DB on the host at `./data/privacy.db`, layer on `docker-compose.bind-mount.yml` after the one-time `mkdir -p data && sudo chown 100:101 data`: `docker compose -f docker-compose.yml -f docker-compose.bind-mount.yml up --build -d`. The `compose-smoke` CI job exercises both paths (and asserts `/fonts/InterVariable.woff2` + `/brand-icon.png` actually serve — the runtime image must copy `public/`). Since the Docker cutover (Phase 6, batch 6b) the image runs the Rust server, `pt-core serve --host 0.0.0.0 --site /app/site`, on Alpine: about 56 MB, with no Node, no `node_modules` and `tzdata` so a `TZ` zone name still works. The Dockerfile's `BACKEND` build argument picks the runtime stage, and `node` builds the `next start` image, kept as the rollback until v0.3.0 has shipped (`PRIVACYTRACKER_BACKEND=node` in `.env` for compose; `backend: node` in `release.yml` for published images). Both open the same volume as the same user, pinned to 100:101 in the Rust stage, and answer the same busybox `wget` healthcheck, which is why the three compose files did not change. `container-smoke` builds and boots both, checks that the Rust image carries no Node, ships its five notices under `/app/third-party/` and serves the OCR assets from `site/public/ocr/`, and logs in over TLS through the Caddy example expecting a `Secure` cookie. The core is built with `cargo auditable`, which embeds its dependency list so Trivy and the SBOM still see the Rust crates. `core/THIRD-PARTY-RUST.md` is the image's crate list, regenerated by `pnpm notices:rust` with the desktop one, and CI diffs both. Only releases reach the public image: `docker-publish.yml` called from `release.yml` on a `v*` tag publishes `ghcr.io/privacykey/privacytracker` with the version tag, plus `<major>.<minor>` and `latest` for a final release (a prerelease gets its version tag only), while a push to main publishes `edge` and `sha-<commit>` to the private `ghcr.io/privacykey/privacytracker-edge`, where `pr-preview.yml` pushes `pr-<n>` too. `scripts/docker-image.mjs` makes that choice (pinned by `tests/release/docker-image.test.mjs`). GHCR visibility is per package, not per tag, so never point main or previews at the public package.
...
6. **Notify**: when the diff found changes, the same commit inserts the bell row that the bell UI (`/api/notifications`) polls, and once the commit has landed `fireWebhookIfConfigured` (`lib/notifications.ts`) posts the immediate webhook if one is set to "Each change". Quiet hours defer the bell row (`not_before`), never the post. The daily and weekly webhook summaries read the same rows from the 30-minute tick.
7. **Summarize policy** (`lib/privacy-policy.ts`) optionally follows the developer privacy-policy link, hashes the text, and — if the hash changed and an AI provider is configured — regenerates a structured `summary_json` keyed by the lenses in `POLICY_TOPIC_GUIDES` (collection_scope, ads_marketing, third_party_sharing, etc.). Long documents are split into ~12k-char chunks for small/local models (`providerLikelyNeedsChunking`). Every fetch leaves a `privacy_snapshots` row on the History timeline (`appendPolicyChangeEntry`), but only a `changed` event may carry `changes_detected = 1` and raise a bell notification (via `createNotification`, which also feeds the webhooks), and only while `flag.notifications.types.policy_updates` is on (default off). `first`, `same` and `error` events are timeline-only, and an unusable body never replaces the stored hash or text.
...
### Global device scope ("family mode")

An install can hold apps imported from several phones and tablets (the `devices` table + the `app_devices` junction, `lib/devices.ts`). **Which of those devices the app is currently showing is a global axis**, owned by the nav's `DeviceScopePicker` and read by every surface. It sits alongside the two axes the dashboard layout already uses:

1. **Capability** — `flag.nav.device_scope` decides whether the picker exists. The picker additionally renders nothing when the install has NO devices; it deliberately still renders for a single one, because a one-device install can still choose between that device and the apps tied to none, and because silently hiding a device control reads as the feature being broken to someone who just imported. The picker **self-gates** on this via `useFlagBundle` rather than taking a prop from `Nav`: nearly every page renders `<Nav />` with no flags at all, so a prop would resolve to its `true` default almost everywhere and the flag would look wired while doing nothing.
2. **Preference** — the `DeviceScope` blob (`lib/device-scope.ts`) stored in `app_settings` under `device.scope`, deciding which devices' apps every surface counts.

`lib/device-scope.ts` mirrors `lib/dashboard-layout.ts`: pure types + `reconcileScope` / `appMatchesScope` / `toggleScopeDevice` / `describeScope` / `parseScopeParam`, no React and no SQLite, so client components and route handlers both import it. Server-only helpers are in `lib/device-scope-server.ts`; the API is `GET/PUT/DELETE /api/device-scope`, which returns `{ scope, devices }` together so the picker's checkboxes can never disagree with the scope the server just reconciled.

**Three invariants, each of which a straightforward change would break:**

- **A bare request is never scoped.** `scopeFromRequest()` reads `?devices=` and nothing else — it does NOT fall back to the stored scope. `GET /api/apps` with no params returns the whole fleet whatever the user last clicked, exactly as documented; the client provider passes the param when it wants scoping. Same opt-in posture as `?limit=`. Pinned by `tests/app/device-scope-server.test.ts` ("a STORED scope does not narrow a bare request"). `serialiseScopeParam` returns `null` for the unrestricted scope so the param is *omitted*, not sent empty.
- **Reconcile fails open, and collapses.** A subset whose ids have all been deleted becomes `SCOPE_ALL` (showing too much is one click from fixed; showing nothing looks like data loss), and a subset naming every known device *plus* unattached also collapses to `SCOPE_ALL` so a later import is included rather than silently hidden. `deviceIds` is canonicalised to the known-device order, because `scopeKey` is used as a React key and a fetch-effect dependency and an unstable one causes spurious remounts.
- **The grid is keyed on the data's scope, not the live one.** `AppsGridLoader` stores `scopeKey` *inside* `LoadedState` and passes `key={state.scopeKey}` to `<AppGrid>`. Keying on the live scope remounts the instant the user picks a device — while `state` still holds the previous scope's apps — and because AppGrid seeds all of its state at mount and ignores later props, that stale payload is what it keeps. Symptom when you get this wrong: correct cards, stale denominator ("6 of 10 apps").

**Two ways to apply a scope in SQL, not interchangeable.** `scopeSqlClause(scope, alias)` is a fragment for queries selecting from `apps` (`getAppsPage`, `countApps`, `getAllApps`, `getTriageData`, `getStats`); `scopeAppIdClause(scope, appIdExpr)` is the correlated `EXISTS` form for queries starting elsewhere (`privacy_categories`, `privacy_snapshots`, `notifications`, `accessibility_features`). `getScopedAppIds(scope)` returns a `Set` for callers filtering rows already in memory. In `getAppsPage` the filter goes **inside** the `page_apps` CTE — post-filtering would return short pages and break the grid's offset arithmetic. Both helpers interpolate their alias/expression argument, so those must always be caller-supplied literals; the device ids themselves are bound parameters.

**Deliberately NOT scoped — and the UI says so.** Every export path is whole-install: `/api/export` (CSV/JSON, linked from BOTH the Stats page and Settings → Export Data), `/api/shortlist/export`, `/api/export/audit-bundle`, and the DB backup. An export is a record of the install, and whoever opens the file cannot tell a partial one from a complete one; a file whose contents depend on a nav setting the reader can't see is worse than one that is simply complete. Per-device export, if ever wanted, belongs in the export UI as an explicit choice.

That decision is only defensible if the user is told, so `ScopeExportNote` renders "Exports cover every device, not just {scope}" beside each export control **while a scope is active**, and nothing at all otherwise — it is a reconciliation, not a permanent disclaimer. Without it the Stats page reads "3 Apps Tracked" directly above a download containing ten. If you add an export surface, add the note; `tests/e2e/device-scope.spec.ts` pins both the Stats and the Settings copies, because one surface admitting the mismatch while another stays quiet is worse than neither doing it.
...
`devices.owner_label` (free text: "Mum", "Leo") and `devices.owner_audience` (`self` | `loved_one` | `guardian`) record **whose** a device is. Both are nullable and **never inferred** — not from the device name, not from anything. A wrong guess mislabels a real person's phone in the nav and can push an unwanted focus change at them, which is worse than knowing nothing. Set via `PATCH /api/devices/[id]` (`ownerLabel` / `ownerAudience`, each independently clearable with `null`; an omitted key is left alone) and edited in Settings → Devices. `setDeviceOwner` never touches the device NAME: several devices can share one owner, which is the whole reason grouping is worth doing.

Two things read it:

- **The picker groups by owner.** `groupDevicesByOwner` matches labels case-insensitively and trimmed ("Mum" and "mum " are one person), keeps the first spelling as the display label, and puts owner-less devices in a trailing `UNASSIGNED_OWNER` group — returned as a group, not dropped, so an unlabelled device stays selectable. Headings are suppressed entirely when nothing has an owner, so a one-person install never sprouts a "whose?" label.
- **The focus-switch prompt.** `scopeOwnerAudience(scope, devices)` returns an audience ONLY when the scope unambiguously resolves to one owner who has stated one: an unrestricted scope, a mixed selection, any device with no stated owner, or a scope that also includes the unattached bucket all yield null. It drives a **suggestion**, never an automatic change — the audience decides which features the whole app shows, and flipping that because someone clicked a device filter would be a far worse surprise than the mismatch it fixes.
...
**Matching the mode is necessary, not sufficient.** For any device whose owner is not `self`, the gate also requires `devices.permission_acknowledged_at` — the user's explicit, timestamped attestation that they have the owner's permission to view and act on it. Missing, it returns `reason: 'permission_unacknowledged'` (checked after the mode match, so a mismatch is reported first). Without this, "I'm helping someone" would have unlocked deletion on any device merely *labelled* as theirs. The attestation is taken at import — Step 3's "Whose device is this?" fieldset, shown **from the second device onward** (the import will create a device row, i.e. not a re-sync, AND the install already has a real device; the seeded "Unknown device" placeholder doesn't count), pre-selected from the current focus, and blocking the import for a non-self owner until ticked — or later in Settings → Devices. **The first device of an install is not asked and is created with no owner recorded** — deliberately not the focus default, which the user never saw. It stays under the pre-ownership rule until ownership is set in Settings → Devices, which the review page's audience banner now points at. It is stored only for non-self owners, cleared automatically if the owner is set back to `self` or unset (a stale "yes" must not survive a change of owner), and every tick is audit-logged as `devices.permission_acknowledged`. `acknowledgeNoBackup` does not stand in for it: that flag is a statement about risk to data, not about permission.

**This was a deliberate expansion, approved as a product decision** (the delete flow is meant to serve both the user and the person they are helping). Deleting apps off another person's device is now reachable — but only after the user explicitly records that the device is theirs, explicitly attests that they have that person's permission, AND explicitly switches into the matching mode, and still only through the rest of the chain: the device physically connected, unlocked and trusting this Mac, `flag.devopts.cfgutil_uninstall` on, a fresh verified backup, and a native Touch ID prompt per app inside `run_cfgutil_remove_app`. **The gate stops mistakes; Touch ID is what stops a compromised webview.** `acknowledgeNoBackup` relaxes the BACKUP requirement only and must never unlock ownership or the flag — pinned by test.
...
`lib/historical-import.ts` back-fills privacy-label history from archive.org. The earliest target it will probe is **2021-02-01** (Q1 2021), exposed as `APP_STORE_HISTORICAL_FLOOR` and aliased as `APP_STORE_WEB_LAUNCH` for back-compat. The floor was originally 2025-11-05 because the live scraper only knew the modern `<script id="serialized-server-data">` blob Apple introduced with the Nov 2025 redesign; pushing it back to Q1 2021 became safe once the scraper grew an `extractFromShoebox` fallback that parses Apple's older Ember/FastBoot site (privacy data lives at `d[0].attributes.privacy.privacyTypes` inside `shoebox-media-api-cache-apps`). The two parsers share the same identifier enums (`DATA_USED_TO_TRACK_YOU`, etc.), so downstream snapshot diffing, severity styling, and change detection all work without translation. `computeHistoricalTargets(today, floor, { intervalMonths, anchorDates })` builds the target list: by default one target per calendar quarter (`intervalMonths` defaults to `QUARTER_MONTHS` = 3, but the per-app route accepts `1..6` for a denser e.g. monthly reconstruction), plus it always folds in an **install anchor** — `importAppHistory` reads the app's `apps.firstSeen` and adds it as an extra target so the reconstruction reaches for the privacy state from when the user actually started tracking the app (that install-era snapshot is the baseline `getSinceInstallDiff` diffs against). Anchors outside `[floor, today]` are dropped and anchors within a day of an existing target are de-duped. `computeQuarterlyTargets` is a thin back-compat wrapper. The importer lists every capture of the page once via the CDX index (`listWaybackCaptures` in `lib/wayback.ts`: `web.archive.org/cdx/search/cdx?url=…&filter=statuscode:200&collapse=timestamp:8`) and picks each target's closest capture locally; a capture more than 45 days (`CAPTURE_DRIFT_TOLERANCE_MS`) off its target is dropped (`skipped_drift`) rather than standing in for the whole bucket. If the index is unreachable or malformed it falls back to the per-target `archive.org/wayback/available` walk (`findCaptureWithinTolerance`, ±14/±28/±42 days via `WAYBACK_FALLBACK_OFFSET_DAYS`). A 429 / 5xx from any archive endpoint throws `WaybackUnavailableError` (with `retryAfterMs`) instead of reading as "no capture" — `skipped_no_capture` therefore genuinely means archive.org has nothing near the quarter. This tool never writes to Wayback beyond the Save-Page-Now request described below. Wayback replay URLs always use the `id_` suffix (`/web/<ts>id_/<orig>`) so Apple's original HTML comes through clean of the toolbar injector.

Imported rows go through the same `diffSnapshots → saveSnapshot` path as live scrapes, but `saveSnapshot` accepts `{ source: 'wayback', waybackUrl, scrapedAt, skipChangeCountBump, triggeredBy }`. The defaults for wayback are: `source='wayback'`, `skipChangeCountBump=true` (back-dating shouldn't inflate the unacknowledged badge), `scrapedAt` set to the archive capture's timestamp so timeline ordering matches reality, and `triggeredBy='wayback'` inferred from `source`. `writeWaybackSnapshot` does the insert in one transaction with two chain rules: the oldest row is a **baseline** with no changes (`getSnapshotBefore` returns null and must never fall back to today's `privacy_types` — that produced a diff pointing backwards through time on the 2021 row), and the wayback row that now directly follows the new one is re-diffed against it (`repairSuccessorWaybackDiff`) so a denser or later import can't double-count. Live rows are never rewritten — the last archive → first-live-scrape hop is derived at read time by `bridgeOldestLiveRow` in `getChangelog` (`SnapshotChangelogRow.archive_bridge`), so archive deltas never enter the review queue. The dedupe window scales with cadence (`dedupeWindowForInterval`: 45 days quarterly, 15 days monthly), and the install anchor is skipped when the install is within that window of today (the first live scrape already covers it). Pinned by `tests/app/historical-import-chain.test.ts`.

Save Page Now (`requestFreshCapture`) runs once per app **after** the target loop, and only when the archive has no capture within 45 days of *today* (CDX path: nothing recent in the index; fallback path: the newest target ended uncovered) — archiving the live page cannot fill a 2021 gap, and the old rule fired it on the first empty quarter for nearly every app on every run. The outcome rides on `result.targets` as an extra entry dated today (`requested_snapshot` / `skipped_save_now_failed`) and is counted in `ImportAppHistoryResult.snapshotsRequested`, which the bulk and per-app route summaries fold into their activity-log strings ("N snapshots requested"). Only successes write a timeline note (`appendWaybackAttemptEntry`); `removeImportedHistory` deletes those notes (`source='live' AND triggered_by='wayback'`) together with the wayback rows.

Entry points:

- `POST /api/apps/[id]/import-history` — single-app backfill. Optional body `{ intervalMonths: 1..6, force: true }`. **`force` re-probes every target** instead of skipping the ones a nearby row already covers (`skipped_existing` is otherwise returned without the archive being asked at all) — the right behaviour for a routine pass, the wrong one after a parser fix or once the archive has gained captures. Forcing can add rows but never duplicates one: the capture-URL check still applies. `AppHistoryImportCard` (`app/components/detail/`, above the History timeline, gated by `flag.detail.timeline.wayback_import` → depends on `flag.detail.timeline.wayback_rows`) always posts `force: true` — a person clicking "Check the archive" means "ask again", and an unforced re-run from that button would usually do nothing visible. `DELETE` on the same route purges that app's wayback rows.
...
**Global kill-switch for policy scraping.** `policy_scrape_disabled` (`'true'`/`'false'`, default `'false'`) in `app_settings` is a stronger gate than the throttle: when on, `fetchAndStorePolicySource` short-circuits before the HTTP call for every caller (per-app auto-trigger from `scraper.ts`, bulk runner, scheduled sync, instrumentation resume). The single user-initiated escape hatch is `bypassThrottle=true` (the "Force re-scrape" path) — every other call respects the kill-switch. The bulk-sync route returns 409 with code `policy_scrape_disabled`, the per-app regenerate route returns 409 for `phase: 'fetch' | 'all'` but lets `phase: 'summarise'` through (cached text + AI is still useful when fetches are off), and `instrumentation.ts`'s policy resume clears the queue + mutex with a `bulk-skipped-disabled` activity row instead of starting a run. UI lives at `settings.policy_throttle.scrape_disabled_*` in i18n; toggling it greys the throttle inputs and shows an "inert when disabled" note above them. Test coverage in `tests/app/policy-scrape-disabled.test.ts` pins the contract (zero fetches when on, bypassThrottle override, cache preservation, and a first run that reads as skipped rather than failed: no row left behind, a `partial` "Policy skipped: scraping disabled" activity row, counted as skipped in bulk totals). Over a stored analysis the kill-switch and the throttle both return that row with their own run-log line (`disabled` / `throttled`) last, and that line, not the stored status (which describes an earlier run), decides the activity row ("Policy skipped: scraping disabled" / "Policy skipped: throttled", both `partial`) and the bulk bucket (skipped / throttled). Read the LAST line, not any line: under the kill-switch an `all` run over a clean stored capture still summarises it and is judged by that result. Pinned by `tests/app/policy-skip-activity.test.ts` and mirrored in `core/src/server/policy_store.rs` + `policy_runner.rs`.

The automatic policy fetch that follows an import or App Store sync (`schedulePostAppUpdatePolicyFetch` in `lib/post-app-update-policy-fetch.ts`) checks the kill-switch itself when its timer fires, before its busy check so no retry is queued, and starts no bulk run while it is on. Without that check the run fetched nothing but walked every app through the store's gate, which re-logs each stored analysis: a stored `fetch_error` came back as a fresh "Fetch failed" row and a failure in the bulk totals on every import or sync. The skip is logged to the console only. Do not add an Activity row for it: it is the state the user chose, and a row per import or sync would be noise. Pinned in `tests/app/policy-scrape-disabled.test.ts`, which awaits the drain through the module's `__drainForTests` hook.

The timeline renders wayback rows with a purple dot (`.timeline-dot.wayback`), a clock (🕰) glyph, a "Wayback · YYYY-MM-DD" badge, and a "View on Wayback" link to the capture URL. `lib/changelog-types.ts` exposes these via `source?: 'live' | 'wayback'` and `wayback_snapshot_url` on `SnapshotChangelogRow` — both are normalised server-side in `getChangelog` so older rows default to `'live'`. Each row also carries a `triggered_by` value (`'scheduled' | 'manual' | 'import' | 'wayback' | null`) which drives the `TriggerPill` in the card header; legacy (pre-migration) rows get `null` and render a generic "Live sync" pill. When a wayback baseline snapshot_json is byte-identical to an adjacent live row, `getChangelog` sets `matches_live_sync: true` and the timeline renders a green "Matches live sync" badge so users can see at a glance that the archive and the live App Store page agree. The detail payload carries only the newest 50 rows (`getChangelogPage` → `changelog` + `changelogHasMore`); `ChangelogTimeline` offers "Show older entries", paging through `GET /api/apps/[id]/changelog?before=<ms>&limit=`, so imported 2021 rows are never pushed out of view by a run of no-change syncs, and the "first scan" marker is suppressed while older rows remain. The first-run checklist offers `import_label_history` (self + monitor) linking to Settings → Admin → `#wayback-import`; it completes once any wayback row exists (`hasWaybackHistory`).

### Database

`lib/db.ts` exports a **singleton** `better-sqlite3` instance. Opening it runs `CREATE TABLE IF NOT EXISTS` for every table and applies inline `ALTER TABLE` migrations (`firstSeen`, `changeCount`, `bundleId`, `developer`, `privacyPolicyUrl`, `privacy_categories.type_id`, and `privacy_snapshots.source` / `privacy_snapshots.wayback_snapshot_url`). Any new column must be added both to the `CREATE TABLE` body **and** to the `migrations` array, or existing installs will break. **And in three places, not two:** the Rust core mirrors this schema (`core/src/schema_sql.rs` for the CREATE text, `core/src/db.rs` for the per-table `apply_col_migrations` block at the same point in the sequence), and CI's `core-parity` job runs `pnpm parity:schema`, which fails the build if the Rust migrator's logical schema differs from `lib/db.ts` by so much as one column. Run it locally before pushing. Adding a flag key is the same shape: `pnpm parity:settings-cases` regenerates `core/src/server/flag_rules.json` + the settings fixture, both must be committed, and the key-count pin in `core/src/server/flags.rs` bumps with them. The demo set is the same shape again: after editing `lib/sample-apps.ts` (or the policy lens list, or the accessibility catalogue it resolves against), `pnpm parity:seed-cases` regenerates `core/src/server/sample_apps.json` + the seed fixture, both must be committed, and the app-count pin in `core/src/server/seed_writes.rs` bumps with them.
...
**Permissions are private by contract:** every open runs `tightenDataPermissions()` (`lib/db.ts`) — `0700` on the data dir, `0600` on `privacy.db` + `-wal`/`-shm` — so pre-existing loose installs are fixed on their next boot, and the WAL pragma runs *after* the chmod so sidecar files inherit `0600`. The Rust core does the same on every open (`tighten_permissions` in `core/src/db.rs`), and the Node sidecar also sets `0700` on the dir at creation (`src-tauri/src/sidecar.rs`). Best-effort on purpose (no-op on Windows; never fatal). Consequences: on a bind-mount deploy the host user needs `sudo` to look inside `./data` (CI's `compose-smoke` does `sudo test -f`), and anything new that writes secrets inside the data dir should still set its own `0600` like `lib/backup.ts` does. Pinned by `tests/app/data-dir-permissions.test.ts`.
...
**`skipTrailingSlashRedirect: true` in `next.config.js` is load-bearing — do not remove it.** Next normally answers `/dashboard/` with its own 308, emitted in `dist/server/lib/router-utils/resolve-routes.js` *before* middleware runs, and that redirect branch returns `resHeaders: null` — discarding not just `proxy.ts`'s headers but the static `headers()` block in `next.config.js` too. The result was a 308 with **zero** security headers next to a 200 carrying six. The flag suppresses Next's version so `proxy.ts` issues the redirect itself (step 0.5, sitting where the router's used to) and it goes out through `attachSecurityHeaders` like everything else. Build the Location with a plain `URL` over `request.url`, **never `request.nextUrl.clone()`** — NextURL's pathname setter reports the new value from its getter but doesn't rebuild `href`, so the redirect serialises with the slash still on and loops forever. Pinned by `tests/e2e/security-headers.spec.ts`, which asserts over real HTTP because the older `tests/app/security-regression.test.ts` calls `proxy()` directly and by construction cannot see a response Next answered without it. One gap remains and is not reachable from app code: Next's repeated-slash / backslash normalisation (`//dashboard`, `/dashboard\stats`) 308s before the route table is consulted at all, so those carry no headers either. Both are bodiless redirects to a same-origin canonical path.
...
**Screenshot OCR is the one place WebAssembly runs, and it has its own policy.** tesseract.js's defaults start its worker from a `blob:` URL and fetch the worker script, the engine and the English model from public CDNs; the CSP refuses that, and the wizard hung on "Preparing screenshot scan…". `scripts/stage-ocr-assets.mjs` (the first step of `pnpm build`, `build:standalone` and `dev`) copies `worker.min.js`, the three `tesseract-core*-lstm.wasm.js` builds and `eng.traineddata.gz` out of node_modules into the gitignored `public/ocr/`, with `THIRD-PARTY-OCR.md` and the licence texts, and `lib/ocr-assets.ts` points `createWorker` at them (`workerBlobURL: false`). A dedicated worker started from a same-origin URL runs under the policy delivered with ITS script, so `proxy.ts` and `core/src/server/csp_policy.rs` give exactly `/ocr/worker.min.js` a worker policy with `'wasm-unsafe-eval'` and every page keeps its own, which never allows it; the page needs nothing new, as its `script-src 'self'` covers the worker. Keep both halves: moving the keyword onto pages widens every page, and dropping it from the worker breaks OCR silently (the engine's compile error never reaches `createWorker`, which `awaitOcrStart` times out). `tests/e2e/onboard-ocr.spec.ts` scans a real screenshot on both backends and fails on any request that leaves the machine or any CSP report. `stage-site.mjs`, `stage-standalone.mjs` and `verify-macos-bundle.mjs` refuse a build without the assets. Bump `tesseract.js` and `tesseract.js-core` together; the staging script stops the build when they disagree.
...
**The schedule defaults to Manual, except for Monitor.** `POST /api/focus` with the Monitor goal calls `applyMonitorSyncDefault` (`lib/scheduler.ts`, mirrored in `core/src/server/writes.rs` `focus`), which sets `sync_schedule = daily` only when no schedule is stored. A stored `sync_schedule` is the user's choice (only `POST /api/settings` writes it), so an explicit Manual or Weekly is never overridden, and nothing reverts the default when Monitor is turned off. Keep it that way: do not add another writer of `sync_schedule` that is not a user's choice, or the "never chosen" test stops meaning anything. When the default writes, it also stores `sync_schedule_default_at`, and `getSchedulerStatus` (and the Rust `sync_status` / `scheduled_check`) count the schedule from `max(last_auto_sync, sync_schedule_default_at)`: an install that has never synced would otherwise be due at the first tick, 15 s after boot or right after onboarding, re-fetching every app the import just fetched (and, in the E2E suite, reaching the real App Store from any spec that saves a Monitor focus and seeds apps). `lastRun` stays the last real sync.
...
**A check never resumes a run its own process started.** From the database, a run started in the first 8/10/12 s after boot (Settings' "Sync now", or the deferred policy fetch that a single app's sync schedules) looks exactly like a crash-left one: a blob with pending work and a held mutex. Before the guard, the check "resumed" it: a second runner on the same queue, every pending app fetched twice, and a "resumed after server restart" notification with no restart. Each runner now registers itself as live until it settles (`withLiveBulkRun` in `lib/live-bulk-runs.ts`, an in-memory registry on `globalThis`), and each check returns before touching anything while its job is live. The registry dies with the process, so a crash-left run is still resumed. `core/src/server/live_runs.rs` is the Rust side, keyed by the server's connection because one Rust process can hold several servers. Pinned on both sides by `tests/app/bulk-resume-live-run.test.ts` (the real `register()` closures) and `core/src/server/live_runs_tests.rs`; the runner oracles don't cover this interleaving.
...
**CSS ownership is documented in [docs/CSS.md](docs/CSS.md).** Co-located plain CSS next to its component (matching the repo's existing precedent), with an explicit list of what stays global — tokens, severity/category styling, the `data-a11y-shapes` clip-paths, and cross-surface primitives (`.modal-overlay`, `.btn*`, `.settings-section*`). Class names are load-bearing (e2e selectors, `data-flag-target`, the a11y clip-paths), which is why CSS Modules were rejected. `tests/e2e/visual.spec.ts` is the screenshot net for CSS moves — deliberately local-only (`VISUAL=1`, gitignored baselines), because baselines are platform-specific and app screenshots stay out of the public repo. Its `beforeEach` builds a FIXED two-device fleet with differing owners (`ensureDeviceFixture`) rather than deleting devices down to nothing: with zero devices the nav picker renders nothing at all, so the whole device-scope chrome sat outside the net. The fixture is idempotent because `devices.delete` and the flag-override endpoint are both rate limited server-side, and re-running the setup once per shot was throttled into inconsistent state (visible as `DENY feature_flag.override` lines, which also meant later shots were taken with sections missing).

Two capture rules were learned the hard way when the device shots were added, and apply to any new shot of overlay chrome. **Shoot the element, not the page**, when the subject is an overlay: the drawer and the popover are narrower than the viewport, so a page shot captures whatever the grid behind them happens to render, and that varies with what earlier specs left in the DB. Masking cannot substitute — Playwright paints masks ON TOP, so masking the page paints over the overlay itself. **Budget a few pixels for `backdrop-filter` surfaces**: glass computes its own pixels from what is behind it, so the blurred fringe at a rounded corner moves even when the frame is otherwise identical (`GLASS_EDGE_TOLERANCE`, set two orders of magnitude below any real regression this net has caught). Long pages with derived content need the same care — the Stats shot is scoped to `.page-header` because its policy radar auto-selects which apps to plot.

Two process rules for reading the net's results. **Re-run it after rebasing onto `main`, before calling it green** — local baselines only know the tree they were captured from, and a run that predates the rebase says nothing about what gets pushed. **When an old and a new baseline differ, decide what's real with Playwright's own comparator** — swap the old set into `tests/e2e/visual.spec.ts-snapshots/` and run the current tree against it — not with an any-pixel image diff, which over-reports: most of what it flags on overlay shots is the page ghosting through `backdrop-filter` glass, below Playwright's per-pixel threshold. `~F` in a captured shot is the spec's date-freezer token, not product copy.
...
Localised UI ships through next-intl. `locales/en.json` is the source of truth; every other `locales/<lang>.json` is round-tripped through Crowdin (free OSS plan) so non-developer reviewers can edit copy in a friendly UI without touching JSON. The full workflow — Crowdin project setup, repo secrets, the weekly pull-request cycle, and how to add a new locale — lives at [Translations](https://docs.privacytracker.privacykey.org/develop/translations). The short version for daily work is: edit `locales/en.json`, run `pnpm lint:i18n` to catch parity drift, push to main, and the GitHub Action handles the rest. That action (`.github/workflows/crowdin.yml`) needs the `CROWDIN_PROJECT_ID` and `CROWDIN_PERSONAL_TOKEN` repository secrets and the repository setting that lets Actions open pull requests. Without the secrets it skips with a notice rather than failing; every run before that guard failed for exactly that reason, and the workflow was disabled. Until the secrets exist and the workflow is re-enabled, `zh.json` is maintained by hand in the same PR as the `en.json` change.
...
- The App Store page parser depends on Apple's HTML. If labels suddenly stop appearing, the first suspect is the shelf fallback chain in `saveToDb` (modern: `serialized-server-data` → `shelfMapping.privacyTypes.items`, fallback: `privacyHeader` flatten, fallback: `pageData.shelves`, fallback: `extractFromShoebox` for the historical Ember/FastBoot shape) or the privacy-policy link regex (which must handle both straight `'` and curly `’` apostrophes in the "Developer's Privacy Policy" aria-label). The shoebox extractor is also what lets the Wayback importer reach back to Q1 2021 — it must probe *both* shoebox shapes: `shoebox-media-api-cache-apps` (late 2021 – Nov 2025, JSON-string values keyed by API URL, record at `d[0].attributes`) and `shoebox-ember-data-store` (Feb – ~Oct 2021, plain objects keyed by app id, record at `data.attributes`). Until the second shape was added every 2021 capture failed as `skipped_parse_failure`, so the floor was documented but never reached. Captures from the first weeks of Feb 2021 may legitimately carry no `privacy` attribute at all (Apple was still rolling label data into the web pages); those land as `skipped_no_labels` — counted as skipped, not failed — so a run over them isn't flagged "partial" forever. `skipped_parse_failure` is reserved for HTML that isn't a recognisable product page at all.
...
- `privacy_policy_analyses.status` must be one of `POLICY_ANALYSIS_STATUSES` (`lib/policy-summary-meta.ts`). `hydratePolicyAnalysis` reads anything else as `analysis_error`, which the AI Policy tab renders as a failed AI run, the activity log records as "Policy summary failed" and the bulk runner counts as failed. Two writers got this wrong: the run marker's `'pending'` placeholder (PR #275) and the audit-bundle importer's `'ok'`, whose stored rows `lib/db.ts` and `core/src/db.rs` repair on open. The placeholder is never a result: a run that returns before its first real write deletes it (guarded on `status = 'pending'`) and returns null, as a first fetch stopped by the kill-switch and a Summarise with nothing stored both do. A new writer picks a real status; a new status is added to that list, `getPolicyStatusMessage` in `PolicySummaryPanel`, the bulk runner's `classifyOutcome` and the Rust list in `core/src/server/policy.rs` together.
- **The Node sidecar build (`just tauri-dev-node` / `just tauri-build-node`) needs a one-time `just fetch-node-sidecar`.** The default recipes build on the Rust backend and never need it. A fresh clone has no `src-tauri/binaries/node-<target-triple>` — the binaries are ~139MB and gitignored. `scripts/stage-standalone.mjs` treats a missing one as **fatal**, so the failure surfaces as `stage-standalone: cannot find Node binary at …` → `The "beforeDevCommand" terminated with a non-zero status code`, several minutes into a run whose `next build` and cargo build both looked healthy. The version is not a free choice: the bundled Node's ABI must match the Node that ran `pnpm install`, because `pnpm install` resolves better-sqlite3's prebuild against that ABI and staging copies the same `better_sqlite3.node` into the bundle. A mismatch builds cleanly and then kills the sidecar with `NODE_MODULE_VERSION`. `scripts/fetch-node-sidecar.sh` defaults to the host Node for exactly that reason; CI pins `NODE_VERSION: '24.20.0'` in `macos-release.yml` and *also installs* under it, which is why its pin can differ from yours without either being wrong. Full rationale and the GPG-verification chain: [src-tauri/binaries/README.md](src-tauri/binaries/README.md).
...
- The capability also needs `"local": true` AND a `"remote": { "urls": [...] }` entry that matches the webview's actual URL. For us that is `http://127.0.0.1:*` only, where the backend serves the app; `http://localhost:*` was dropped on purpose when the capability was tightened (commit bda0bc7). Without the remote entry, the ACL grants the permission to the *capability* but the *capability* doesn't apply to the URL, and the dispatcher rejects the invoke with the cryptic "Command X not allowed on window 'main', webview 'main', URL: http://127.0.0.1:NNN allowed on: [windows: 'main', URL: local]" — only visible in debug builds. Release builds strip the diagnostic and just say "Command X not allowed by ACL", which sent us down a long red herring chasing the (correct) permission setup. The backend's URL is not in `tauri.conf.json`: the window is defined there as `about:blank`, and `main.rs` navigates it at boot to the base URL `backend::boot` reports, `http://127.0.0.1:<port>`. The capability's remote URLs must match that host, so a debug build's `PRIVACYTRACKER_DEV_URL` must name `127.0.0.1` as well; with `localhost` every invoke is refused.

- **The Rust shell's own requests to its backend go through `backend::get` and `backend::post` (`src-tauri/src/backend.rs`).** Both attach the launch credential (next bullet). `proxy.ts` refuses a mutating `/api` request with 403 "Cross-origin mutation rejected" unless its `Origin` matches the Host it was sent to or it carries a token header, and the desktop never has an admin token (neither backend is given one). The webview's `fetch` calls get an Origin from the browser; a bare `ureq::post` gets none, which is how the tray's Sync now and Wayback import, the zoom level and the devtools state all failed with nothing on screen. `backend::post` sets `Origin` from the same parsed URL ureq writes `Host` from, and a unit test beside it (run by CI's `pnpm test:tauri`) fails on any other ureq request (GETs included) in the production code of `src-tauri/src`; test modules are skipped, because they call the server without the credential on purpose. A POST whose route answers only when its work is done (`/api/sync/trigger`) must not time out early either, or the shell logs a failure for work that is still going. Both backends finish a handler whose client hung up: against `pt-core serve`, a `/api/sync/trigger` dropped after 1 s still synced every app and released `sync_running`, and a streamed Wayback import dropped after its headers kept running. `tray.rs` shows both shapes, waiting for the sync and taking the Wayback import's NDJSON headers as its start.

- **On the Rust backend the desktop's local server answers `/api` only to the current launch.** The loopback bind keeps the network out but not other processes on the Mac, so `embedded::start` mints 32 random bytes per launch and passes them in the env map as `PRIVACYTRACKER_DESKTOP_TOKEN`. While that is set, the core's gate (step 0.75, `core/src/server/desktop_auth.rs`) refuses every `/api/*` request, public reads and `/api/csp-report` included, with 401 "Desktop credential required" unless it carries the credential in the `X-PrivacyTracker-Desktop-Token` header or the `pt_desktop_session` cookie (compared in constant time). Only the header stands in for a matching `Origin` on a mutation; the cookie never does. Page shells stay public. It is deliberately NOT `AUDITOR_ADMIN_TOKEN`: the frontend reads a configured admin token as a network deployment (pages redirect to `/login`, the unlock card and "Session locked" appear, the dev routes switch on), and `/api/auth/admin-token/status` still answers `configured: false` on the desktop, so nothing the user sees changes. The window gets the cookie from a one-time link: `backend::entry_url` issues a nonce in process and main.rs navigates to `/api/desktop/bootstrap?nonce=…` instead of the base URL; the gate answers it (single use, 60 s) with an `HttpOnly; SameSite=Strict; Path=/api` session cookie and a 303 to `/`. Same-origin navigations (`location.assign`, reloads, deep links) keep the cookie; anything that points a webview at the server afresh must use a new `entry_url`, never the bare base URL. Same-user tools (the MCP companion) read the credential from `<data dir>/.desktop-token` (0600, rewritten every launch, removed on a clean exit) and the port from `.desktop-port`, and send the header. The link is gate-answered, not routed, and has no `app/api` file, so the parity manifest has nothing to classify; web, Docker and every parity harness never set the variable. **The Node sidecar rollback has no such protection**, and a debug build attached through `PRIVACYTRACKER_DEV_URL` sends no credential.

- **A `@tauri-apps/plugin-*` npm package does nothing without its Rust half.** Importing it compiles, bundles and type-checks, but every call is refused at runtime (for example "process.restart not allowed. Plugin not found") unless `tauri-plugin-<name>` is in `src-tauri/Cargo.toml`, registered with `.plugin(...)` in `src-tauri/src/main.rs`, and granted in `src-tauri/capabilities/main.json`. That is how the updater's `relaunch()` failed after every successful install until the process plugin was added. `tests/app/tauri-updater.test.ts` checks all three for every plugin package in `package.json`. Keep the crate on the npm package's major.minor: the tauri CLI refuses to build a mismatch unless given `--ignore-version-mismatches`.

- **Every reveal of the main window goes through `window_lock::reveal` (`src-tauri/src/window_lock.rs`).** That is where "Require Touch ID / password to open the window" is enforced, with an in-process unlocked flag that hiding the window clears (`window_lock::hide`) and auto-lock clears after the idle time. A bare `window.show()` anywhere else opens the window without asking, so a test beside the gate fails on any other window `.show()` or `.hide()` in `src-tauri/src`. Two things would get around it without touching the shell's Rust: a `core:window:allow-show`-style grant in `capabilities/main.json` (the page uses no window API) and the window-state plugin restoring visibility, which it does by default and through its `restore_state` command; the plugin is built without `StateFlags::VISIBLE` and the page gets no `window-state:*` grant. `tests/app/tauri-shell-acl.test.ts` pins both, and that every command in `generate_handler!` is in `build.rs` and granted.

- **Updates install through `install_verified_update` (`src-tauri/src/update_guard.rs`), not the updater plugin's `downloadAndInstall()`.** The `version` in `latest-v2.json` is not signed; the minisign signature covers only the archive. The command reads `CFBundleShortVersionString` from the signed archive before installing and from the installed bundle after, and `lib/tauri-updater.ts` relaunches only when both are newer than the running version. The page is granted `updater:allow-check` alone; granting `updater:default` again would let it install without that check.

- **"Start at login" is a LaunchAgent** (`MacosLauncher::LaunchAgent`, `~/Library/LaunchAgents/privacytracker.plist`). The AppleScript launcher drives System Events, which needs an Apple Events entitlement and usage description that neither entitlements file has, so in the signed app it fails silently. `src-tauri/src/autostart.rs` registers the LaunchAgent once for installs that had the setting on.

- **The desktop app runs on the Rust backend; the Node sidecar is the rollback.** Built with `--features rust-backend` the shell serves the app from its own process with the Rust core; built without it, it spawns the Node sidecar as it always did. Since the desktop cutover (Phase 6) every place a desktop build is chosen defaults to Rust: `just tauri-dev` / `pnpm tauri:dev` (a `pnpm build`, then the app, with no `fetch-node-sidecar` and no standalone tarball), `just tauri-build` / `pnpm tauri:build`, and the release, where `Prepare verified release draft` passes `backend: rust` to **macOS desktop release**, whose own default is `rust` too. The Node build stays buildable until v0.3.0 has shipped as the rollback: `just tauri-dev-node`, `just tauri-build-node`, and `backend: node` in `release.yml`, released as a NEW patch version because the updater never installs an older one. `src-tauri/src/backend.rs` is what the rest of the shell talks to; `sidecar.rs` and `embedded.rs` are the two implementations, each compiled only on its own path. The feature stays out of Cargo's default set so a build without it is still pure Node: `tests/app/rust-core-inert.test.ts` requires the optional dependency, the gated module declaration in `main.rs` and the `#![cfg(feature = "rust-backend")]` at the top of `embedded.rs`, and keeps the core off every other Node path (the Next.js server and the shell built without the feature), so the rollback stays Node. The Dockerfile left its list at the Docker cutover: it builds the core for its default stage, and its `node` stage copies nothing of it. CI's `rust-check` compiles the shell both ways. The embedded backend reuses the port it bound last (`<data>/.desktop-port`), which is what keeps the page's origin, and therefore its local storage, across relaunches, and once it is serving it deletes what a Node build extracted into the data directory (`<data>/standalone` and its markers; a rollback build extracts its own again). A packaged Rust build stages the frontend into the bundle's Resources with `scripts/stage-site.mjs` through the `src-tauri/tauri.rust.conf.json` overlay, rather than extracting a tarball into the data directory, and `scripts/parity/handoff.mjs` (`just handoff`, and CI's core-parity job) proves either backend can open and serve the database the other wrote, which is what a rollback depends on. `rust` releases build with the overlay and `entitlements-rust.plist`, and `scripts/verify-macos-bundle.mjs <app> <arch> [backend]` (default `rust`) checks a Rust bundle by driving the packaged app through its hidden `--smoke-server <dir>` mode. Keep the Node-only steps in that workflow gated on `env.BACKEND`: fetching the Node sidecar, the staging keychain, `build:standalone` and `STANDALONE_PRE_BUILT` all exist for the sidecar and break or waste time on a Rust build. A Rust bundle also carries its notices under `third-party/` (`scripts/stage-notices.mjs`), and the crate list they include is generated: after changing a Rust dependency run `pnpm notices:rust` and commit `src-tauri/THIRD-PARTY-RUST.md` and `lib/rust-crates.json`, or CI's `rust-check` job fails on the difference.

- **Git worktrees nested under `.claude/worktrees/` need two root anchors.** Claude Code's worktrees live *inside* the main clone, so every file in one sits under a `.claude` path segment and the parent clone's lockfile is an ancestor. Two settings exist purely to keep that layout working and should stay: `outputFileTracingRoot: __dirname` in `next.config.js` (otherwise Next picks the parent clone as the workspace root, warns on every build, and `pnpm build:standalone` fails with `ENOENT .next/standalone/server.js` because the tree lands under a nested path) and the root-anchored `"!!.claude"` exclusion in `biome.jsonc` (the old `!!**/.claude` matched the worktree itself, so `pnpm lint` reported "Checked 0 files" and passed). Treat "Checked 0 files" as a failure, never a pass.
