...
test("AI model option lists are scoped by provider", () => {
  ...
  assert.ok(
    getAiModelOptions("openai").some(
      (option) => option.value === "gpt-4.1-mini"
    )
  );
  assert.ok(
    getAiModelOptions("anthropic").some(
      (option) => option.value === "claude-sonnet-4-20250514"
    )
  );
  assert.ok(
    getAiModelOptions("custom").some((option) => option.value === "llama3.2")
  );
});
...
test("AI timeout defaults and overrides account for slow local models", () => {
  ...
  assert.equal(
    resolveAiTimeoutMs(undefined, "openai", "gpt-4.1-mini", "direct"),
    90_000
  );
  assert.equal(
    resolveAiTimeoutMs("55000.9", "openai", "gpt-4.1-mini", "direct"),
    55_000
  );
  assert.equal(
    resolveAiTimeoutMs("not-a-number", "openai", "gpt-4.1-mini", "direct"),
    90_000
  );
  assert.equal(
    resolveAiTimeoutMs("1", "openai", "gpt-4.1-mini", "direct"),
    AI_TIMEOUT_MIN_MS
  );
  assert.equal(
    resolveAiTimeoutMs("999999999", "custom", "gemma3n:e4b", "merge"),
    AI_TIMEOUT_MAX_MS
  );
});
