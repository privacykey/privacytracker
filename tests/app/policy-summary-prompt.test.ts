...
test("policy summary prompt is grounded, schema-shaped, and injection-resistant", () => {
  ...
  assert.match(
    preview.user,
    /exactly one entry for each key in this exact order/
  );
  ...
  assert.match(
    preview.user,
    /concrete practice, actor, data type, right, or retention limit/
  );
...
test("guardian prompt preview asks for the minor-safety section only for guardian audience", () => {
  ...
  assert.equal(
    JSON.stringify(selfPreview.schema).includes("safetySummary"),
    false
  );
...
test("policy system prompt preserves the legal-index failure behavior", () => {
  assert.match(
    POLICY_SYSTEM_PROMPT,
    /navigation page, legal index, table of contents, cookie banner/
  );
  assert.match(POLICY_SYSTEM_PROMPT, /set every lens rating to `unclear`/);
  assert.match(
    POLICY_SYSTEM_PROMPT,
    /The linked page did not contain a substantive privacy policy/
  );
});
