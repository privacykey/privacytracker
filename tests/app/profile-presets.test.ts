...
test("matchPreset round-trips every named preset", () => {
  ...
  for (const presetKey of PROFILE_PRESET_KEYS) {
    ...
    assert.equal(
      matchPreset(fresh),
      presetKey,
      `expected matchPreset to return "${presetKey}"`
    );
  }
});
...
test("matchPreset returns null for empty, sparse, and customised profiles", () => {
  assert.equal(
    matchPreset(null),
    null,
    "null profile should not match a preset"
  );
  assert.equal(
    matchPreset(undefined),
    null,
    "undefined profile should not match a preset"
  );
  assert.equal(
    matchPreset({}),
    null,
    "empty profile should not match a preset"
  );
  ...
  sparseStrict.OTHER = undefined;
  assert.equal(
    matchPreset(sparseStrict),
    null,
    "sparse profile should not match a preset"
  );
...
test("describePresetTransition: preset → custom edit returns null (custom is not a preset)", () => {
  ...
  assert.equal(
    describePresetTransition(PROFILE_PRESETS.strict, customised),
    null
  );
});

test("describePresetTransition: custom → custom returns null (no boundary crossed)", () => {
  const customA: PrivacyProfile = {
    LOCATION: "tracking",
    CONTACT_INFO: "tracking",
  };
  const customB: PrivacyProfile = {
    LOCATION: "linked",
    CONTACT_INFO: "tracking",
  };
