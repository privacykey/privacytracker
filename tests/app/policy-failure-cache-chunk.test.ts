...
test("policy sync records too-short and unsupported-content failures without calling AI", async () => {
  ...
  seedTrackedApp({
    id: "too-short-app",
    name: "Too Short",
    privacyPolicyUrl: "https://example.com/short",
  });
  seedTrackedApp({
    id: "pdf-app",
    name: "PDF Policy",
    privacyPolicyUrl: "https://example.com/policy.pdf",
  });
  let aiCalls = 0;
  ...
  global.fetch = (async (raw: string | URL | Request) => {
    ...
    if (url === "https://example.com/short") {
      return new Response("We collect data.", {
        status: 200,
...
test("blocked policy fetch stores diagnostics for the activity log", async () => {
  ...
  seedTrackedApp({
    id: "blocked-app",
    name: "Blocked Policy",
    privacyPolicyUrl: "https://example.com/blocked",
  });
  ...
  assert.equal(
    detail.fetchDiagnostics?.requestedUrl,
...
test("unchanged policy text skips AI, while changed text preserves the previous summary", async () => {
  ...
  seedTrackedApp({
    id: "cache-app",
    name: "Cache Policy",
    privacyPolicyUrl: "https://example.com/cache",
  });
  ...
  global.fetch = (async (raw: string | URL | Request, init?: RequestInit) => {
    ...
    if (url === "https://example.com/cache") {
      return new Response(policyText, {
        status: 200,
        headers: { "content-type": "text/plain" },
      });
    }
    if (url.endsWith("/chat/completions")) {
      ...
      return aiSummaryResponse(
        aiOverviews.shift() ?? "Unexpected extra summary"
      );
    }
    if (url.startsWith("https://archive.org/wayback/available")) {
      return new Response(JSON.stringify({ archived_snapshots: {} }), {
    ...
    if (url.startsWith("https://web.archive.org/save/")) {
      return new Response("", {
        status: 200,
        headers: {
          "content-location": "/web/20260101000000/https://example.com/cache",
        },
      });
    }
    throw new Error(`Unexpected fetch: ${url}`);
...
test("long policy text uses chunked AI summarisation and stores reusable chunk notes", async () => {
  ...
  seedTrackedApp({
    id: "chunk-app",
    name: "Chunk Policy",
    privacyPolicyUrl: "https://example.com/chunk",
  });
  ...
  global.fetch = (async (raw: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.endsWith("/chat/completions")) {
      ...
      if (schemaName === "privacy_policy_chunk_note") {
        return openAiContentResponse({
          summary:
            "This chunk discusses collection, sharing, controls, retention, and children.",
          highlights: [
    ...
    if (url.startsWith("https://web.archive.org/save/")) {
      return new Response("", {
        status: 200,
        headers: {
          "content-location": "/web/20260101000000/https://example.com/chunk",
        },
      });
    }
    throw new Error(`Unexpected fetch: ${url}`);
  ...
  assert.ok(
    requestNames.filter((name) => name === "privacy_policy_chunk_note")
      .length >= 2
  );
