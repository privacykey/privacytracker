...
import { readBulkState, writeBulkState } from "../../lib/wayback-bulk-state";
...
test("Save Page Now resolves redirect metadata without downloading the archived page", async () => {
  ...
  global.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    return new Response("x".repeat(512 * 1024), {
      ...
      headers: {
        location:
          "/web/20260512010203/https://apps.apple.com/au/app/tiktok/id835599320",
        "content-type": "text/html; charset=utf-8",
      },
    });
  }) as typeof fetch;

  const result = await submitToWaybackSaveNow(
    "https://apps.apple.com/au/app/tiktok/id835599320"
  );

  assert.equal(result.ok, true);
  assert.deepEqual(calls, [
    "https://web.archive.org/save/https://apps.apple.com/au/app/tiktok/id835599320",
  ]);
  if (result.ok) {
    assert.equal(
      result.snapshot.url,
