...
test('privacy policy sync fetches source text, calls AI once, and stores a normalized summary', async () => {
  ...
  global.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url === `${AI_BASE_URL}/chat/completions`) {
      assert.equal((init?.headers as Record<string, string>).Authorization, 'Bearer fixture-api-key');
    ...
    if (url.startsWith('https://web.archive.org/save/')) {
      return new Response('', {
        status: 200,
        headers: { 'content-location': '/web/20260101000000/https://example.com/privacy-clock' },
      });
    }
  ...
  const versionCount = db
    .prepare('SELECT COUNT(*) AS count FROM privacy_policy_versions WHERE app_id = ?')
...
function buildAiSummaryResponse() {
  return {
    overview: 'Fixture Developer collects account, device, usage, and location data to operate and improve the service.',
    highlights: [
