...
import { getPendingChangeCategoriesByApp } from '../lib/scraper';
...
test('resync detects privacy-label diffs, bumps change count, and writes a notification', async () => {
  ...
  await fetchAndParseApp('https://apps.apple.com/us/app/diff-fixture/id2001', false, false, 'import');
  ...
  const result = await fetchAndParseApp('https://apps.apple.com/us/app/diff-fixture/id2001', true, false, 'manual');

  assert.equal(result.changesDetected, true);
  assert.equal(result.changeCount, 1);

  const app = db.prepare('SELECT changeCount FROM apps WHERE id = ?').get('2001') as { changeCount: number };
  ...
  `).all('2001') as Array<{ changes_detected: number; changes_summary: string | null; triggered_by: string | null }>;
  ...
  const notification = db.prepare('SELECT change_summary FROM notifications WHERE app_id = ?').get('2001') as { change_summary: string };
...
test('privacyHeader legacy purposes are flattened into privacy categories', async () => {
            ...
            pageData: {
              shelves: [{
                ...
                items: [{
                  ...
                  purposes: [
                    { categories: [{ identifier: 'CONTACTS', title: 'Contacts' }] },
                    { categories: [{ identifier: 'CONTACTS', title: 'Contacts' }, { identifier: 'IDENTIFIERS', title: 'Identifiers' }] },
                  ],
                }],
              }],
            },
  ...
  await fetchAndParseApp('https://apps.apple.com/us/app/header-fixture/id2002', false, false, 'import');
  const app = getAppWithPrivacy('2002') as AppWithPrivacy;
  const categories = app.privacyTypes[0].categories.map(category => category.identifier).sort();
...
test('generic pageData privacy shelves are parsed as final fallback', async () => {
  ...
  await fetchAndParseApp('https://apps.apple.com/us/app/page-data-fixture/id2003', false, false, 'import');
  const app = getAppWithPrivacy('2003') as AppWithPrivacy;
  assert.equal(app.privacyTypes[0].identifier, 'DATA_NOT_LINKED_TO_YOU');
  assert.deepEqual(app.privacyTypes[0].categories.map(category => category.identifier), ['DIAGNOSTICS']);
});
...
test('No Details Provided pages store an explicit empty privacy-label state', async () => {
  installScraperFetchMock({
    appHtml: appStoreHtml({
      ...
      shelfMapping: {},
      extraBody: 'No Details Provided. The developer will be required to provide privacy details when they submit their next app update.',
    }),
  });

  await fetchAndParseApp('https://apps.apple.com/us/app/no-details-fixture/id2004', false, false, 'import');
...
test('older App Store HTML with raw array payload and single-quoted script id is parsed', async () => {
  ...
  await fetchAndParseApp('https://apps.apple.com/us/app/raw-array-fixture/id2005', false, false, 'import');
  const app = getAppWithPrivacy('2005') as AppWithPrivacy;
  assert.equal(app.privacyTypes[0].identifier, 'DATA_USED_TO_TRACK_YOU');
  assert.deepEqual(app.privacyTypes[0].categories.map(category => category.identifier), ['IDENTIFIERS']);
});
...
test('curly-apostrophe privacy policy link is extracted and sanitised', async () => {
  installScraperFetchMock({
    appHtml: appStoreHtml({
      ...
      privacyItems: [],
      extraBody: '<a href="https://example.com/privacy" aria-label="Developer’s Privacy Policy">Privacy Policy</a>',
    }),
  });

  await fetchAndParseApp('https://apps.apple.com/us/app/policy-link-fixture/id2006', false, false, 'import');
...
type AppWithPrivacy = {
  ...
  privacyTypes: Array<{ identifier: string; categories: Array<{ identifier: string }> }>;
};
...
function privacyType(identifier: string, title: string, categories: Array<[string, string]>) {
  return {
    identifier,
    title,
    detail: '',
