...
test('stored notification prefs parsing keeps only known boolean keys', () => {
  const parsed = parseStoredPrefs(JSON.stringify({
    labelChanges: false,
    aiTimeout: true,
    policyUpdates: 'false',
    unknownType: false,
...
test('resolvePrefs merges partial stored prefs with every default key', () => {
  ...
  assert.deepEqual(Object.keys(resolved).sort(), [...NOTIFICATION_TYPE_KEYS].sort());

  for (const key of NOTIFICATION_TYPE_KEYS) {
    if (key === 'labelChanges') continue;
...
test('classifyNotificationType maps synthetic payload markers before fallbacks', () => {
  ...
  assert.equal(classifyNotificationType([{ type: 'manual_apps_prompt' }]), 'manualAppsPrompt');
  assert.equal(classifyNotificationType([{ type: 'import_completed' }]), 'importCompleted');
  assert.equal(classifyNotificationType([{ type: 'profile_mismatch' }]), 'profileMismatch');
  assert.equal(classifyNotificationType([{ type: 'version_update' }]), 'versionUpdates');
  assert.equal(classifyNotificationType([{ type: 'category_added' }, { type: 'policy_summary' }]), 'policyUpdates');
  assert.equal(classifyNotificationType([{ type: 'category_added' }]), 'labelChanges');
});
