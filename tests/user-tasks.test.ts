...
function emptyCtx(overrides: Partial<TaskCompletionContext> = {}): TaskCompletionContext {
  return {
...
test('TASK_DEFS has the seven expected task ids in order', () => {
...
test('default self audience with no goals: universal tasks only, no declutter/understand-only ones', () => {
  ...
  const resolved = resolveTasks(f, ctx, { tasks: {} }, { isDesktop: false }, NOW);
...
test('completionCheck for create_privacy_profile flips on hasPrivacyProfile', () => {
  ...
  assert.equal(open.find(r => r.id === 'create_privacy_profile')!.state, 'ready');
  assert.equal(done.find(r => r.id === 'create_privacy_profile')!.state, 'completed');
});
...
test('isAllSettled returns true when every task is completed or dismissed', () => {
  ...
  const mixed = tasks.map((t, i) => i === 0 ? { ...t, state: 'ready' as const } : t);
...
test('getOptInCandidates surfaces opt-in tasks not yet opted in', () => {
  const f = focus({});
  const ctx = emptyCtx({ focus: f });
  const candidates = getOptInCandidates(f, ctx, { tasks: {} }, { isDesktop: false });
...
test('setup_background_mode completes when sync_schedule != manual OR wizard completed', () => {
  ...
  assert.equal(r.find(x => x.id === 'setup_background_mode')!.state, 'completed');
...
test('remove_apps_from_phone is blocked without a privacy profile (universal prereq)', () => {
  ...
  assert.equal(r.find(x => x.id === 'remove_apps_from_phone')!.state, 'blocked');

  const withProfile = emptyCtx({ focus: f, hasPrivacyProfile: true });
  r = resolveTasks(f, withProfile, optedIn, { isDesktop: false }, NOW);
  assert.equal(r.find(x => x.id === 'remove_apps_from_phone')!.state, 'ready');
});
...
test('remove_apps_from_phone completes once at least one uninstall verdict is set', () => {
  ...
  assert.equal(r.find(x => x.id === 'remove_apps_from_phone')!.state, 'completed');
});
