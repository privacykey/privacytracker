                        ...
                        features: [
                          {
                            ...
                            artwork: {
                              template: "systemimage://voice.control",
                            },
                          },
                        ],
                      },
...
test("accessibility parser falls back to compact title-only shelf", () => {
  ...
  assert.deepEqual(
    features?.map((feature) => feature.identifier),
...
test("accessibility parser distinguishes absent shelf from empty declared shelf", () => {
  assert.equal(
    extractAccessibilityFeatures([{ data: { shelfMapping: {} } }]),
    null
  );
  assert.deepEqual(
    extractAccessibilityFeatures([
...
test("accessibility feature slugs are stable for punctuation and spacing", () => {
  assert.equal(
    slugifyFeatureTitle("Differentiate Without Color Alone"),
