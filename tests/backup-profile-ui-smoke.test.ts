...
import { getAccessibilityProfile, saveAccessibilityProfile } from '../lib/accessibility-profile-server';
import { getPrivacyProfile, savePrivacyProfile } from '../lib/privacy-profile-server';
...
test('profile routes sanitise invalid keys and clear saved profiles', async () => {
  ...
  const privacyPut = await privacyRoute.PUT(jsonRequest('/api/privacy-profile', {
    profile: {
      LOCATION: 'not_linked',
  ...
  assert.deepEqual((await privacyPut.json()).profile, { LOCATION: 'not_linked' });

  const privacyGet = await privacyRoute.GET();
  assert.deepEqual((await privacyGet.json()).profile, { LOCATION: 'not_linked' });

  const a11yPut = await a11yRoute.PUT(jsonRequest('/api/accessibility-profile', {
    profile: {
      voiceover: 'required',
  ...
  const clear = await privacyRoute.PUT(jsonRequest('/api/privacy-profile', { profile: null }));
...
test('backup restore round-trips apps, profiles, annotations, summaries, imports, and labels', () => {
  ...
  const batch = createImport({ source: 'file', sourceLabel: 'backup.csv', total: 1 });
  ...
  assert.equal(JSON.stringify(envelope).includes('should-not-leave-backup'), false);
  ...
  assert.ok(app);
  assert.deepEqual(app.privacyTypes[0].categories.map(category => category.identifier), ['CONTACT_INFO']);
  assert.deepEqual(app.accessibilityFeatures.map(feature => feature.identifier), ['voiceover']);
...
test('audit bundle excludes private annotations while preserving exportable context', () => {
  ...
  seedAccessibilityFeature({ appId: 'audit-app', identifier: 'captions', title: 'Captions' });
  savePrivacyProfile({ DIAGNOSTICS: 'not_linked' });
  createAnnotation({ appId: 'audit-app', content: 'Share this note', visibility: 'export' });
  createAnnotation({ appId: 'audit-app', content: 'Keep this private', visibility: 'private' });
  ...
  assert.deepEqual(bundle.apps[0].accessibility_features.map(feature => ({
  ...
  assert.deepEqual(bundle.annotations.map(annotation => annotation.content), ['Share this note']);
});
...
test('dashboard/detail data helpers load seeded app privacy and accessibility data', () => {
  ...
  seedAccessibilityFeature({ appId: 'ui-app', identifier: 'voiceover', title: 'VoiceOver' });

  const apps = getAllApps() as Array<{ id: string; categoryCount: number; accessibilityCount: number }>;
