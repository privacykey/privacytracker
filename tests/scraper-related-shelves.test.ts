...
test('extracts may_also_like from customersAlsoBoughtAppsCollection.items', () => {
  const raw = buildRawData({
    customersAlsoBoughtAppsCollection: {
      items: [
        { id: '111', name: 'Counterpart One', artistName: 'Acme', url: 'https://apps.apple.com/us/app/id111', artwork: { url: 'https://img.test/111.png' } },
        { id: '222', name: 'Counterpart Two', artistName: 'Beta', url: 'https://apps.apple.com/us/app/id222' },
      ],
    },
  });
...
test('extracts more_by_developer from moreByThisDeveloperCollection.items', () => {
  const raw = buildRawData({
    moreByThisDeveloperCollection: {
      items: [
        { id: '333', name: 'Sibling App', artistName: 'Same Dev', url: 'https://apps.apple.com/us/app/id333' },
      ],
    },
  });
...
test('extracts both shelves from the same page', () => {
  const raw = buildRawData({
    customersAlsoBoughtAppsCollection: {
      items: [{ id: '111', name: 'A', url: 'https://apps.apple.com/us/app/id111' }],
    },
    moreByThisDeveloperCollection: {
      items: [{ id: '222', name: 'B', url: 'https://apps.apple.com/us/app/id222' }],
    },
  });

  const out = extractRelatedAppShelves(raw);
  const byType: Record<string, string[]> = { may_also_like: [], more_by_developer: [] };
  for (const r of out) byType[r.shelfType].push(r.relatedAppleId);
  ...
  const raw = buildRawData({
    customersAlsoBoughtApps: {
      seeAllAction: {
        pageData: {
          shelves: [
            { items: [{ id: '999', name: 'Wrapped App', url: 'https://apps.apple.com/us/app/id999' }] },
          ],
        },
      },
    },
...
test('coerces numeric ids to strings (Apple sometimes ships them as numbers)', () => {
  const raw = buildRawData({
    customersAlsoBoughtAppsCollection: {
      items: [{ id: 12345, name: 'Numeric ID', url: 'https://apps.apple.com/us/app/id12345' }],
    },
  });
...
test('returns empty array on missing shelfMapping (and never throws)', () => {
  ...
  assert.deepEqual(extractRelatedAppShelves([{ data: { shelfMapping: {} } }]), []);
});
