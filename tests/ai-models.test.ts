...
test('AI model discovery resolves the saved masked OpenAI key and filters non-text models', async () => {
  ...
  const res = await route.POST(new Request('http://127.0.0.1/api/ai/models', {
    method: 'POST',
    headers: {
  ...
  assert.deepEqual(body.models?.map(model => model.id), ['gpt-5.4-mini', 'o4-mini']);
});
...
test('AI model discovery follows Anthropic model pagination', async () => {
  ...
  assert.deepEqual(body.models, [
    ...
    { id: 'claude-haiku-4-5-20251001', label: 'Claude Haiku 4.5', source: 'anthropic' },
  ]);
