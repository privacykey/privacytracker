...
test("sample policy summary route uses the selected model and saved masked OpenAI key", async () => {
  ...
  const route = (await import(
    "../app/api/ai/policy-sample/route"
  )) as SampleRoute;
  const res = await route.POST(
    new Request("http://127.0.0.1/api/ai/policy-sample", {
      method: "POST",
      headers: {
  ...
  assert.ok(
    body.sample?.reviewChecklist?.some((item) =>
      /Judge the selected model/.test(item)
    )
  );
  assert.ok((body.sample?.expectedSignals?.length ?? 0) >= 8);
  assert.match(
    body.sample?.policyText ?? "",
    /We do not sell personal information/
  );
  assert.deepEqual(
    body.summary?.lenses?.map((lens) => lens.key),
    POLICY_LENSES.map((lens) => lens.key)
  );
  ...
  assert.match(
    aiRequests[0].messages[1].content,
    /We do not sell personal information/
  );
...
function buildSampleSummaryResponse() {
  return {
    overview:
      "Example App Co. collects account, device, usage, diagnostics, and optional location data to run and secure Sample Notes.",
    highlights: [
    ...
    lenses: POLICY_LENSES.map(({ key }) => ({
      key,
      rating:
        key === "ads_marketing" || key === "data_retention"
          ? "favorable"
          : "mixed",
