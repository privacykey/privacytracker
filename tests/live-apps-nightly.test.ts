...
for (const appFixture of LIVE_APPS) {
  test(`nightly live App Store import: ${appFixture.id}`, {
    skip: RUN_NIGHTLY_LIVE_TESTS ? false : 'Set RUN_NIGHTLY_LIVE_TESTS=1 to hit the live App Store.',
  }, async () => {
    resetTestDb();

    const result = await fetchAndParseApp(appFixture.url, false, false, 'manual');
