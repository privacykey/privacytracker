...
test('a single edit drops the active preset match', () => {
  ...
  const tweaked: DashboardLayout = {
    v: 1,
    order: [DASHBOARD_PRESETS.default.order[1], DASHBOARD_PRESETS.default.order[0], ...DASHBOARD_PRESETS.default.order.slice(2)],
    hidden: [],
  };
...
test('matchDashboardPreset returns null for empty / malformed layouts', () => {
  ...
  assert.equal(
    matchDashboardPreset({ v: 1, order: [], hidden: [] }),
    null,
  );
});
...
test('reconcileLayout strips unknown card ids', () => {
  ...
  for (const id of out.order) {
    assert.ok(CANONICAL_ORDER.includes(id), `${id} should not survive reconcile`);
  }
  for (const id of out.hidden) {
    assert.ok(FIRST_CLASS_CARDS.has(id), `${id} survived in hidden but isn't first-class`);
  }
});
...
test('reconcileLayout appends new canonical cards next to their neighbour', () => {
  ...
  const trimmed: readonly DashboardCardId[] = CANONICAL_ORDER.filter(id => id !== 'activity_section');
  ...
  assert.equal(activityIdx, staleIdx + 1, 'activity_section should slot right after stale_section');
});
...
test('reconcileLayout drops callouts from hidden[]', () => {
  ...
  const out = reconcileLayout(stored);
  for (const id of out.hidden) {
    assert.ok(FIRST_CLASS_CARDS.has(id), `${id} is in hidden[] but is a callout`);
  }
  assert.ok(out.hidden.includes('hero'), 'first-class hidden entries should survive');
});
...
test('describeLayoutTransition is silent for re-saves of the same preset', () => {
  assert.equal(
    describeLayoutTransition(DASHBOARD_PRESETS.minimal, DASHBOARD_PRESETS.minimal),
    null,
  );
});
