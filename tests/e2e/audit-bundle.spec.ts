...
browserFlow(
  "audit-bundle round-trip: export carries the Strict preset and the import UI surfaces it",
  async ({ page, request }) => {
    ...
    await expect(
      exportRes,
      "export endpoint must succeed under loved_one focus"
    ).toBeOK();
    ...
    await expect(
      modal.locator(".audit-bundle-import__envelope-preset")
    ).toContainText(/Strict/i);
    ...
    await expect(result).toContainText(
      new RegExp(`${RECOMMENDER_NAME}.*Strict.*preset`, "i")
    );
  }
);
...
browserFlow(
  ...
  async ({ page }) => {
    ...
    await panel
      .locator(".audit-bundle-export__field-input")
      .fill(EXPORT_RECOMMENDER_NAME);
