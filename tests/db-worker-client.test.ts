  ...
  const result = await runBulkWrite([
    {
      sql: `INSERT INTO apps (id, name, url, lastSynced, firstSeen, changeCount)
            VALUES (?, ?, ?, ?, ?, ?)`,
      params: ['test-1', 'Fixture App', 'https://apps.apple.com/us/app/x/id1', Date.now(), Date.now(), 0],
    },
  ]);
  assert.equal(result.totalChanges, 1);
  const row = db.prepare('SELECT name FROM apps WHERE id = ?').get('test-1') as { name: string } | undefined;
...
test('runBulkWrite chunks transactions and commits earlier chunks before failure', async () => {
  ...
  for (let i = 0; i < 30; i += 1) {
    statements.push({
      ...
      params: [`chunk-${i}`, i === 25 ? null : `App ${i}`, `https://apps.apple.com/us/app/x/id${i}`, baseTs, baseTs, 0],
    });
  }
  await assert.rejects(runBulkWrite(statements, { chunkSize: 10 }));

  // Earlier chunks (0-9 and 10-19) committed before the failure.
  const earlier = db.prepare('SELECT COUNT(*) as count FROM apps WHERE id LIKE ?').get('chunk-%') as { count: number };
  assert.equal(earlier.count, 20, 'expected 20 rows from the first two chunks to have committed');

  // The bad chunk (20-29) rolled back, including rows BEFORE the bad
  // one in the same chunk (20-24).
  const inBadChunk = db.prepare('SELECT COUNT(*) as count FROM apps WHERE id IN (?, ?, ?, ?, ?)')
    .get('chunk-20', 'chunk-21', 'chunk-22', 'chunk-23', 'chunk-24') as { count: number };
  assert.equal(inBadChunk.count, 0, 'rows 20-24 must roll back with the failing chunk');
});
...
test('runBulkWrite supports object-style named parameters', async () => {
  ...
  assert.equal(result.totalChanges, 1);
  const row = db.prepare('SELECT name FROM apps WHERE id = ?').get('named-1') as { name: string } | undefined;
    ...
    await runBulkWrite([
      {
        sql: `INSERT INTO apps (id, name, url, lastSynced, firstSeen, changeCount)
              VALUES (?, ?, ?, ?, ?, ?)`,
        params: ['ok-row', 'Good', 'https://apps.apple.com/us/app/x/id1', baseTs, baseTs, 0],
      },
      {
        sql: `INSERT INTO apps (id, name, url, lastSynced, firstSeen, changeCount)
              VALUES (?, ?, ?, ?, ?, ?)`,
        // Same primary key as the previous row → UNIQUE constraint failure.
        params: ['ok-row', 'Dup', 'https://apps.apple.com/us/app/x/id2', baseTs, baseTs, 0],
      },
    ]);
    assert.fail('runBulkWrite should have thrown on the duplicate insert');
  } catch (err) {
    const e = err as Error & { failedAtIndex?: number };
    assert.equal(e.failedAtIndex, 1, 'failedAtIndex should point at the duplicate row');
