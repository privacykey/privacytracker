...
test('fixture App Store import stores Clock privacy labels, accessibility labels, and initial snapshot', async () => {
  ...
  const privacyByType = new Map(app.privacyTypes.map(type => [
    type.identifier,
    type.categories.map(category => category.identifier).sort(),
  ]));
  assert.deepEqual(privacyByType.get('DATA_LINKED_TO_YOU'), ['CONTACT_INFO', 'IDENTIFIERS']);
  assert.deepEqual(privacyByType.get('DATA_NOT_LINKED_TO_YOU'), ['DIAGNOSTICS']);
  assert.deepEqual(privacyByType.get('DATA_USED_TO_TRACK_YOU'), ['LOCATION']);

  assert.deepEqual(app.accessibilityFeatures.map(feature => feature.identifier).sort(), [
    'captions',
    'voiceover',
  ]);
                  ...
                  items: [{
                    features: [
                      ...
                      },
                      {
                        ...
                        artwork: { template: 'systemimage://captions.bubble' },
                      },
                    ],
                  }],
                }],
