...
import { getImportQueueStatus, runImportQueueTick } from "../lib/import-queue";
import {
  addImportItems,
  claimQueuedBatch,
  createImport,
...
test("import queue claims untracked apps before already-tracked rescrapes", async () => {
  ...
  assert.equal(
    claimed[0].query,
    "Brand New",
    "untracked row must be claimed first"
  );
  assert.equal(
    claimed[1].query,
    "Already Tracked",
    "tracked rescrape must come last"
  );
});
...
test("import queue ignores pending_search rows even when they are past their next_attempt_at", async () => {
  ...
  const after = getImport(batch.id);
  const pending = after?.items.find(
    (i) => i.query === "Pending Search Fixture"
  );
  const queued = after?.items.find((i) => i.query === "Real Queued Fixture");
  assert.equal(
    pending?.status,
    "pending_search",
    "pending_search row must not be flipped to error"
  );
  assert.equal(
    pending?.attemptCount,
    0,
    "pending_search row must not have its attempt counter bumped"
  );
...
function installQueueFetchMock(input: {
  appStatus: number;
  retryAfter?: string;
}) {
  global.fetch = (async (raw: string | URL | Request) => {
    ...
    if (url.startsWith("https://apps.apple.com/")) {
      return new Response(
        input.appStatus === 200 ? appStoreHtml() : "rate limited",
        {
